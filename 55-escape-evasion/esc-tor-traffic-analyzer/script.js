/* Tor Traffic Analyzer — Onion routing traffic analysis simulation — Workshop DIY v1.2 */
...
const LANG={en:{kidsMode:'Kids Mode',kidsMascotHi:'Hi there, Agent!',kidsMission:'Mission Timer',kidsMissionDone:'Mission Complete!',kidsStickers:'Stickers',
 ...
 theory: 'Tor Traffic Analyzer démontre les principes clés de évasion. Tor (The Onion Router) provides anonymous communication by encrypting traffic through three relays. Each relay only knows the previous and next hop, not the full path. La simulation modélise ces mécanismes à l aide d équations mathématiques dans votre navigateur. Chaque contrôle correspond à un paramètre réel. En expérimentant ici, vous développez l intuition que les professionnels acquièrent après des années d expérience pratique.',
 ...
 theory: 'Tor Traffic Analyzer يوضح المبادئ الأساسية في الهروب والتملص. Tor (The Onion Router) provides anonymous communication by encrypting traffic through three relays. Each relay only knows the previous and next hop, not the full path. تحاكي هذه المحاكاة الآليات الحقيقية باستخدام معادلات رياضية في متصفحك. كل عنصر تحكم يتوافق مع معامل حقيقي. بالتجربة هنا تبني نفس الحدس الذي يطوره المحترفون عبر سنوات من الخبرة العملية.',
...
function drawCanvas(){if(!ctx)return;const w=canvas.offsetWidth,h=canvas.offsetHeight;ctx.fillStyle='rgba(10,10,26,0.12)';ctx.fillRect(0,0,w,h);const colors={circuit:'#a855f7',guard:'#22c55e',exit:'#ef4444'};const col=colors[currentMethod]||'#a855f7';
// Onion layers visualization
const cx=w/2,cy=h/2;for(let r=3;r>=0;r--){ctx.beginPath();ctx.arc(cx,cy,20+r*18+Math.sin(Date.now()/500+r)*5,0,Math.PI*2);ctx.strokeStyle=col;ctx.globalAlpha=0.15+r*0.1;ctx.lineWidth=2;ctx.stroke();ctx.globalAlpha=1}
particles.forEach((p,i)=>{p.x+=p.vx;p.y+=p.vy;p.life-=0.012;if(p.life<=0){particles.splice(i,1);return}ctx.beginPath();ctx.arc(p.x,p.y,p.r*p.life,0,Math.PI*2);ctx.fillStyle=col;ctx.globalAlpha=p.life*0.6;ctx.fill();ctx.globalAlpha=1});
const pct=Math.min(opCount/80,1);ctx.fillStyle='rgba(0,0,0,0.3)';ctx.fillRect(10,h-12,w-20,6);ctx.fillStyle=col+'80';ctx.fillRect(10,h-12,(w-20)*pct,6);ctx.font='7px monospace';ctx.fillStyle='rgba(255,255,255,0.3)';ctx.textAlign='center';ctx.fillText(s1+' circuits analyzed',w/2,h-2)}
function addParticles(){const w=canvas?canvas.offsetWidth:300,h=canvas?canvas.offsetHeight:200;for(let i=0;i<3;i++)particles.push({x:w*0.2,y:h/2+rI(-30,30),vx:1.5+Math.random()*2,vy:(Math.random()-0.5)*0.5,r:2+Math.random()*3,life:1});if(particles.length>200)particles.splice(0,50)}
function updateUI(){$('stat1').textContent=s1;$('stat2').textContent=s2;$('stat3').textContent=rI(50,400);$('stat4').textContent=Math.min(Math.round(opCount/80*100),100)+'%'}
...
function simTick(){const op=genOp();opCount++;s1++;s2+=rI(1,4);s3+=rI(1,3);addOpToList(op);addParticles();updateUI();log(`${op.type}: ${op.data}`,'tx')}
