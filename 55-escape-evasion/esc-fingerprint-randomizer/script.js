/**
 * Fingerprint Randomizer — Browser FP Spoofing
...
const LANG={en:{kidsMode:'Kids Mode',kidsMascotHi:'Hi there, Agent!',kidsMission:'Mission Timer',kidsMissionDone:'Mission Complete!',kidsStickers:'Stickers',
 ...
 theory: 'Fingerprint Randomizer يوضح المبادئ الأساسية في الهروب والتملص. Tor (The Onion Router) provides anonymous communication by encrypting traffic through three relays. Each relay only knows the previous and next hop, not the full path. تحاكي هذه المحاكاة الآليات الحقيقية باستخدام معادلات رياضية في متصفحك. كل عنصر تحكم يتوافق مع معامل حقيقي. بالتجربة هنا تبني نفس الحدس الذي يطوره المحترفون عبر سنوات من الخبرة العملية.',
...
/* ═══════ APP LOGIC — Fingerprint Randomizer ═══════ */
let simRunning=false,simInterval=null,opCount=0,s1=0,s2=0,s3=0,currentMethod='canvas';
...
const OPS=["Canvas hash randomized: 0x"+randHex(8),"WebGL vendor spoofed: NVIDIA GeForce","Audio context noise injected","Screen resolution masked: 1920x1080","Timezone offset shifted","Font list randomized","Plugin list shuffled","Navigator UA rotated","Color depth spoofed","WebRTC leak blocked","Hardware concurrency spoofed","Device memory masked"];
function genOp(){let op=OPS[randInt(0,OPS.length)];if(op.includes('0x'))op="Canvas hash: 0x"+randHex(8);return{type:currentMethod.toUpperCase().slice(0,4),data:op,status:'OK'}}
...
function drawCanvas(){if(!ctx)return;const w=canvas.offsetWidth,h=canvas.offsetHeight;ctx.fillStyle='rgba(10,10,26,0.12)';ctx.fillRect(0,0,w,h);const colors={canvas:'#f59e0b',webgl:'#3b82f6',audio:'#22c55e'};const col=colors[currentMethod]||'#f59e0b';particles.forEach((p,i)=>{p.x+=p.vx;p.y+=p.vy;p.life-=0.012;if(p.life<=0){particles.splice(i,1);return}ctx.beginPath();ctx.arc(p.x,p.y,p.r*p.life,0,Math.PI*2);ctx.fillStyle=col;ctx.globalAlpha=p.life*0.6;ctx.fill();ctx.globalAlpha=1;if(i>0&&i<particles.length){const prev=particles[i-1];ctx.beginPath();ctx.moveTo(prev.x,prev.y);ctx.lineTo(p.x,p.y);ctx.strokeStyle=col;ctx.globalAlpha=p.life*0.15;ctx.lineWidth=0.5;ctx.stroke();ctx.globalAlpha=1}});const t=(Date.now()%4000)/4000;const ax=t*w,ay=h/2+Math.sin(t*Math.PI*4)*25;ctx.beginPath();ctx.arc(ax,ay,4,0,Math.PI*2);ctx.fillStyle=col;ctx.globalAlpha=0.8;ctx.fill();ctx.globalAlpha=1;const pct=Math.min(opCount/80,1);ctx.fillStyle='rgba(0,0,0,0.3)';ctx.fillRect(10,h-12,w-20,6);ctx.fillStyle=col+'80';ctx.fillRect(10,h-12,(w-20)*pct,6);ctx.font='7px monospace';ctx.fillStyle='rgba(255,255,255,0.3)';ctx.textAlign='center';ctx.fillText(opCount+' fingerprints randomized',w/2,h-2)}
function addParticles(){const w=canvas?canvas.offsetWidth:300,h=canvas?canvas.offsetHeight:200;for(let i=0;i<3;i++)particles.push({x:Math.random()*w*0.3,y:h/2+Math.random()*40-20,vx:1+Math.random()*2,vy:(Math.random()-0.5)*0.8,r:2+Math.random()*4,life:1});if(particles.length>200)particles.splice(0,50)}
function updateUI(){$('stat1').textContent=s1;$('stat2').textContent='0x'+randHex(4);$('stat3').textContent=s3;$('stat4').textContent=Math.min(Math.round(opCount/80*100),100)+'%'}
...
function simTick(){const op=genOp();opCount++;s1++;s2+=randInt(1,5);s3+=randInt(1,3);addOpToList(op);addParticles();updateUI();log(`${op.type}: ${op.data}`,'tx')}
...
/* ═══════ RICH CANVAS — Fingerprint Entropy Map ═══════ */
(function richVis(){const CVS_ID='richVisCanvas';function ensureCanvas(){if(document.getElementById(CVS_ID))return document.getElementById(CVS_ID);const wrap=document.querySelector('.card');if(!wrap)return null;const card=document.createElement('div');card.className='card';card.innerHTML='<div class="card-header"><div class="card-title"><span class="icon">🖐️</span> Fingerprint Entropy Map</div></div>';const c=document.createElement('canvas');c.id=CVS_ID;c.style.cssText='width:100%;height:260px;border-radius:12px;background:#0a0a1a;display:block;margin-top:8px;';card.appendChild(c);wrap.parentNode.insertBefore(card,wrap.nextSibling);return c}let nodes=[],_raf=null,frame=0;function draw(){const c=document.getElementById(CVS_ID);if(!c){_raf=null;return}const ctx2=c.getContext('2d');const W=c.width=c.offsetWidth*2,H=c.height=c.offsetHeight*2;ctx2.scale(2,2);const w=W/2,h=H/2;ctx2.fillStyle='rgba(10,10,26,0.1)';ctx2.fillRect(0,0,w,h);frame++;const isRunning=typeof simRunning!=='undefined'&&simRunning;const colors={canvas:'#f59e0b',webgl:'#3b82f6',audio:'#22c55e'};const col=colors[typeof currentMethod!=='undefined'?currentMethod:'canvas']||'#f59e0b';if(isRunning&&frame%10===0)nodes.push({x:Math.random()*w,y:Math.random()*h,r:3+Math.random()*5,alpha:1,pulse:0});nodes.forEach((n,i)=>{n.alpha-=0.003;n.pulse+=0.05;if(n.alpha<=0){nodes.splice(i,1);return}ctx2.beginPath();ctx2.arc(n.x,n.y,n.r+Math.sin(n.pulse)*2,0,Math.PI*2);ctx2.fillStyle=col;ctx2.globalAlpha=n.alpha*0.5;ctx2.fill();ctx2.strokeStyle=col;ctx2.globalAlpha=n.alpha*0.3;ctx2.lineWidth=1;ctx2.stroke();ctx2.globalAlpha=1});for(let i=0;i<nodes.length;i++)for(let j=i+1;j<nodes.length;j++){const dx=nodes[i].x-nodes[j].x,dy=nodes[i].y-nodes[j].y,dist=Math.sqrt(dx*dx+dy*dy);if(dist<80){ctx2.beginPath();ctx2.moveTo(nodes[i].x,nodes[i].y);ctx2.lineTo(nodes[j].x,nodes[j].y);ctx2.strokeStyle=col;ctx2.globalAlpha=0.1*(1-dist/80);ctx2.lineWidth=0.5;ctx2.stroke();ctx2.globalAlpha=1}}if(nodes.length>80)nodes.splice(0,20);ctx2.font='8px monospace';ctx2.fillStyle='rgba(255,255,255,0.25)';ctx2.textAlign='center';ctx2.fillText('Fingerprint Entropy Map',w/2,h-8);_raf=requestAnimationFrame(draw)}function boot(){const c=ensureCanvas();if(!c)return setTimeout(boot,500);if(!_raf)draw()}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot()})();
