...
const LANG = {
 ...
 missionTitle:'MISSION BRIEFING',missionClassified:'CLASSIFIED',missionObjective:'Your mission objective:',missionAgent:'AGENT-112DF0',missionSkip:'Skip',missionGo:'ACCEPT MISSION',mission_obj:'Explore and master Genetic Algorithm Radio \u2014 analyze, experiment, and complete all challenges.',nightVisionTitle:'Night Vision Mode',nightVisionOn:'NV ON',nightVisionOff:'NV OFF',nightVisionAuto:'Auto NV',
 ...
 title: 'Genetic Algorithm Radio', subtitle: '📨 encrypt · 📡 drop · 🔓 decrypt',
 ...
 msgPlaceholder: 'Type your secret message...',
 keyPlaceholder: 'Shared encryption key...',
 encryptBtn: 'Encrypt', sendBtn: 'Send Drop', receiveBtn: 'Receive', scanBtn: 'Scan',
 bleOff: 'BLE: Idle', bleScan: 'BLE: Scanning...', bleTx: 'BLE: Transmitting...', bleRx: 'BLE: Receiving...',
 howStep1: 'Agent A writes a secret message and enters a shared key.',
 howStep2: 'The message is encrypted using XOR cipher with the shared key.',
 howStep3: 'The encrypted message is "dropped" via BLE broadcast into the air.',
 howStep4: 'Agent B scans, receives the drop, and decrypts with the same key.',
 challenge1: 'What happens if you use the wrong key to decrypt?',
 challenge2: 'Can you intercept a drop without the key?',
 challenge3: 'Design a key exchange protocol for two agents.',
 challengeReveal1: 'You get garbled nonsense! XOR decryption with the wrong key produces random characters. This is why key secrecy is critical.',
 challengeReveal2: 'Yes, you can intercept the BLE broadcast, but you only see encrypted bytes. Without the key, the message remains unreadable.',
 ...
 howto_1:'The main display shows the Swarm Genetic Algorithm Radio simulation. At the top you see the live visualization — colors and animations represent real data changing in real time. Below it, the control panel has buttons and sliders that each adjust a specific parameter. Start by looking at how Agent A writes a secret message and enters a shared key.',
 ...
 wiki_ble_title: '📡 BLE (Bluetooth Low Energy)', wiki_ble: 'A wireless protocol designed for low power. Devices broadcast advertising packets that nearby scanners can detect. Range: 10-50m.',
 wiki_xor_title: '🔑 XOR Cipher', wiki_xor: 'A symmetric cipher: plaintext XOR key = ciphertext. Ciphertext XOR key = plaintext. Simple but effective for learning.',
 wiki_ddp_title: '📨 Dead Drop Protocol', wiki_ddp: 'Agent A encrypts and broadcasts. Agent B scans and decrypts. No direct connection needed — just shared airwaves and a shared key.',
 wiki_radio_title: '📻 micro:bit Radio', wiki_radio: 'micro:bit v2 uses a 2.4GHz radio for BLE and custom radio groups. Packets are limited to ~20 bytes. RSSI measures signal strength.',
 working: 'Working…',
 encrypted: 'Encrypted', decrypted: 'Decrypted', dropped: 'Dropped', received: 'Received',
 noMsg: 'Enter a message first', noKey: 'Enter a key first', noDrops: 'No drops available',
 dropSent: 'Drop sent via BLE', dropReceived: 'Drop received!', scanning: 'Scanning for drops...',
 scanFound: 'Drop detected!', scanEmpty: 'No drops found nearby',
 t_mosque: 'Mosque', t_zellige: 'Zellige', t_andalus: 'Andalus',
 t_riad: 'Riad', t_medina: 'Medina',
 t_space: 'Space', t_jungle: 'Jungle', t_robot: 'Robot',
 ready: '📨 Dead Drop ready — enter your secret!',
 ...
 themeChanged: '🎨 Theme →',step1Title:'Initialize Swarm',step1Desc:'Agent A writes a secret message and enters a shared key. Watch the visualization update in real time as this stage processes. The display shows exactly what is happening internally — each color and movement represents a specific data transformation.',step2Title:'Set Rules',step2Desc:'The message is encrypted using XOR cipher with the shared key. Notice how the indicators change during this phase. The activity log records every event, letting you trace the exact sequence of operations and verify the results.',step3Title:'Simulate',step3Desc:'The encrypted message is "dropped" via BLE broadcast into the air. This stage transforms the input data using the algorithm shown in the visualization. Compare the before and after values to understand the mathematical relationship.',step4Title:'Analyze Patterns',step4Desc:'Agent B scans, receives the drop, and decrypts with the same key. The output of this stage feeds into the next one. Try pausing here to examine the intermediate state — understanding each step separately builds deeper insight.',sectionCode:'Device Code',faq_q1:'What is Genetic Algorithm Radio?',faq_a1:'Swarm Genetic Algorithm Radio is an interactive simulation that demonstrates earth science concepts. Encrypt and exchange secret messages via BLE simulation. Everything runs in your browser — no hardware or installation needed. Adjust the controls, observe the results, and build real understanding through experimentation.',faq_q2:'How does the simulation work?',faq_a2:'The simulation models real swarm intelligence behavior in your browser. You control the inputs using sliders and buttons, and the visualization updates in real time to show you the results.',faq_q3:'What do the controls do?',faq_a3:'Press "Start" to begin. Each button and slider changes a specific parameter — the visualization responds immediately so you can see the effect. Check the How-To tab for a step-by-step guide.',faq_q4:'What is the science behind this?',faq_a4:'This app is based on real swarm intelligence principles used by professionals. The simulation applies the same math and physics — the difference is that here you can safely experiment without expensive equipment.',faq_q5:'What should I experiment with?',faq_a5:'Change one parameter at a time and observe the effect. Try extreme values to find the limits. Then combine changes to see how different factors interact. The challenges section gives you specific experiments to try.',faq_q6:'What hardware do I need for the real version?',faq_a6:'The simulation needs no hardware. To build the real project, you need ESP32 / micro:bit. See the Device Code section for wiring diagrams and ready-to-use firmware.',faq_q7:'Is my data private?',faq_a7:'Yes. Everything runs locally in your browser using JavaScript. No data is sent to any server, no account is needed, and the app works completely offline. Your experiments stay on your device.',faq_q8:'What should I explore next?',faq_a8:'Try Swarm Ant Colony Mesh and Swarm Cellular Automata Mesh. Each app in this category teaches a different aspect of swarm intelligence.',demo_s1:'Welcome to Genetic Algorithm Radio! Look at the main display — this is where the swarm intelligence simulation runs.',demo_s2:'Type a secret message and a shared key in the main card. Watch how the visualization reacts.',demo_s3:'Try adjusting the controls. Each slider or button changes a specific parameter of the simulation.',demo_s4:'Scroll down to see "How It Works" for detailed data. The numbers and charts update as the simulation runs.',demo_s5:'Great job! Now try the challenges section to test your understanding of swarm intelligence.',sectionDemo:'Watch Demo',demoPlay:'Play',demoPause:'Pause',demoPrev:'Prev',demoNext:'Next',learn1Title:'Emergent Behavior',learn1Desc:'How simple rules create complex group patterns. Watch the simulation to see this happen in real time. The visualization makes the invisible visible.',learn1Tag:'AI',learn2Title:'Mesh Networks',learn2Desc:'How swarms communicate without a central leader. The controls let you experiment with different conditions. Each change reveals how this principle responds.',learn2Tag:'Networking',learn3Title:'Algorithms',learn3Desc:'How nature-inspired math solves hard problems. Try the challenges section to test your understanding. Real engineers use these same concepts daily.',learn3Tag:'Computing',learn4Title:'Optimization',learn4Desc:'How swarms find the best solution together. Compare results with different settings to build intuition. The data panels show precise measurements.',learn4Tag:'Math',sectionLearn:'What You Shall Learn',learnLevelVal:'Intermediate 🟡',learnLevel:'Level:',learnTimeVal:'20 min ⏱',learnTime:'Time:',learnAgeVal:'12+ 🧒',kidTitle:'For Young Explorers',kidIntro:'Welcome to Swarm Genetic Algorithm Radio! This is like a science experiment on your computer. You get to control a real earth science simulation — press buttons, move sliders, and watch what happens on screen. Try changing the controls and watch how Agent A writes a secret message and enters a share Nothing can break — it is all just a simulation running safely in your browser!',kidSafe:'Completely safe! Nothing you do here can break anything. It all runs inside your browser like a game.',kidTry:'Press the big Start button and watch the screen change. Then try moving sliders to see what they do.',kidParent:'This app teaches swarm intelligence concepts through hands-on simulation. Suitable for STEM education in physics, electronics, and computer science.',ch1Title:'Parameter Sweep',ch1Desc:'Systematically change one variable while keeping others constant. Record the results. Can you find the relationship between input and output?',ch2Title:'Edge Case',ch2Desc:'Push a parameter to its extreme value. What happens? Does the system behave differently at the boundary? Why?',ch3Title:'Predict Then Test',ch3Desc:'Before pressing Start, predict what will happen based on the current settings. Were you right? What did you miss?',codeTitle:'Starter Code',codeLang:'Arduino (ESP32)',codeSnippet:'#include <WiFi.h>\\n\\nconst char* ssid = "MyNetwork";\\nconst char* pass = "secret123";\\n\\nvoid setup() {\\n Serial.begin(115200);\\n WiFi.mode(WIFI_STA);\\n WiFi.begin(ssid, pass);\\n while (WiFi.status() != WL_CONNECTED) {\\n delay(500);\\n Serial.print(".");\\n }\\n Serial.println("\\nConnected!");\\n Serial.println(WiFi.localIP());\\n}\\n\\nvoid loop() {\\n // Your code here\\n}',codeExplain:'This Arduino sketch connects the ESP32 to a WiFi network. WiFi.mode(WIFI_STA) sets it as a client (station mode). The while loop waits until connected, then prints the IP address. From here you can add HTTP servers, MQTT, UDP packets, or BLE scanning.',purpose:'Genetic Algorithm Radio: Encrypt and exchange secret messages via BLE simulation. This simulation lets you experiment hands-on instead of just reading theory. Every parameter you change produces visible results, building real intuition for how the system behaves. The workflow goes from Initialize Swarm through Set Rules to Simulate and Analyze Patterns.',guideTitle:'What Am I Looking At?',guideCanvas:'The main area shows a live visualization of the simulation. Colors and movement represent data changing in real time.',guideControls:'The buttons below the main display control the simulation. Start begins it, Stop pauses it, Reset clears everything.',guideSections:'Below the main card, "How It Works" and "Lab" show detailed data and analysis. Click the section headers to expand or collapse them.',guideStatus:'The colored dot in the top-right corner shows the connection status. Green means running, red means stopped.',learnAge:'Ages:',relatedTitle:'\ud83d\udd17 Related Apps',related1_name:'Acoustic Fence',related1_desc:'Emit ultrasonic tone, detect Doppler shifts from movement',related1_path:'../../44-acoustic-warfare/sonic-acoustic-fence/index.html',related2_name:'Dead Drop — BLE Message Transfer',related2_desc:'Encrypt and exchange secret messages via BLE simulation',related2_path:'../../45-time-manipulation/chrono-time-reversal-mirror/index.html',related3_name:'Thermal Signature \\u2014 Through-Wall Detection',related3_desc:'Detect human thermal signatures through walls using RF',related3_path:'../../43-bio-radio/bio-thermal-signature/index.html',pathTitle:'\ud83d\udee4\ufe0f Learning Path',pathPrev_name:'Dead Drop — BLE Message Transfer',pathPrev_path:'../../46-swarm-intelligence/swarm-flock-formation/index.html',pathNext_name:'Dead Drop — BLE Message Transfer',pathNext_path:'../../46-swarm-intelligence/swarm-hive-mind-sensor/index.html',
 shortcutsTitle:'⌨️ Keyboard Shortcuts',
 shortcutsInfo:'? = Help, Esc = Close, S = Start, R = Reset, 1-9 = Tabs',
 achieveTitle:'🏆 Achievements',
 achieveExplorer:'Explorer — visited 4+ help tabs',
 ...
 achieveExperimenter:'Experimenter — changed 5+ parameters'
 ,
 printBtn: '🖨️ Print Worksheet',quizTab:'Quiz',quizTitle:'Test Your Knowledge',quizRetry:'Retry',quizCorrect:'Correct!',quizWrong:'Wrong!',quizScore:'Score',quiz_q1:'What is a key advantage of swarm systems?',quiz_q1a:'Central control',quiz_q1b:'Robustness through redundancy',quiz_q1c:'Single point of failure',quiz_q1d:'High cost',quiz_q1_answer:'1',quiz_q2:'In Morse code, what represents the letter E?',quiz_q2a:'Dash',quiz_q2b:'Single dot',quiz_q2c:'Two dots',quiz_q2d:'Dot-dash',quiz_q2_answer:'1',quiz_q3:'Which natural system inspired swarm algorithms?',quiz_q3a:'Planets',quiz_q3b:'Ant colonies',quiz_q3c:'Rivers',quiz_q3d:'Mountains',quiz_q3_answer:'1',quiz_q4:'What does SOS look like in Morse code?',quiz_q4a:'---...---',quiz_q4b:'...---...',quiz_q4c:'...-...-',quiz_q4d:'-.-.-.',quiz_q4_answer:'1',quiz_q5:'What does AM stand for in radio?',quiz_q5a:'Audio Modulation',quiz_q5b:'Amplitude Modulation',quiz_q5c:'Analog Modulation',quiz_q5d:'Active Modulation',quiz_q5_answer:'1',realworldTitle:'🌍 Real-World Stories',realworld1:'The SolarWinds attack (2020) compromised 18,000 organizations by hiding malware inside trusted software updates. Attackers had 9 months of undetected access to US Treasury, Commerce, and Homeland Security systems.',realworld2:'Heartbleed (2014) was a buffer overflow in OpenSSL that let attackers read 64KB of server memory per request — potentially grabbing private keys, passwords, and session tokens from any HTTPS server worldwide.',realworld3:'Alan Turing\'s team at Bletchley Park cracked the Enigma machine during WWII, reading 84,000 encrypted German messages per month by 1945. This achievement shortened the war by an estimated 2 years.',experimentTitle:'🔬 Experiments',experiment_1_title:'Baseline Measurement',experiment_1:'Set all controls to default values and record the initial readings. These are your baseline measurements. Good scientists always establish a baseline before changing variables — it gives you a reference point to measure all future changes against.',experiment_2_title:'Sensitivity Analysis',experiment_2:'Change one parameter to its minimum value, record the result, then set it to maximum. The difference reveals the system\'s sensitivity to that variable. Repeat for each control. In advanced cryptography, knowing which parameters matter most helps you focus your efforts efficiently.',experiment_3_title:'Interaction Effects',experiment_3:'After testing parameters individually, change two simultaneously. Does the combined effect equal the sum of individual effects? Or is there a synergy (or cancellation)? Non-linear interactions are common in advanced cryptography and reveal the hidden complexity beneath simple-looking systems.',wiki_concept_title:'💡 Core Concept',wiki_concept:'Genetic Algorithm Radio demonstrates a fundamental concept in advanced cryptography. At its core, this simulation models how real systems process signals, data, or physical phenomena. The key insight is that complex behaviors emerge from simple rules applied repeatedly. Understanding this principle — that sophisticated outcomes arise from basic building blocks — is the foundation of engineering and scientific thinking.',wiki_realworld_title:'🌐 Real-World Applications',wiki_realworld:'The principles demonstrated in Genetic Algorithm Radio have direct real-world applications. Professionals in advanced cryptography use these same concepts daily. In industry, browser and similar hardware implement these algorithms in embedded systems. In research, these models help scientists predict and analyze complex phenomena. The skills you develop here — systematic experimentation, parameter tuning, and data interpretation — are exactly what employers seek.',wiki_safety_title:'⚠️ Safety & Responsibility',wiki_safety:'Working with advanced cryptography carries important responsibilities. Always operate within legal boundaries — many countries regulate equipment and techniques in this field. Never test on systems you do not own without explicit written permission. This simulation is designed for safe educational use — it does not transmit real signals or access real networks. When you progress to real hardware, research your local regulations first.',proTipTitle:'💡 Pro Tips',proTip1:'Use your browser\\x27s Performance tab to measure frame rate. If the simulation drops below 30fps, reduce the data points or update interval for smoother animation.',proTip2:'Export simulation data using the Copy button, paste into a spreadsheet, and create your own charts. Comparing multiple runs in a chart reveals patterns invisible on screen.',funFactTitle:'🎯 Did You Know?',funFact:'A 256-bit AES key has more possible combinations (2²⁵⁶) than atoms in the observable universe (≈2²⁶⁶). Even checking a trillion keys per second, brute-forcing would take longer than the age of the universe.',mistakeTitle:'⚠️ Common Mistakes',mistake1:'Changing multiple parameters at once makes it impossible to isolate cause and effect. Always change ONE variable at a time.',mistake2:'Skipping the baseline measurement. Without knowing the default behavior, you cannot measure how your changes affect the system.',mistake3:'Ignoring the activity log. It records every event with timestamps — essential for understanding sequences and debugging unexpected results.'},
 wiki_history_title: '📜 History of Earth Science',
 wiki_history: 'Encryption dates back to ancient Egypt (1900 BCE). The Caesar cipher, Enigma machine, and modern AES represent key milestones in cryptographic evolution. Swarm Genetic Algorithm Radio builds on this foundation, letting you explore these historical concepts through interactive simulation.',
 wiki_math_title: '📐 Mathematics Behind Swarm Genetic Algorithm Radio',
 wiki_math: 'The mathematics behind Swarm Genetic Algorithm Radio: Modern encryption relies on computational hardness: integer factorization (RSA), discrete logarithms (Diffie-Hellman), and elliptic curves (ECC). Cipher strength is measured in bits of security. AES-256 provides 256-bit security, meaning 2^256 operations to brute-force — more than atoms in the universe. XOR has perfect balance: for any fixed key bit, the output is equally likely to be 0 or 1. This property makes it ideal for mixing key material with plaintext.',
 wiki_advanced_title: '🔬 Advanced Techniques',
 wiki_advanced: 'Beyond the basics demonstrated in this simulation, advanced earth science practitioners use sophisticated techniques. Adaptive algorithms automatically adjust parameters based on environmental conditions. Machine learning classifies signals with high accuracy. Multi-channel correlation detects patterns invisible to single-channel analysis. Distributed sensor networks combine data from multiple locations for triangulation. These techniques build on the fundamentals you learn here.',
 wiki_compare_title: '⚖️ Comparing Approaches',
 wiki_compare: 'There are several approaches to earth science. Hardware-based solutions using browser offer real-time performance and direct signal access. Software simulations (like this app) provide safe experimentation without equipment cost. Cloud-based platforms offer scalability but introduce latency and privacy concerns. Each approach has trade-offs: cost vs. fidelity, speed vs. safety, simplicity vs. capability. This simulation gives you the conceptual foundation to use any approach effectively.',
 wiki_debug_title: '🔧 Troubleshooting Guide',
 wiki_debug: 'Common issues when working with earth science: (1) Unexpected results often come from incorrect parameter settings — reset to defaults and change one variable at a time. (2) If the visualization seems frozen, check that the simulation is running (not paused). (3) Noisy or erratic readings usually indicate interference — in real hardware, move away from electronic devices. (4) If calculations seem wrong, verify your units (Hz vs kHz vs MHz). Systematic debugging is a core skill in earth science.',
 wiki_ethics_title: '⚖️ Ethics & Legal Considerations',
 wiki_ethics: 'Earth Science carries important ethical and legal responsibilities. Many countries regulate the use of browser and similar equipment. Always obtain proper authorization before testing on systems you do not own. Respect privacy laws and data protection regulations. This simulation is designed for educational purposes — it demonstrates principles without transmitting real signals or accessing real networks. Responsible use of these skills contributes to security for everyone.',
 ...
 gloss2_term: 'Key Space',
 gloss2_def: 'The total number of possible keys for a cipher. Larger key spaces make brute-force attacks computationally infeasible.',
 gloss3_term: 'One-Time Pad',
 gloss3_def: 'An encryption technique using a random key as long as the message, XORed once. Mathematically proven unbreakable if the key is truly random and never reused.',
 gloss4_term: 'Port',
 ...
 theory: 'Swarm Genetic Algorithm Radio demonstrates key principles from earth science. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. A cipher is a specific algorithm for performing encryption and decryption. Block ciphers process fixed-size chunks; stream ciphers process one bit at a time. XOR (exclusive OR) is a bitwise operation where the output is 1 only when inputs differ. It is reversible: A⊕B⊕B = A, making it fundamental to many ciphers. The simulation models these real-world mechanisms using mathematical equations running in your browser. Every control maps to a real parameter that engineers tune in professional settings. By experimenting here, you build the same intuition that professionals develop through years of hands-on experience.',
 faq_q9: 'What common mistakes should I avoid?',
 faq_a9: 'The most common mistake is changing multiple parameters at once, which makes it impossible to understand cause and effect. Always change one thing at a time. Another common error is ignoring the activity log — it records every event and helps you understand the sequence of operations. Finally, do not skip the challenge section: those questions test whether you truly understand the concepts or just memorized the button sequence.',
 faq_q10: 'How does this relate to real-world earth science?',
 faq_a10: 'This simulation models the same physics and mathematics used in professional earth science systems. The parameters you adjust correspond to real equipment settings. The visualizations show data patterns identical to what you would see on professional instruments like oscilloscopes, spectrum analyzers, or protocol decoders. The main difference is that this runs safely in your browser — real systems use browser hardware and may have legal requirements for operation. Skills you develop here transfer directly to hands-on work.',
 ...
 missionTitle:'BRIEFING DE MISSION',missionClassified:'CLASSIFI\xc9',missionObjective:'Objectif de mission :',missionAgent:'AGENT-112DF0',missionSkip:'Passer',missionGo:'ACCEPTER LA MISSION',mission_obj:'Explorer et ma\xeetrisez Genetic Algorithm Radio \u2014 analysez, exp\xe9rimentez et compl\xe9tez tous les d\xe9fis.',nightVisionTitle:'Mode Vision Nocturne',nightVisionOn:'VN ON',nightVisionOff:'VN OFF',nightVisionAuto:'VN Auto',
 ...
 title: 'Genetic Algorithm Radio', subtitle: '📨 chiffrer · 📡 déposer · 🔓 déchiffrer',
 ...
 msgPlaceholder: 'Tapez votre message secret...',
 keyPlaceholder: 'Clé de chiffrement partagée...',
 encryptBtn: 'Chiffrer', sendBtn: 'Envoyer Drop', receiveBtn: 'Recevoir', scanBtn: 'Scanner',
 bleOff: 'BLE : Inactif', bleScan: 'BLE : Scan...', bleTx: 'BLE : Émission...', bleRx: 'BLE : Réception...',
 howStep1: 'L\'agent A écrit un message secret et entre une clé partagée.',
 howStep2: 'Le message est chiffré avec le chiffrement XOR et la clé partagée.',
 howStep3: 'Le message chiffré est "déposé" par diffusion BLE dans les airs.',
 howStep4: 'L\'agent B scanne, reçoit le drop et déchiffre avec la même clé.',
 challenge1: 'Que se passe-t-il si vous utilisez la mauvaise clé pour déchiffrer ?',
 challenge2: 'Peut-on intercepter un drop sans la clé ?',
 challenge3: 'Concevez un protocole d\'échange de clés pour deux agents.',
 challengeReveal1: 'Vous obtenez du charabia ! Le déchiffrement XOR avec la mauvaise clé produit des caractères aléatoires. C\'est pourquoi le secret de la clé est crucial.',
 challengeReveal2: 'Oui, vous pouvez intercepter la diffusion BLE, mais vous ne voyez que des octets chiffrés. Sans la clé, le message reste illisible.',
 ...
 howto_1:'L écran principal affiche la simulation Swarm Genetic Algorithm Radio. En haut, la visualisation en direct — les couleurs et animations représentent des données réelles changeant en temps réel. En dessous, le panneau de contrôle a des boutons et curseurs qui ajustent chaque paramètre. Start by looking at how Agent A writes a secret message and enters a shared key.',
 ...
 wiki_ble_title: '📡 BLE (Bluetooth Low Energy)', wiki_ble: 'Un protocole sans fil basse consommation. Les appareils diffusent des paquets que les scanners à proximité détectent. Portée : 10-50m.',
 wiki_xor_title: '🔑 Chiffrement XOR', wiki_xor: 'Un chiffrement symétrique : texte XOR clé = chiffré. Chiffré XOR clé = texte. Simple mais efficace pour apprendre.',
 wiki_ddp_title: '📨 Protocole Dead Drop', wiki_ddp: 'L\'agent A chiffre et diffuse. L\'agent B scanne et déchiffre. Pas de connexion directe — juste les ondes et une clé partagée.',
 wiki_radio_title: '📻 Radio micro:bit', wiki_radio: 'Le micro:bit v2 utilise une radio 2.4GHz pour BLE et les groupes radio. Paquets limités à ~20 octets. RSSI mesure la puissance du signal.',
 working: 'En cours…',
 encrypted: 'Chiffré', decrypted: 'Déchiffré', dropped: 'Déposé', received: 'Reçu',
 noMsg: 'Entrez d\'abord un message', noKey: 'Entrez d\'abord une clé', noDrops: 'Aucun drop disponible',
 dropSent: 'Drop envoyé via BLE', dropReceived: 'Drop reçu !', scanning: 'Recherche de drops...',
 scanFound: 'Drop détecté !', scanEmpty: 'Aucun drop trouvé à proximité',
 t_mosque: 'Mosquée', t_zellige: 'Zellige', t_andalus: 'Andalous',
 t_riad: 'Riad', t_medina: 'Médina',
 t_space: 'Espace', t_jungle: 'Jungle', t_robot: 'Robot',
 ready: '📨 Dead Drop prêt — entrez votre secret !',
 ...
 themeChanged: '🎨 Thème →',step1Title:'Initialiser l\'essaim',step1Desc:'L\'agent A écrit un message secret et entre une clé partagée. Regardez la visualisation se mettre à jour en temps réel pendant cette étape. L affichage montre exactement ce qui se passe en interne — chaque couleur et mouvement représente une transformation.',step2Title:'Définir les règles',step2Desc:'Le message est chiffré avec le chiffrement XOR et la clé partagée. Remarquez comment les indicateurs changent pendant cette phase. Le journal d activité enregistre chaque événement pour vérifier les résultats.',step3Title:'Simuler',step3Desc:'Le message chiffré est "déposé" par diffusion BLE dans les airs. Cette étape transforme les données d entrée selon l algorithme affiché. Comparez les valeurs avant et après pour comprendre la relation mathématique.',step4Title:'Analyser les motifs',step4Desc:'L\'agent B scanne, reçoit le drop et déchiffre avec la même clé. Le résultat de cette étape alimente la suivante. Essayez de faire pause ici pour examiner l état intermédiaire.',sectionCode:'Code Appareil',faq_q1:'Qu\'est-ce que Genetic Algorithm Radio ?',faq_a1:'Swarm Genetic Algorithm Radio est une simulation interactive qui démontre les concepts de sciences de la Terre. Encrypt and exchange secret messages via BLE simulation. Tout fonctionne dans votre navigateur — aucun matériel requis. Ajustez les contrôles, observez les résultats et construisez une vraie compréhension par l expérimentation.',faq_q2:'Comment fonctionne la simulation ?',faq_a2:'L\'application modélise un vrai comportement de intelligence en essaim. Tu contrôles les entrées et tu observes les sorties changer en temps réel à l\'écran.',faq_q3:'Que font les contrôles ?',faq_a3:'Chaque bouton et curseur modifie un paramètre spécifique. Consulte l\'onglet Guide pour une procédure pas à pas.',faq_q4:'Quelle est la science derrière ?',faq_a4:'Cette application utilise de vrais principes de intelligence en essaim. Les mêmes concepts sont utilisés par les professionnels.',faq_q5:'Que dois-je expérimenter ?',faq_a5:'Change un paramètre à la fois et observe l\'effet. Pousse les valeurs à l\'extrême pour voir les limites du système.',faq_q6:'Quel matériel pour la version réelle ?',faq_a6:'La simulation ne nécessite aucun matériel. Pour construire le vrai projet, il te faut ESP32 / micro:bit. Voir la section Code Appareil.',faq_q7:'Mes données sont-elles privées ?',faq_a7:'Oui. Tout fonctionne localement dans ton navigateur. Aucune donnée n\'est envoyée, aucun compte nécessaire, et ça marche hors ligne.',faq_q8:'Que découvrir ensuite ?',faq_a8:'Essaie d\'autres applications de cette catégorie. Chacune enseigne un aspect différent de intelligence en essaim.',demo_s1:'Bienvenue dans Genetic Algorithm Radio ! Regarde l\'écran principal — c\'est ici que la simulation de intelligence en essaim fonctionne.',demo_s2:'Clique sur Démarrer et regarde la visualisation réagir.',demo_s3:'Essaie d\'ajuster les contrôles. Chaque curseur ou bouton modifie un paramètre spécifique.',demo_s4:'Descends pour voir les données détaillées. Les chiffres et graphiques se mettent à jour en temps réel.',demo_s5:'Bravo ! Maintenant essaie les défis pour tester ta compréhension de intelligence en essaim.',sectionDemo:'Voir la Démo',demoPlay:'Jouer',demoPause:'Pause',demoPrev:'Préc',demoNext:'Suiv',learn1Title:'Emergent Behavior',learn1Desc:'How simple rules create complex group patterns. Regarde la simulation pour voir ça en temps réel. La visualisation rend visible l\'invisible.',learn1Tag:'AI',learn2Title:'Mesh Networks',learn2Desc:'How swarms communicate without a central leader. Les contrôles te permettent d\'expérimenter. Chaque changement révèle comment ce principe réagit.',learn2Tag:'Networking',learn3Title:'Algorithms',learn3Desc:'How nature-inspired math solves hard problems. Essaie les défis pour tester ta compréhension. Les vrais ingénieurs utilisent ces mêmes concepts.',learn3Tag:'Computing',learn4Title:'Optimization',learn4Desc:'How swarms find the best solution together. Compare les résultats avec différents réglages. Les panneaux de données montrent des mesures précises.',learn4Tag:'Math',sectionLearn:'Ce que tu vas apprendre',learnLevelVal:'Intermédiaire 🟡',learnLevel:'Niveau :',learnTimeVal:'20 min ⏱',learnTime:'Durée :',learnAgeVal:'12+ 🧒',kidTitle:'Pour les Jeunes Explorateurs',kidIntro:'Bienvenue dans Swarm Genetic Algorithm Radio ! C est comme une expérience scientifique sur ton ordinateur. Tu contrôles une vraie simulation de sciences de la Terre — appuie sur les boutons, bouge les curseurs et regarde ce qui se passe. Try changing the controls and watch how Agent A writes a secret message and enters a share Rien ne peut casser — tout est une simulation qui tourne en sécurité dans ton navigateur !',kidSafe:'Complètement sûr ! Rien de ce que tu fais ici ne peut casser quoi que ce soit. Tout fonctionne dans ton navigateur comme un jeu.',kidTry:'Appuie sur le gros bouton Démarrer et regarde l\'écran changer. Puis essaie de bouger les curseurs.',kidParent:'Cette appli enseigne des concepts de intelligence en essaim par simulation interactive. Adaptée à l\'enseignement STEM en physique, électronique et informatique.',ch1Title:'Balayage de Paramètre',ch1Desc:'Change systématiquement une variable en gardant les autres constantes. Peux-tu trouver la relation entrée-sortie ?',ch2Title:'Cas Limite',ch2Desc:'Pousse un paramètre à sa valeur extrême. Que se passe-t-il ? Le système se comporte-t-il différemment aux limites ?',ch3Title:'Prédis Puis Teste',ch3Desc:'Avant de démarrer, prédis le résultat. Avais-tu raison ? Qu\'as-tu manqué ?',codeTitle:'Code de Démarrage',codeLang:'Arduino (ESP32)',codeSnippet:'#include <WiFi.h>\\n\\nconst char* ssid = "MyNetwork";\\nconst char* pass = "secret123";\\n\\nvoid setup() {\\n Serial.begin(115200);\\n WiFi.mode(WIFI_STA);\\n WiFi.begin(ssid, pass);\\n while (WiFi.status() != WL_CONNECTED) {\\n delay(500);\\n Serial.print(".");\\n }\\n Serial.println("\\nConnected!");\\n Serial.println(WiFi.localIP());\\n}\\n\\nvoid loop() {\\n // Your code here\\n}',codeExplain:'Ce sketch Arduino connecte l\'ESP32 à un réseau WiFi. WiFi.mode(WIFI_STA) le configure en mode client. La boucle while attend la connexion, puis affiche l\'adresse IP. Ensuite tu peux ajouter des serveurs HTTP, MQTT, UDP ou du scan BLE.',purpose:'Genetic Algorithm Radio : Encrypt and exchange secret messages via BLE simulation. Cette simulation te permet d\'expérimenter au lieu de simplement lire la théorie. Chaque paramètre que tu changes produit des résultats visibles, construisant une vraie intuition du comportement du système.',guideTitle:'Que vois-je à l\'écran ?',guideCanvas:'La zone principale affiche une visualisation en direct de la simulation. Les couleurs et mouvements représentent les données en temps réel.',guideControls:'Les boutons sous l\'écran principal contrôlent la simulation. Démarrer la lance, Arrêter la met en pause, Réinitialiser efface tout.',guideSections:'Sous la carte principale, les sections montrent les données détaillées et l\'analyse. Clique sur les en-têtes pour les déplier.',guideStatus:'Le point coloré en haut à droite montre l\'état. Vert signifie en marche, rouge signifie arrêté.',learnAge:'Âge :',relatedTitle:'\ud83d\udd17 Apps Similaires',related1_name:'Cloture Acoustique',related1_desc:'Emettre un ultrason, detecter les mouvements par effet Doppler',related1_path:'../../44-acoustic-warfare/sonic-acoustic-fence/index.html',related2_name:'Dead Drop — Transfert BLE',related2_desc:'Chiffrez et échangez des messages secrets via simulation BLE',related2_path:'../../45-time-manipulation/chrono-time-reversal-mirror/index.html',related3_name:'Signature Thermique \\u2014 D\\u00e9tection Murale',related3_desc:'D\\u00e9tecter les signatures thermiques humaines',related3_path:'../../43-bio-radio/bio-thermal-signature/index.html',pathTitle:'\ud83d\udee4\ufe0f Parcours',pathPrev_name:'Dead Drop — Transfert BLE',pathPrev_path:'../../46-swarm-intelligence/swarm-flock-formation/index.html',pathNext_name:'Dead Drop — Transfert BLE',pathNext_path:'../../46-swarm-intelligence/swarm-hive-mind-sensor/index.html',
 printBtn: '🖨️ Imprimer',quizTab:'Quiz',quizTitle:'Testez vos connaissances',quizRetry:'Rejouer',quizCorrect:'Correct !',quizWrong:'Faux !',quizScore:'Score',quiz_q1:'Quel est l\'avantage clé des systèmes en essaim ?',quiz_q1a:'Contrôle central',quiz_q1b:'Robustesse par redondance',quiz_q1c:'Point unique de défaillance',quiz_q1d:'Coût élevé',quiz_q1_answer:'1',quiz_q2:'En Morse, que représente la lettre E ?',quiz_q2a:'Trait',quiz_q2b:'Un point',quiz_q2c:'Deux points',quiz_q2d:'Point-trait',quiz_q2_answer:'1',quiz_q3:'Quel système naturel a inspiré les algorithmes en essaim ?',quiz_q3a:'Planètes',quiz_q3b:'Colonies de fourmis',quiz_q3c:'Rivières',quiz_q3d:'Montagnes',quiz_q3_answer:'1',quiz_q4:'Comment s\'écrit SOS en code Morse ?',quiz_q4a:'---...---',quiz_q4b:'...---...',quiz_q4c:'...-...-',quiz_q4d:'-.-.-.',quiz_q4_answer:'1',quiz_q5:'Que signifie AM en radio ?',quiz_q5a:'Modulation Audio',quiz_q5b:'Modulation d\'Amplitude',quiz_q5c:'Modulation Analogique',quiz_q5d:'Modulation Active',quiz_q5_answer:'1',realworldTitle:'🌍 Histoires réelles',realworld1:'L\'attaque SolarWinds (2020) a compromis 18 000 organisations en cachant des malwares dans des mises à jour logicielles de confiance.',realworld2:'Heartbleed (2014) était un dépassement de tampon dans OpenSSL qui permettait aux attaquants de lire 64 Ko de mémoire serveur par requête.',realworld3:'L\'équipe d\'Alan Turing à Bletchley Park a décrypté la machine Enigma pendant la WWII, lisant 84 000 messages allemands chiffrés par mois en 1945.',experimentTitle:'🔬 Expériences',experiment_1_title:'Mesure de référence',experiment_1:'Réglez tous les contrôles sur les valeurs par défaut et notez les lectures initiales. Ce sont vos mesures de référence. Un bon scientifique établit toujours une référence avant de modifier des variables — cela donne un point de comparaison pour mesurer tous les changements futurs.',experiment_2_title:'Analyse de sensibilité',experiment_2:'Changez un paramètre à sa valeur minimale, notez le résultat, puis réglez-le au maximum. La différence révèle la sensibilité du système à cette variable. En cryptographie avancée, savoir quels paramètres comptent le plus vous aide à concentrer vos efforts.',experiment_3_title:'Effets d\'interaction',experiment_3:'Après avoir testé les paramètres individuellement, changez-en deux simultanément. L\'effet combiné est-il égal à la somme des effets individuels? Les interactions non linéaires sont courantes en cryptographie avancée et révèlent la complexité cachée sous des systèmes simples en apparence.',wiki_concept_title:'💡 Concept fondamental',wiki_concept:'Genetic Algorithm Radio illustre un concept fondamental en cryptographie avancée. Cette simulation modélise comment les systèmes réels traitent les signaux, les données ou les phénomènes physiques. L\'idée clé est que des comportements complexes émergent de règles simples appliquées de manière répétée.',wiki_realworld_title:'🌐 Applications réelles',wiki_realworld:'Les principes démontrés dans Genetic Algorithm Radio ont des applications directes dans le monde réel. Les professionnels de cryptographie avancée utilisent ces mêmes concepts quotidiennement. Dans l\'industrie, browser et du matériel similaire implémentent ces algorithmes dans des systèmes embarqués.',wiki_safety_title:'⚠️ Sécurité et responsabilité',wiki_safety:'Travailler en cryptographie avancée implique des responsabilités importantes. Opérez toujours dans les limites légales. Cette simulation est conçue pour un usage éducatif sûr — elle ne transmet pas de vrais signaux et n\'accède pas à de vrais réseaux.',proTipTitle:'💡 Conseils de pro',proTip1:'Utilisez l\x27onglet Performance de votre navigateur pour mesurer le taux de rafraîchissement. Si la simulation tombe sous 30fps, réduisez les points de données.',proTip2:'Exportez les données avec le bouton Copier, collez dans un tableur et créez vos propres graphiques.',funFactTitle:'🎯 Le saviez-vous ?',funFact:'Une clé AES 256 bits a plus de combinaisons possibles (2²⁵⁶) que d\x27atomes dans l\x27univers observable. Même en testant un billion de clés par seconde, le forçage prendrait plus longtemps que l\x27ge de l\x27univers.',mistakeTitle:'⚠️ Erreurs courantes',mistake1:'Changer plusieurs paramètres à la fois rend impossible l\x27isolation de la cause et de l\x27effet. Changez toujours UNE seule variable à la fois.',mistake2:'Sauter la mesure de référence. Sans connaître le comportement par défaut, vous ne pouvez pas mesurer l\x27impact de vos changements.',mistake3:'Ignorer le journal d\x27activité. Il enregistre chaque événement avec des horodatages — essentiel pour comprendre les séquences.'},
 wiki_history_title: '📜 Histoire de sciences de la Terre',
 wiki_history: 'Encryption dates back to ancient Egypt (1900 BCE). The Caesar cipher, Enigma machine, and modern AES represent key milestones in cryptographic evolution. Swarm Genetic Algorithm Radio s appuie sur ces fondations pour vous permettre d explorer ces concepts historiques par simulation interactive.',
 wiki_math_title: '📐 Mathématiques de Swarm Genetic Algorithm Radio',
 wiki_math: 'Les mathématiques derrière Swarm Genetic Algorithm Radio : Modern encryption relies on computational hardness: integer factorization (RSA), discrete logarithms (Diffie-Hellman), and elliptic curves (ECC). Cipher strength is measured in bits of security. AES-256 provides 256-bit security, meaning 2^256 operations to brute-force — more than atoms in the universe. XOR has perfect balance: for any fixed key bit, the output is equally likely to be 0 or 1. This property makes it ideal for mixing key material with plaintext.',
 wiki_advanced_title: '🔬 Techniques avancées',
 wiki_advanced: 'Au-delà des bases de cette simulation, les praticiens avancés de sciences de la Terre utilisent des techniques sophistiquées. Les algorithmes adaptatifs ajustent automatiquement les paramètres. L apprentissage automatique classifie les signaux avec précision. La corrélation multi-canaux détecte des motifs invisibles à l analyse mono-canal. Les réseaux de capteurs distribués combinent les données de plusieurs emplacements.',
 wiki_compare_title: '⚖️ Comparaison des approches',
 wiki_compare: 'Il existe plusieurs approches pour sciences de la Terre. Les solutions matérielles avec browser offrent des performances en temps réel. Les simulations logicielles (comme cette app) permettent une expérimentation sûre sans coût de matériel. Les plateformes cloud offrent une évolutivité mais introduisent latence et préoccupations de confidentialité. Cette simulation vous donne les bases pour utiliser efficacement toute approche.',
 wiki_debug_title: '🔧 Guide de dépannage',
 wiki_debug: 'Problèmes courants en sciences de la Terre : (1) Les résultats inattendus proviennent souvent de paramètres incorrects — réinitialisez et modifiez une variable à la fois. (2) Si la visualisation semble gelée, vérifiez que la simulation tourne. (3) Les lectures erratiques indiquent des interférences. (4) Vérifiez vos unités (Hz vs kHz vs MHz). Le débogage systématique est une compétence essentielle.',
 wiki_ethics_title: '⚖️ Éthique et aspects légaux',
 wiki_ethics: 'Sciences de la terre implique des responsabilités éthiques et légales importantes. De nombreux pays réglementent l utilisation de browser. Obtenez toujours une autorisation avant de tester des systèmes que vous ne possédez pas. Respectez les lois sur la vie privée et la protection des données. Cette simulation est conçue à des fins éducatives — elle démontre des principes sans transmettre de signaux réels ni accéder à de vrais réseaux.',
 gloss1_term: 'Ciphertext',
 gloss1_def: 'The encrypted output of a plaintext message. Without the correct decryption key, ciphertext appears as random data.',
 gloss2_term: 'Key Space',
 gloss2_def: 'The total number of possible keys for a cipher. Larger key spaces make brute-force attacks computationally infeasible.',
 ...
 theory: 'Swarm Genetic Algorithm Radio démontre les principes clés de sciences de la Terre. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. A cipher is a specific algorithm for performing encryption and decryption. Block ciphers process fixed-size chunks; stream ciphers process one bit at a time. XOR (exclusive OR) is a bitwise operation where the output is 1 only when inputs differ. It is reversible: A⊕B⊕B = A, making it fundamental to many ciphers. La simulation modélise ces mécanismes à l aide d équations mathématiques dans votre navigateur. Chaque contrôle correspond à un paramètre réel. En expérimentant ici, vous développez l intuition que les professionnels acquièrent après des années d expérience pratique.',
 faq_q9: 'Quelles erreurs courantes dois-je éviter ?',
 faq_a9: 'L erreur la plus courante est de modifier plusieurs paramètres simultanément, ce qui rend impossible la compréhension de cause à effet. Modifiez toujours un seul élément à la fois. Une autre erreur est d ignorer le journal d activité — il enregistre chaque événement. Enfin, ne sautez pas la section défis : ces questions testent votre compréhension réelle des concepts.',
 faq_q10: 'Quel est le lien avec earth science dans le monde réel ?',
 ...
 title: 'Genetic Algorithm Radio', subtitle: '📨 تشفير · 📡 إيداع · 🔓 فك تشفير',
 ...
 msgPlaceholder: 'اكتب رسالتك السرية...',
 keyPlaceholder: 'مفتاح التشفير المشترك...',
 encryptBtn: 'تشفير', sendBtn: 'إرسال Drop', receiveBtn: 'استقبال', scanBtn: 'مسح',
 bleOff: 'BLE: خامل', bleScan: 'BLE: يبحث...', bleTx: 'BLE: يُرسل...', bleRx: 'BLE: يستقبل...',
 howStep1: 'العميل أ يكتب رسالة سرية ويدخل مفتاحًا مشتركًا.',
 howStep2: 'يتم تشفير الرسالة باستخدام شيفرة XOR مع المفتاح المشترك.',
 howStep3: 'الرسالة المشفرة تُودَع عبر بث BLE في الهواء.',
 howStep4: 'العميل ب يمسح ويستقبل الإيداع ويفك تشفيره بنفس المفتاح.',
 challenge1: 'ماذا يحدث إذا استخدمت المفتاح الخاطئ لفك التشفير؟',
 challenge2: 'هل يمكنك اعتراض إيداع بدون المفتاح؟',
 challenge3: 'صمم بروتوكول تبادل مفاتيح لعميلين.',
 challengeReveal1: 'تحصل على رموز عشوائية! فك تشفير XOR بالمفتاح الخاطئ ينتج أحرفًا عشوائية. لهذا سرية المفتاح حاسمة.',
 challengeReveal2: 'نعم، يمكنك اعتراض بث BLE لكنك ترى فقط بايتات مشفرة. بدون المفتاح تبقى الرسالة غير مقروءة.',
 ...
 howto_1:'تعرض الشاشة الرئيسية محاكاة Swarm Genetic Algorithm Radio. في الأعلى ترى التصور المباشر — الألوان والرسوم المتحركة تمثل بيانات حقيقية تتغير في الوقت الفعلي. أسفلها لوحة التحكم بها أزرار ومنزلقات تضبط كل معامل. Start by looking at how Agent A writes a secret message and enters a shared key.',
 ...
 wiki_ble_title: '📡 BLE (بلوتوث منخفض الطاقة)', wiki_ble: 'بروتوكول لاسلكي منخفض الاستهلاك. تبث الأجهزة حزمًا إعلانية يكتشفها الماسحون القريبون. المدى: 10-50 متر.',
 wiki_xor_title: '🔑 شيفرة XOR', wiki_xor: 'شيفرة متماثلة: نص XOR مفتاح = مشفّر. مشفّر XOR مفتاح = نص. بسيطة لكن فعالة للتعلم.',
 wiki_ddp_title: '📨 بروتوكول Dead Drop', wiki_ddp: 'العميل أ يشفر ويبث. العميل ب يمسح ويفك التشفير. لا حاجة لاتصال مباشر — فقط موجات مشتركة ومفتاح مشترك.',
 wiki_radio_title: '📻 راديو micro:bit', wiki_radio: 'يستخدم micro:bit v2 راديو 2.4GHz لـ BLE ومجموعات الراديو. الحزم محدودة بـ 20 بايت تقريبًا. RSSI يقيس قوة الإشارة.',
 working: 'جارٍ…',
 encrypted: 'مشفّر', decrypted: 'تم فك التشفير', dropped: 'تم الإيداع', received: 'مستلم',
 noMsg: 'أدخل رسالة أولاً', noKey: 'أدخل مفتاحًا أولاً', noDrops: 'لا توجد إيداعات',
 dropSent: 'تم إرسال الإيداع عبر BLE', dropReceived: 'تم استلام الإيداع!', scanning: 'البحث عن إيداعات...',
 scanFound: 'تم اكتشاف إيداع!', scanEmpty: 'لم يتم العثور على إيداعات قريبة',
 t_mosque: 'مسجد', t_zellige: 'زليج', t_andalus: 'أندلس',
 t_riad: 'رياض', t_medina: 'مدينة',
 t_space: 'فضاء', t_jungle: 'أدغال', t_robot: 'روبوت',
 ready: '📨 Dead Drop جاهز — أدخل سرّك!',
 ...
 themeChanged: '🎨 المظهر ←',step1Title:'تهيئة السرب',step1Desc:'العميل أ يكتب رسالة سرية ويدخل مفتاحًا مشتركًا. شاهد التصور يتحدث في الوقت الفعلي أثناء هذه المرحلة. يعرض الشاشة بالضبط ما يحدث داخلياً — كل لون وحركة يمثل تحولاً محدداً في البيانات.',step2Title:'تعيين القواعد',step2Desc:'يتم تشفير الرسالة باستخدام شيفرة XOR مع المفتاح المشترك. لاحظ كيف تتغير المؤشرات خلال هذه المرحلة. يسجل سجل النشاط كل حدث لتتبع تسلسل العمليات والتحقق من النتائج.',step3Title:'محاكاة',step3Desc:'الرسالة المشفرة تُودَع عبر بث BLE في الهواء. تحول هذه المرحلة بيانات الإدخال باستخدام الخوارزمية المعروضة. قارن القيم قبل وبعد لفهم العلاقة الرياضية.',step4Title:'تحليل الأنماط',step4Desc:'العميل ب يمسح ويستقبل الإيداع ويفك تشفيره بنفس المفتاح. ناتج هذه المرحلة يغذي المرحلة التالية. حاول التوقف هنا لفحص الحالة الوسيطة.',sectionCode:'كود الجهاز',faq_q1:'ما هو Genetic Algorithm Radio؟',faq_a1:'Swarm Genetic Algorithm Radio هي محاكاة تفاعلية توضح مفاهيم علوم الأرض. Encrypt and exchange secret messages via BLE simulation. كل شيء يعمل في متصفحك — لا حاجة لأجهزة أو تثبيت. اضبط عناصر التحكم، راقب النتائج، وابنِ فهماً حقيقياً من خلال التجربة.',faq_q2:'كيف تعمل المحاكاة؟',faq_a2:'التطبيق يحاكي سلوكاً حقيقياً في ذكاء السرب. أنت تتحكم في المدخلات وتشاهد المخرجات تتغير في الوقت الفعلي.',faq_q3:'ماذا تفعل أدوات التحكم؟',faq_a3:'كل زر ومنزلق يغيّر معاملاً محدداً. راجع تبويب "كيف تستخدم" للحصول على دليل خطوة بخطوة.',faq_q4:'ما العلم وراء هذا؟',faq_a4:'هذا التطبيق يستخدم مبادئ حقيقية من ذكاء السرب. نفس المفاهيم يستخدمها المحترفون.',faq_q5:'بماذا أجرّب؟',faq_a5:'غيّر معاملاً واحداً في كل مرة وراقب التأثير. ادفع القيم للحدود القصوى لترى حدود النظام.',faq_q6:'ما العتاد المطلوب للنسخة الحقيقية؟',faq_a6:'المحاكاة لا تحتاج عتاداً. لبناء المشروع الحقيقي تحتاج ESP32 / micro:bit. راجع قسم كود الجهاز.',faq_q7:'هل بياناتي خاصة؟',faq_a7:'نعم. كل شيء يعمل محلياً في متصفحك. لا تُرسل أي بيانات، لا حاجة لحساب، ويعمل بدون إنترنت.',faq_q8:'ماذا أستكشف بعد ذلك؟',faq_a8:'جرّب تطبيقات أخرى في هذه الفئة. كل تطبيق يعلّم جانباً مختلفاً من ذكاء السرب.',demo_s1:'مرحباً في Genetic Algorithm Radio! انظر إلى الشاشة الرئيسية — هنا تعمل محاكاة ذكاء السرب.',demo_s2:'اضغط بدء وشاهد كيف يتفاعل التصوير المرئي.',demo_s3:'جرّب تعديل أدوات التحكم. كل منزلق أو زر يغيّر معاملاً محدداً.',demo_s4:'انزل للأسفل لرؤية البيانات التفصيلية. الأرقام والرسوم البيانية تتحدث في الوقت الفعلي.',demo_s5:'أحسنت! الآن جرّب قسم التحديات لاختبار فهمك لـذكاء السرب.',sectionDemo:'شاهد العرض',demoPlay:'تشغيل',demoPause:'إيقاف',demoPrev:'السابق',demoNext:'التالي',learn1Title:'Emergent Behavior',learn1Desc:'How simple rules create complex group patterns. شاهد المحاكاة لرؤية هذا في الوقت الفعلي. التصور المرئي يجعل غير المرئي مرئياً.',learn1Tag:'AI',learn2Title:'Mesh Networks',learn2Desc:'How swarms communicate without a central leader. أدوات التحكم تتيح لك التجريب. كل تغيير يكشف كيف يستجيب هذا المبدأ.',learn2Tag:'Networking',learn3Title:'Algorithms',learn3Desc:'How nature-inspired math solves hard problems. جرّب التحديات لاختبار فهمك. المهندسون الحقيقيون يستخدمون نفس هذه المفاهيم.',learn3Tag:'Computing',learn4Title:'Optimization',learn4Desc:'How swarms find the best solution together. قارن النتائج بإعدادات مختلفة لبناء الفهم. لوحات البيانات تعرض قياسات دقيقة.',learn4Tag:'Math',sectionLearn:'ماذا ستتعلم',learnLevelVal:'متوسط 🟡',learnLevel:'المستوى:',learnTimeVal:'20 min ⏱',learnTime:'المدة:',learnAgeVal:'12+ 🧒',kidTitle:'للمستكشفين الصغار',kidIntro:'مرحباً في Swarm Genetic Algorithm Radio! هذا مثل تجربة علمية على حاسوبك. تتحكم في محاكاة حقيقية لـعلوم الأرض — اضغط الأزرار، حرك المنزلقات وشاهد ما يحدث على الشاشة. Try changing the controls and watch how Agent A writes a secret message and enters a share لا شيء يمكن أن ينكسر — كل شيء محاكاة آمنة في متصفحك!',kidSafe:'آمن تماماً! لا شيء تفعله هنا يمكن أن يكسر أي شيء. كل شيء يعمل داخل متصفحك مثل لعبة.',kidTry:'اضغط على زر البدء الكبير وشاهد الشاشة تتغير. ثم جرّب تحريك المنزلقات.',kidParent:'يعلّم هذا التطبيق مفاهيم ذكاء السرب من خلال المحاكاة التفاعلية. مناسب لتعليم STEM في الفيزياء والإلكترونيات وعلوم الحاسوب.',ch1Title:'مسح المعاملات',ch1Desc:'غيّر متغيراً واحداً بشكل منهجي مع تثبيت الباقي. هل تجد العلاقة بين المدخل والمخرج؟',ch2Title:'الحالة الحدية',ch2Desc:'ادفع معاملاً لقيمته القصوى. ماذا يحدث؟ هل يتصرف النظام بشكل مختلف عند الحدود؟',ch3Title:'توقع ثم اختبر',ch3Desc:'قبل الضغط على بدء، توقع ما سيحدث. هل كنت محقاً؟ ما الذي فاتك؟',codeTitle:'كود البداية',codeLang:'Arduino (ESP32)',codeSnippet:'#include <WiFi.h>\\n\\nconst char* ssid = "MyNetwork";\\nconst char* pass = "secret123";\\n\\nvoid setup() {\\n Serial.begin(115200);\\n WiFi.mode(WIFI_STA);\\n WiFi.begin(ssid, pass);\\n while (WiFi.status() != WL_CONNECTED) {\\n delay(500);\\n Serial.print(".");\\n }\\n Serial.println("\\nConnected!");\\n Serial.println(WiFi.localIP());\\n}\\n\\nvoid loop() {\\n // Your code here\\n}',codeExplain:'يوصل هذا الكود ESP32 بشبكة WiFi. الوضع WIFI_STA يضبطه كعميل. حلقة while تنتظر الاتصال ثم تطبع عنوان IP. بعدها يمكنك إضافة خوادم HTTP أو MQTT أو مسح BLE.',purpose:'Genetic Algorithm Radio: Encrypt and exchange secret messages via BLE simulation. هذه المحاكاة تتيح لك التجريب العملي بدلاً من قراءة النظرية فقط. كل معامل تغيّره ينتج نتائج مرئية، مما يبني فهماً حقيقياً لسلوك النظام.',guideTitle:'ماذا أرى على الشاشة؟',guideCanvas:'المنطقة الرئيسية تعرض تصويراً مباشراً للمحاكاة. الألوان والحركة تمثل البيانات المتغيرة في الوقت الفعلي.',guideControls:'الأزرار أسفل الشاشة الرئيسية تتحكم في المحاكاة. بدء يشغلها، إيقاف يوقفها مؤقتاً، إعادة تعيين تمسح كل شيء.',guideSections:'أسفل البطاقة الرئيسية، الأقسام تعرض البيانات التفصيلية والتحليل. انقر على العناوين لطيها أو فتحها.',guideStatus:'النقطة الملونة في أعلى اليمين تُظهر الحالة. أخضر يعني يعمل، أحمر يعني متوقف.',
 wiki_history_title: '📜 تاريخ علوم الأرض',
 wiki_history: 'Encryption dates back to ancient Egypt (1900 BCE). The Caesar cipher, Enigma machine, and modern AES represent key milestones in cryptographic evolution. يبني Swarm Genetic Algorithm Radio على هذه الأسس ليتيح لك استكشاف هذه المفاهيم التاريخية من خلال المحاكاة التفاعلية.',
 wiki_math_title: '📐 الرياضيات وراء Swarm Genetic Algorithm Radio',
 wiki_math: 'الرياضيات وراء Swarm Genetic Algorithm Radio: Modern encryption relies on computational hardness: integer factorization (RSA), discrete logarithms (Diffie-Hellman), and elliptic curves (ECC). Cipher strength is measured in bits of security. AES-256 provides 256-bit security, meaning 2^256 operations to brute-force — more than atoms in the universe. XOR has perfect balance: for any fixed key bit, the output is equally likely to be 0 or 1. This property makes it ideal for mixing key material with plaintext.',
 wiki_advanced_title: '🔬 تقنيات متقدمة',
 wiki_advanced: 'بما يتجاوز الأساسيات في هذه المحاكاة، يستخدم ممارسو علوم الأرض المتقدمون تقنيات متطورة. تضبط الخوارزميات التكيفية المعاملات تلقائياً. يصنف التعلم الآلي الإشارات بدقة عالية. يكتشف الارتباط متعدد القنوات أنماطاً غير مرئية للتحليل أحادي القناة. تجمع شبكات الاستشعار الموزعة البيانات من مواقع متعددة.',
 wiki_compare_title: '⚖️ مقارنة الأساليب',
 wiki_compare: 'هناك عدة أساليب في علوم الأرض. توفر الحلول المادية باستخدام browser أداءً في الوقت الفعلي. توفر المحاكاة البرمجية (مثل هذا التطبيق) تجربة آمنة بدون تكلفة المعدات. توفر المنصات السحابية قابلية التوسع لكنها تقدم تأخيراً ومخاوف تتعلق بالخصوصية. تمنحك هذه المحاكاة الأساس لاستخدام أي نهج بفعالية.',
 wiki_debug_title: '🔧 دليل استكشاف الأخطاء',
 wiki_debug: 'مشاكل شائعة في علوم الأرض: (1) النتائج غير المتوقعة غالباً ما تأتي من إعدادات معاملات غير صحيحة — أعد التعيين وغير متغيراً واحداً في كل مرة. (2) إذا بدت الرسوم المتحركة متجمدة، تأكد أن المحاكاة تعمل. (3) القراءات المتقطعة تشير عادة إلى التداخل. (4) تحقق من وحداتك (هرتز مقابل كيلوهرتز مقابل ميغاهرتز).',
 wiki_ethics_title: '⚖️ الأخلاقيات والاعتبارات القانونية',
 wiki_ethics: 'علوم الأرض يحمل مسؤوليات أخلاقية وقانونية مهمة. تنظم العديد من الدول استخدام browser والمعدات المماثلة. احصل دائماً على إذن مناسب قبل اختبار أنظمة لا تملكها. احترم قوانين الخصوصية وحماية البيانات. هذه المحاكاة مصممة لأغراض تعليمية — تعرض المبادئ دون إرسال إشارات حقيقية أو الوصول لشبكات فعلية.',
 gloss1_term: 'Ciphertext',
 gloss1_def: 'The encrypted output of a plaintext message. Without the correct decryption key, ciphertext appears as random data.',
 gloss2_term: 'Key Space',
 gloss2_def: 'The total number of possible keys for a cipher. Larger key spaces make brute-force attacks computationally infeasible.',
 ...
 theory: 'Swarm Genetic Algorithm Radio يوضح المبادئ الأساسية في علوم الأرض. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. A cipher is a specific algorithm for performing encryption and decryption. Block ciphers process fixed-size chunks; stream ciphers process one bit at a time. XOR (exclusive OR) is a bitwise operation where the output is 1 only when inputs differ. It is reversible: A⊕B⊕B = A, making it fundamental to many ciphers. تحاكي هذه المحاكاة الآليات الحقيقية باستخدام معادلات رياضية في متصفحك. كل عنصر تحكم يتوافق مع معامل حقيقي. بالتجربة هنا تبني نفس الحدس الذي يطوره المحترفون عبر سنوات من الخبرة العملية.',
 faq_q9: 'ما الأخطاء الشائعة التي يجب تجنبها؟',
 faq_a9: 'الخطأ الأكثر شيوعاً هو تغيير معاملات متعددة في وقت واحد مما يجعل فهم السبب والنتيجة مستحيلاً. غير دائماً شيئاً واحداً في كل مرة. خطأ شائع آخر هو تجاهل سجل النشاط — فهو يسجل كل حدث ويساعدك على فهم تسلسل العمليات. أخيراً لا تتخط قسم التحديات: تلك الأسئلة تختبر فهمك الحقيقي للمفاهيم.',
 faq_q10: 'كيف يرتبط هذا بـearth science في العالم الحقيقي؟',
 faq_a10: 'تحاكي هذه المحاكاة نفس الفيزياء والرياضيات المستخدمة في الأنظمة المهنية. تتوافق المعاملات التي تضبطها مع إعدادات المعدات الحقيقية. تعرض الرسوم البيانية أنماط بيانات مطابقة لما تراه على الأجهزة المهنية. الفرق الرئيسي هو أن هذا يعمل بأمان في متصفحك — تستخدم الأنظمة الحقيقية أجهزة browser وقد تتطلب تراخيص قانونية للتشغيل.',
 glossTitle: '📚 مصطلحات أساسية',learnAge:'العمر:',relatedTitle:'\ud83d\udd17 تطبيقات ذات صلة',related1_name:'السياج الصوتي',related1_desc:'إصدار نغمة فوق صوتية وكشف الحركة بتأثير دوبلر',related1_path:'../../44-acoustic-warfare/sonic-acoustic-fence/index.html',related2_name:'Dead Drop — نقل رسائل BLE',related2_desc:'شفّر وتبادل رسائل سرية عبر محاكاة BLE',related2_path:'../../45-time-manipulation/chrono-time-reversal-mirror/index.html',related3_name:'\\u0627\\u0644\\u0628\\u0635\\u0645\\u0629 \\u0627\\u0644\\u062d\\u0631\\u0627\\u0631\\u064a\\u0629 \\u2014 \\u0643\\u0634\\u0641 \\u0639\\u0628\\u0631 \\u0627\\u0644\\u062c\\u062f\\u0631\\u0627\\u0646',related3_desc:'\\u0643\\u0634\\u0641 \\u0627\\u0644\\u0628\\u0635\\u0645\\u0627\\u062a \\u0627\\u0644\\u062d\\u0631\\u0627\\u0631\\u064a\\u0629 \\u0627\\u0644\\u0628\\u0634\\u0631\\u064a\\u0629',related3_path:'../../43-bio-radio/bio-thermal-signature/index.html',pathTitle:'\ud83d\udee4\ufe0f مسار التعلم',pathPrev_name:'Dead Drop — نقل رسائل BLE',pathPrev_path:'../../46-swarm-intelligence/swarm-flock-formation/index.html',pathNext_name:'Dead Drop — نقل رسائل BLE',pathNext_path:'../../46-swarm-intelligence/swarm-hive-mind-sensor/index.html',
 printBtn: '🖨️ طباعة',quizTab:'اختبار',quizTitle:'اختبر معلوماتك',quizRetry:'إعادة',quizCorrect:'صحيح!',quizWrong:'خطأ!',quizScore:'النتيجة',quiz_q1:'ما الميزة الرئيسية لأنظمة السرب؟',quiz_q1a:'التحكم المركزي',quiz_q1b:'المتانة من خلال التكرار',quiz_q1c:'نقطة فشل واحدة',quiz_q1d:'تكلفة عالية',quiz_q1_answer:'1',quiz_q2:'في شيفرة مورس، ما يمثل الحرف E؟',quiz_q2a:'شرطة',quiz_q2b:'نقطة واحدة',quiz_q2c:'نقطتان',quiz_q2d:'نقطة-شرطة',quiz_q2_answer:'1',quiz_q3:'أي نظام طبيعي ألهم خوارزميات السرب؟',quiz_q3a:'الكواكب',quiz_q3b:'مستعمرات النمل',quiz_q3c:'الأنهار',quiz_q3d:'الجبال',quiz_q3_answer:'1',quiz_q4:'كيف تبدو SOS بشيفرة مورس؟',quiz_q4a:'---...---',quiz_q4b:'...---...',quiz_q4c:'...-...-',quiz_q4d:'-.-.-.',quiz_q4_answer:'1',quiz_q5:'ماذا تعني AM في الراديو؟',quiz_q5a:'تعديل صوتي',quiz_q5b:'تعديل السعة',quiz_q5c:'تعديل تناظري',quiz_q5d:'تعديل نشط',quiz_q5_answer:'1',realworldTitle:'🌍 قصص واقعية',realworld1:'اخترق هجوم سولار ويندز (2020) أكثر من 18000 منظمة من خلال إخفاء برامج ضارة داخل تحديثات البرمجيات الموثوقة.',realworld2:'كانت ثغرة هارتبليد (2014) تجاوزًا في المخزن المؤقت في OpenSSL سمح للمهاجمين بقراءة 64 كيلوبايت من ذاكرة الخادم لكل طلب.',realworld3:'فك فريق آلان تورينغ في بلتشلي بارك شفرة آلة إنغما خلال الحرب العالمية الثانية وقرأ 84000 رسالة ألمانية مشفرة شهريًا بحلول عام 1945.',experimentTitle:'🔬 تجارب',experiment_1_title:'القياس المرجعي',experiment_1:'اضبط جميع عناصر التحكم على القيم الافتراضية وسجّل القراءات الأولية. هذه هي قياساتك المرجعية. يقوم العالم الجيد دائمًا بتحديد خط الأساس قبل تغيير المتغيرات — فهو يمنحك نقطة مرجعية لقياس جميع التغييرات المستقبلية.',experiment_2_title:'تحليل الحساسية',experiment_2:'غيّر معلمة واحدة إلى قيمتها الدنيا وسجّل النتيجة ثم اضبطها على الحد الأقصى. يكشف الفرق عن حساسية النظام لهذا المتغير. في التشفير المتقدم معرفة المعلمات الأكثر أهمية تساعدك على تركيز جهودك بكفاءة.',experiment_3_title:'تأثيرات التفاعل',experiment_3:'بعد اختبار المعلمات بشكل فردي غيّر اثنتين في وقت واحد. هل التأثير المشترك يساوي مجموع التأثيرات الفردية؟ التفاعلات غير الخطية شائعة في التشفير المتقدم وتكشف التعقيد الخفي تحت أنظمة تبدو بسيطة.',wiki_concept_title:'💡 المفهوم الأساسي',wiki_concept:'Genetic Algorithm Radio يوضح مفهومًا أساسيًا في التشفير المتقدم. تحاكي هذه المحاكاة كيفية معالجة الأنظمة الحقيقية للإشارات والبيانات أو الظواهر الفيزيائية. الفكرة الرئيسية هي أن السلوكيات المعقدة تنشأ من قواعد بسيطة تُطبق بشكل متكرر.',wiki_realworld_title:'🌐 التطبيقات الواقعية',wiki_realworld:'المبادئ المعروضة في Genetic Algorithm Radio لها تطبيقات مباشرة في العالم الحقيقي. يستخدم المحترفون في التشفير المتقدم هذه المفاهيم نفسها يوميًا. في الصناعة يُنفذ browser وأجهزة مماثلة هذه الخوارزميات في أنظمة مدمجة.',wiki_safety_title:'⚠️ السلامة والمسؤولية',wiki_safety:'العمل في مجال التشفير المتقدم يحمل مسؤوليات مهمة. تعمل دائمًا ضمن الحدود القانونية. هذه المحاكاة مصممة للاستخدام التعليمي الآمن — لا ترسل إشارات حقيقية ولا تصل إلى شبكات حقيقية.',proTipTitle:'💡 نصائح احترافية',proTip1:'استخدم علامة تبويب الأداء في المتصفح لقياس معدل الإطارات. إذا انخفضت المحاكاة عن 30 إطارًا في الثانية قلل نقاط البيانات.',proTip2:'صدّر بيانات المحاكاة باستخدام زر النسخ والصقها في جدول بيانات وأنشئ مخططاتك الخاصة.',funFactTitle:'🎯 هل تعلم؟',funFact:'يحتوي مفتاح AES بطول 256 بت على توليفات ممكنة أكثر من عدد الذرات في الكون المرئي.',mistakeTitle:'⚠️ أخطاء شائعة',mistake1:'تغيير عدة معلمات في وقت واحد يجعل من المستحيل عزل السبب والنتيجة. غيّر دائمًا متغيرًا واحدًا فقط في كل مرة.',mistake2:'تخطي القياس المرجعي. بدون معرفة السلوك الافتراضي لا يمكنك قياس تأثير تغييراتك على النظام.',peerTitle:'👥 \u0648\u0636\u0639 \u0627\u0644\u0646\u0638\u064a\u0631',peerConnect:'\u0627\u062a\u0635\u0627\u0644',peerDisconnect:'\u0642\u0637\u0639',peerStatus:'\u062d\u0627\u0644\u0629 \u0627\u0644\u0646\u0638\u064a\u0631',peerSend:'\u0623\u0631\u0633\u0644',peerReceive:'\u0627\u0633\u062a\u0644\u0645',peerInfo:'\u0627\u0641\u062a\u062d \u0647\u0630\u0627 \u0627\u0644\u062a\u0637\u0628\u064a\u0642 \u0641\u064a \u062a\u0628\u0648\u064a\u0628\u064a\u0646 \u0644\u0644\u0645\u0632\u0627\u0645\u0646\u0629',heatmapTitle:'📅 \u062e\u0631\u064a\u0637\u0629 \u0627\u0644\u0646\u0634\u0627\u0637',heatmapToday:'\u0627\u0644\u064a\u0648\u0645',heatmapStreak:'\u0633\u0644\u0633\u0644\u0629',heatmapTotal:'\u0627\u0644\u0645\u062c\u0645\u0648\u0639',heatmapLegend:'\u0623\u0642\u0644 \u2192 \u0623\u0643\u062b\u0631',mistake3:'تجاهل سجل النشاط. يسجل كل حدث مع طوابع زمنية — ضروري لفهم التسلسلات وتصحيح النتائج غير المتوقعة.'}
...
// --- State ---
let currentCipher = '';
let droppedMessages = []; // simulated BLE drop buffer
let bleAnimId = null;
let blePackets = []; // for canvas animation
...
// --- Corrupt bytes by RSSI ---
function corruptByRSSI(text) {
 const rssi = parseInt(($('rssiSlider') || {}).value || '-40');
 const errorRate = Math.max(0, (-rssi - 30) / 100); // -30 = 0% error, -100 = 70% error
 let result = '';
 for (let i = 0; i < text.length; i++) {
 if (Math.random() < errorRate) {
 result += String.fromCharCode(text.charCodeAt(i) ^ Math.floor(Math.random() * 255));
 } else {
 result += text[i];
 }
 }
...
// --- BLE Canvas Visualization ---
function initBleCanvas() {
 const canvas = $('bleCanvas');
 if (!canvas) return;
 const ctx = canvas.getContext('2d');
 ...
 // Draw packets
 for (let i = blePackets.length - 1; i >= 0; i--) {
 const p = blePackets[i];
 p.x += p.vx;
 p.y += Math.sin(p.x / 30) * 0.5;
 p.life -= 0.008;

 if (p.life <= 0 || p.x > canvas.width + 20) {
 blePackets.splice(i, 1);
 continue;
 }
 ...
 bleAnimId = requestAnimationFrame(drawFrame);
 }
 drawFrame();
}

function spawnPackets(count, color) {
 const canvas = $('bleCanvas');
 if (!canvas) return;
...
// --- BLE Status helpers ---
function setBleStatus(key) {
 const dot = $('bleDot');
 const txt = $('bleStatusText');
 const s = LANG[currentLang];
 if (txt && s[key]) txt.textContent = s[key];
 if (dot) {
 dot.classList.toggle('active', key !== 'bleOff');
 }
}
...
function initDeadDrop() {
 ...
 const recvBtn = $('receiveBtn');
 const scanBtnEl = $('scanBtn');
 const rssiSlider = $('rssiSlider');
 ...
 currentCipher = xorCipher(msg, key);
 const hex = toHex(currentCipher);
 const display = $('cipherDisplay');
 if (display) display.textContent = hex;

 log(`🔐 ${s.encrypted}: ${hex}`, 'tx');
 showToast(s.encrypted, 1200);
 playSound('success');
 setStatus(false);
 });

 // Send Drop button
 if (sendBtnEl) sendBtnEl.addEventListener('click', () => {
 const s = LANG[currentLang];
 if (!currentCipher) { log(s.noMsg, 'error'); return; }

 setBleStatus('bleTx');
 showToast(s.dropSent, 2000);
 log(`📡 TX → ${toHex(currentCipher)}`, 'tx');
 spawnPackets(15, '#33ff33');
 playSound('success');

 // Simulate BLE broadcast delay
 droppedMessages.push({
 cipher: currentCipher,
 hex: toHex(currentCipher),
 ts: Date.now()
 });

 setTimeout(() => {
 setBleStatus('bleOff');
 log(`📨 ${s.dropSent}`, 'success');
 setStatus(true);
 }, 1500);
 });
 ...
 log(s.noDrops, 'error');
 showToast(s.noDrops, 1500);
 return;
 }

 setBleStatus('bleRx');
 spawnPackets(10, '#ff9900');

 setTimeout(() => {
 const drop = droppedMessages.pop();
 const corrupted = corruptByRSSI(drop.cipher);
 const decrypted = xorCipher(corrupted, key);
 const display = $('cipherDisplay');
 if (display) display.textContent = decrypted;

 log(`📥 RX ← ${drop.hex}`, 'rx');
 log(`🔓 ${s.decrypted}: ${decrypted}`, 'success');
 showToast(s.dropReceived, 1500);
 playSound('success');
 setBleStatus('bleOff');
 setStatus(true);
 }, 1200);
 ...
 const s = LANG[currentLang];
 setBleStatus('bleScan');
 log(s.scanning, 'info');
 showToast(s.scanning, 2000);
 spawnPackets(8, '#6699ff');

 setTimeout(() => {
 const dropLogEl = $('dropLog');
 if (droppedMessages.length > 0) {
 const found = droppedMessages[droppedMessages.length - 1];
 log(`📡 ${s.scanFound} RSSI: ${($('rssiSlider') || {}).value || -40} dBm`, 'success');
 if (dropLogEl) {
 const line = document.createElement('div');
 line.textContent = `[${new Date().toLocaleTimeString()}] ${found.hex} (RSSI: ${($('rssiSlider') || {}).value || -40} dBm)`;
 dropLogEl.appendChild(line);
 dropLogEl.scrollTop = dropLogEl.scrollHeight;
 }
 playSound('success');
 } else {
 log(s.scanEmpty, 'info');
 playSound('click');
 }
 setBleStatus('bleOff');
 ...
 // RSSI slider
 if (rssiSlider && rssiValue) {
 rssiSlider.addEventListener('input', () => {
 rssiValue.textContent = rssiSlider.value + ' dBm';
 });
 }

 // Init canvas
 // Small delay to allow layout
 setTimeout(initBleCanvas, 300);
}

// Auto-init simulation after DOM ready
