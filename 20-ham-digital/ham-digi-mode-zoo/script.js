...
const LANG = {
 ...
 missionTitle:'MISSION BRIEFING',missionClassified:'CLASSIFIED',missionObjective:'Your mission objective:',missionAgent:'AGENT-6A7C82',missionSkip:'Skip',missionGo:'ACCEPT MISSION',mission_obj:'Explore and master Ham Digi Mode Zoo \u2014 analyze, experiment, and complete all challenges.',nightVisionTitle:'Night Vision Mode',nightVisionOn:'NV ON',nightVisionOff:'NV OFF',nightVisionAuto:'Auto NV',
 ...
 title: 'Ham Digi Mode Zoo', subtitle: '🚀 explore · 🎨 create · 💡 innovate',
 disconnected: 'Disconnected', connected: 'Connected',
 mainSection: 'Digi Mode Zoo', mainDesc: 'Describe your project here',
 ...
 howto_1:'The main display shows the Digi Mode Zoo simulation. At the top you see the live visualization — colors and animations represent real data changing in real time. Below it, the control panel has buttons and sliders that each adjust a specific parameter. Start by looking at how Select the operating band and tune to the target frequency.',
 ...
 missionTitle:'BRIEFING DE MISSION',missionClassified:'CLASSIFI\xc9',missionObjective:'Objectif de mission :',missionAgent:'AGENT-6A7C82',missionSkip:'Passer',missionGo:'ACCEPTER LA MISSION',mission_obj:'Explorer et ma\xeetrisez Ham Digi Mode Zoo \u2014 analysez, exp\xe9rimentez et compl\xe9tez tous les d\xe9fis.',nightVisionTitle:'Mode Vision Nocturne',nightVisionOn:'VN ON',nightVisionOff:'VN OFF',nightVisionAuto:'VN Auto',
 ...
 howto_1:'تعرض الشاشة الرئيسية محاكاة Digi Mode Zoo. في الأعلى ترى التصور المباشر — الألوان والرسوم المتحركة تمثل بيانات حقيقية تتغير في الوقت الفعلي. أسفلها لوحة التحكم بها أزرار ومنزلقات تضبط كل معامل. Start by looking at how Select the operating band and tune to the target frequency.',
...
Object.assign(LANG.en, {"title": "Digi Mode Zoo", "subtitle": "🦁 Decode FT8 PSK31 RTTY and more", "sectionA": "Theory", "sectionB": "Controls", "sectionC": "Simulation", "mainSection": "Digi Mode Zoo", "mainDesc": "Decode FT8 PSK31 RTTY and more", "start": "Start", "stop": "Stop", "simStarted": "Simulation started", "simStopped": "Simulation stopped", "theoryTitle": "Digi Mode Zoo Theory", "theoryDesc": "Decode FT8 PSK31 RTTY and more"});
Object.assign(LANG.fr, {"title": "Zoo Modes Numériques", "subtitle": "🦁 Décodez FT8 PSK31 RTTY et plus", "sectionA": "Théorie", "sectionB": "Contrôles", "sectionC": "Simulation", "mainSection": "Zoo Modes Numériques", "mainDesc": "Décodez FT8 PSK31 RTTY et plus", "start": "Démarrer", "stop": "Arrêter", "simStarted": "Simulation démarrée", "simStopped": "Simulation arrêtée", "theoryTitle": "Théorie Zoo Modes Numériques", "theoryDesc": "Décodez FT8 PSK31 RTTY et plus"});
Object.assign(LANG.ar, {"title": "حديقة الأوضاع الرقمية", "subtitle": "🦁 فك شفرة FT8 PSK31 RTTY وغيرها", "sectionA": "النظرية", "sectionB": "أدوات التحكم", "sectionC": "المحاكاة", "mainSection": "حديقة الأوضاع الرقمية", "mainDesc": "فك شفرة FT8 PSK31 RTTY وغيرها", "start": "بدء", "stop": "إيقاف", "simStarted": "بدأت المحاكاة", "simStopped": "توقفت المحاكاة", "theoryTitle": "نظرية حديقة الأوضاع الرقمية", "theoryDesc": "فك شفرة FT8 PSK31 RTTY وغيرها"});
...
/* ═══════ DIGI MODE ZOO SIM ═══════ */
let simRunning=false,simAnim=null;
const simC=$('simCanvas'),simCtx=simC?simC.getContext('2d'):null;
const specC=$('specCanvas'),specCtx=specC?specC.getContext('2d'):null;
let decodeCount=0;
...
function simFrame(){
 ...
 const img=simCtx.getImageData(0,0,W,H-1);simCtx.putImageData(img,0,1);
 for(let x=0;x<W;x++){
 let v=Math.random()*20;
 if(Math.random()<0.01)v+=60+Math.random()*80;
 if(v<85){simCtx.fillStyle=`rgb(0,0,${v*3|0})`;}
 else if(v<170){const t=v-85;simCtx.fillStyle=`rgb(${t*3|0},${t*3|0},${255-t*3|0})`;}
 else{const t=v-170;simCtx.fillStyle=`rgb(255,${255-t*3|0},0)`;}
 simCtx.fillRect(x,0,1,1);
 }
 }
 if(specCtx){
 const W=specC.width,H=specC.height;
 specCtx.fillStyle='rgba(0,0,0,0.3)';specCtx.fillRect(0,0,W,H);
 specCtx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue('--accent').trim()||'#0f0';
 specCtx.lineWidth=1.5;specCtx.beginPath();
 for(let x=0;x<W;x++){
 let v=10+Math.random()*15;if(Math.random()<0.02)v+=30+Math.random()*30;
 const y=H-v/80*H;x===0?specCtx.moveTo(x,y):specCtx.lineTo(x,y);
 }
 specCtx.stroke();
 }
 // Random decode
 if(Math.random()<0.08){
 decodeCount++;const call=randCall();const grid=randGrid();const snr=randSNR();
 const out=$('decodeOutput');
 if(out){const d=document.createElement('div');
 d.textContent=new Date().toLocaleTimeString()+''+call+''+grid+''+snr+'dB';
 out.appendChild(d);out.scrollTop=out.scrollHeight;}
 log('RX: '+call+''+grid+''+snr+'dB','rx');
 }
 simAnim=requestAnimationFrame(simFrame);
}
function startSim(){if(simRunning)return;simRunning=true;setStatus(true);
 log(LANG[currentLang].simStarted||'Started','success');simFrame();}
function stopSim(){simRunning=false;if(simAnim)cancelAnimationFrame(simAnim);setStatus(false);
 log(LANG[currentLang].simStopped||'Stopped','info');}
function init_digi_mode_zoo(){
