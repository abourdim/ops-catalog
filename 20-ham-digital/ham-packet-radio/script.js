...
const LANG = {
 ...
 missionTitle:'MISSION BRIEFING',missionClassified:'CLASSIFIED',missionObjective:'Your mission objective:',missionAgent:'AGENT-17537B',missionSkip:'Skip',missionGo:'ACCEPT MISSION',mission_obj:'Explore and master Ham Packet Radio \u2014 analyze, experiment, and complete all challenges.',nightVisionTitle:'Night Vision Mode',nightVisionOn:'NV ON',nightVisionOff:'NV OFF',nightVisionAuto:'Auto NV',
 ...
 title: 'Ham Packet Radio', subtitle: '🚀 explore · 🎨 create · 💡 innovate',
 disconnected: 'Disconnected', connected: 'Connected',
 mainSection: 'Packet Radio', mainDesc: 'Describe your project here',
 ...
 howto_1:'The main display shows the Packet Radio simulation. At the top you see the live visualization — colors and animations represent real data changing in real time. Below it, the control panel has buttons and sliders that each adjust a specific parameter. Start by looking at how Select the operating band and tune to the target frequency.',
 ...
 gloss4_term: 'Constellation Diagram',
 gloss4_def: 'A plot showing the amplitude and phase of each symbol in a digital modulation scheme. Each point represents a unique bit pattern. Noise causes points to spread.',
 gloss5_term: 'Latency',
 ...
 missionTitle:'BRIEFING DE MISSION',missionClassified:'CLASSIFI\xc9',missionObjective:'Objectif de mission :',missionAgent:'AGENT-17537B',missionSkip:'Passer',missionGo:'ACCEPTER LA MISSION',mission_obj:'Explorer et ma\xeetrisez Ham Packet Radio \u2014 analysez, exp\xe9rimentez et compl\xe9tez tous les d\xe9fis.',nightVisionTitle:'Mode Vision Nocturne',nightVisionOn:'VN ON',nightVisionOff:'VN OFF',nightVisionAuto:'VN Auto',
 ...
 howto_1:'تعرض الشاشة الرئيسية محاكاة Packet Radio. في الأعلى ترى التصور المباشر — الألوان والرسوم المتحركة تمثل بيانات حقيقية تتغير في الوقت الفعلي. أسفلها لوحة التحكم بها أزرار ومنزلقات تضبط كل معامل. Start by looking at how Select the operating band and tune to the target frequency.',
...
Object.assign(LANG.en, {"title": "Packet Radio", "subtitle": "📟 AX.25 terminal emulator", "sectionA": "Theory", "sectionB": "Controls", "sectionC": "Simulation", "mainSection": "Packet Radio", "mainDesc": "AX.25 terminal emulator", "start": "Start", "stop": "Stop", "simStarted": "Simulation started", "simStopped": "Simulation stopped", "theoryTitle": "Packet Radio Theory", "theoryDesc": "AX.25 terminal emulator"});
Object.assign(LANG.fr, {"title": "Radio Paquet", "subtitle": "📟 Terminal AX.25 avec encodage", "sectionA": "Théorie", "sectionB": "Contrôles", "sectionC": "Simulation", "mainSection": "Radio Paquet", "mainDesc": "Terminal AX.25 avec encodage", "start": "Démarrer", "stop": "Arrêter", "simStarted": "Simulation démarrée", "simStopped": "Simulation arrêtée", "theoryTitle": "Théorie Radio Paquet", "theoryDesc": "Terminal AX.25 avec encodage"});
Object.assign(LANG.ar, {"title": "راديو الحزم", "subtitle": "📟 محاكي طرفية AX.25", "sectionA": "النظرية", "sectionB": "أدوات التحكم", "sectionC": "المحاكاة", "mainSection": "راديو الحزم", "mainDesc": "محاكي طرفية AX.25", "start": "بدء", "stop": "إيقاف", "simStarted": "بدأت المحاكاة", "simStopped": "توقفت المحاكاة", "theoryTitle": "نظرية راديو الحزم", "theoryDesc": "محاكي طرفية AX.25"});
// Re-apply current language to pick up new keys
setLanguage(currentLang);

/* ═══════ PACKET RADIO SIM ═══════ */
let simRunning=false,simTimer=null,simIdx=0,pktCount=0;
const MSGS=["*** CONNECTED to N1BKE-1", "SABM UA", "> Hello from packet radio!", "< QSL, good copy. 73!", "RR0 RR1", "> CQ CQ CQ DE W1AW", "< W1AW DE VE3ABC K", "DISC UA", "*** DISCONNECTED"];
function addLine(text,cls){
 const t=$('termOutput');if(!t)return;
 const d=document.createElement('div');d.style.color=cls||'var(--accent)';
 d.textContent=text;t.appendChild(d);t.scrollTop=t.scrollHeight;
}
function simTick(){
 if(!simRunning)return;
 const msg=MSGS[simIdx%MSGS.length];simIdx++;pktCount++;
 addLine(new Date().toLocaleTimeString()+''+msg);
 const pc=$('pktCount');if(pc)pc.textContent=pktCount+' packets';
 log(msg,'rx');
}
function startSim(){if(simRunning)return;simRunning=true;setStatus(true);
 const cs=$('connStatus');if(cs)cs.textContent='Connected';
 log(LANG[currentLang].simStarted||'Connected','success');
 simTimer=setInterval(simTick,2000);}
function stopSim(){simRunning=false;if(simTimer)clearInterval(simTimer);setStatus(false);
 const cs=$('connStatus');if(cs)cs.textContent='Disconnected';
 log(LANG[currentLang].simStopped||'Disconnected','info');}
function sendMsg(){
 const inp=$('msgInput');if(!inp||!inp.value.trim())return;
 addLine('> '+inp.value,'var(--accent2)');log('TX: '+inp.value,'tx');inp.value='';
}
function init_packet_radio(){
