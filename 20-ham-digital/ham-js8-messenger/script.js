...
const LANG = {
 ...
 missionTitle:'MISSION BRIEFING',missionClassified:'CLASSIFIED',missionObjective:'Your mission objective:',missionAgent:'AGENT-2338A6',missionSkip:'Skip',missionGo:'ACCEPT MISSION',mission_obj:'Explore and master Ham Js8 Messenger \u2014 analyze, experiment, and complete all challenges.',nightVisionTitle:'Night Vision Mode',nightVisionOn:'NV ON',nightVisionOff:'NV OFF',nightVisionAuto:'Auto NV',
 ...
 title: 'Ham Js8 Messenger', subtitle: '🚀 explore · 🎨 create · 💡 innovate',
 disconnected: 'Disconnected', connected: 'Connected',
 mainSection: 'Js8 Messenger', mainDesc: 'Describe your project here',
 ...
 howto_1:'The main display shows the Js8 Messenger simulation. At the top you see the live visualization — colors and animations represent real data changing in real time. Below it, the control panel has buttons and sliders that each adjust a specific parameter. Start by looking at how Select the operating band and tune to the target frequency.',
 ...
 missionTitle:'BRIEFING DE MISSION',missionClassified:'CLASSIFI\xc9',missionObjective:'Objectif de mission :',missionAgent:'AGENT-2338A6',missionSkip:'Passer',missionGo:'ACCEPTER LA MISSION',mission_obj:'Explorer et ma\xeetrisez Ham Js8 Messenger \u2014 analysez, exp\xe9rimentez et compl\xe9tez tous les d\xe9fis.',nightVisionTitle:'Mode Vision Nocturne',nightVisionOn:'VN ON',nightVisionOff:'VN OFF',nightVisionAuto:'VN Auto',
 ...
 howto_1:'تعرض الشاشة الرئيسية محاكاة Js8 Messenger. في الأعلى ترى التصور المباشر — الألوان والرسوم المتحركة تمثل بيانات حقيقية تتغير في الوقت الفعلي. أسفلها لوحة التحكم بها أزرار ومنزلقات تضبط كل معامل. Start by looking at how Select the operating band and tune to the target frequency.',
...
Object.assign(LANG.en, {"title": "JS8 Messenger", "subtitle": "💬 Weak-signal text chat over HF", "sectionA": "Theory", "sectionB": "Controls", "sectionC": "Simulation", "mainSection": "JS8 Messenger", "mainDesc": "Weak-signal text chat over HF", "start": "Start", "stop": "Stop", "simStarted": "Simulation started", "simStopped": "Simulation stopped", "theoryTitle": "JS8 Messenger Theory", "theoryDesc": "Weak-signal text chat over HF"});
Object.assign(LANG.fr, {"title": "Messagerie JS8", "subtitle": "💬 Chat texte signal faible sur HF", "sectionA": "Théorie", "sectionB": "Contrôles", "sectionC": "Simulation", "mainSection": "Messagerie JS8", "mainDesc": "Chat texte signal faible sur HF", "start": "Démarrer", "stop": "Arrêter", "simStarted": "Simulation démarrée", "simStopped": "Simulation arrêtée", "theoryTitle": "Théorie Messagerie JS8", "theoryDesc": "Chat texte signal faible sur HF"});
Object.assign(LANG.ar, {"title": "رسائل JS8", "subtitle": "💬 دردشة نصية بإشارة ضعيفة", "sectionA": "النظرية", "sectionB": "أدوات التحكم", "sectionC": "المحاكاة", "mainSection": "رسائل JS8", "mainDesc": "دردشة نصية بإشارة ضعيفة", "start": "بدء", "stop": "إيقاف", "simStarted": "بدأت المحاكاة", "simStopped": "توقفت المحاكاة", "theoryTitle": "نظرية رسائل JS8", "theoryDesc": "دردشة نصية بإشارة ضعيفة"});
...
const JS8_MSGS=['@ALLCALL HELLO','SNR?','QTH?','GRID EM48','73 GL','INFO?','HEARTBEAT','MSG W1AW HELLO','ACK'];
function addChat(from,msg,snr){
 const cb=$('chatBox');if(!cb)return;
 const d=document.createElement('div');d.style.marginBottom='4px';
 d.innerHTML='<span style="color:var(--accent)">'+from+'</span> <span style="color:var(--text-muted);font-size:.75rem">('+snr+'dB)</span> <span style="color:var(--text)">'+msg+'</span>';
 cb.appendChild(d);cb.scrollTop=cb.scrollHeight;
}
function simTick(){
 if(!simRunning)return;
 const call=randCall();const msg=JS8_MSGS[Math.floor(Math.random()*JS8_MSGS.length)];
 const snr=randSNR();
 addChat(call,msg,snr);log('RX: '+call+''+msg+''+snr+'dB','rx');
 const sd=$('snrDisplay');if(sd)sd.textContent='SNR: '+snr+' dB';
}
function startSim(){if(simRunning)return;simRunning=true;setStatus(true);
 log(LANG[currentLang].simStarted||'Started','success');simTimer=setInterval(simTick,3000);}
...
function sendMsg(){
 ...
 addChat('ME',inp.value,0);log('TX: '+inp.value,'tx');inp.value='';
}
function init_js8_messenger(){
 if($('startBtn'))$('startBtn').onclick=startSim;
 if($('stopBtn'))$('stopBtn').onclick=stopSim;
 if($('sendBtn'))$('sendBtn').onclick=sendMsg;
 const inp=$('msgInput');if(inp)inp.addEventListener('keydown',e=>{if(e.key==='Enter')sendMsg();});
}
init_js8_messenger();
