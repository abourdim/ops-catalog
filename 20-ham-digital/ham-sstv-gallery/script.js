...
const LANG = {
 ...
 missionTitle:'MISSION BRIEFING',missionClassified:'CLASSIFIED',missionObjective:'Your mission objective:',missionAgent:'AGENT-ACD557',missionSkip:'Skip',missionGo:'ACCEPT MISSION',mission_obj:'Explore and master Ham Sstv Gallery \u2014 analyze, experiment, and complete all challenges.',nightVisionTitle:'Night Vision Mode',nightVisionOn:'NV ON',nightVisionOff:'NV OFF',nightVisionAuto:'Auto NV',
 ...
 title: 'Ham Sstv Gallery', subtitle: '🚀 explore · 🎨 create · 💡 innovate',
 disconnected: 'Disconnected', connected: 'Connected',
 mainSection: 'Sstv Gallery', mainDesc: 'Describe your project here',
 ...
 howto_1:'The main display shows the Sstv Gallery simulation. At the top you see the live visualization — colors and animations represent real data changing in real time. Below it, the control panel has buttons and sliders that each adjust a specific parameter. Start by looking at how Select the operating band and tune to the target frequency.',
 ...
 missionTitle:'BRIEFING DE MISSION',missionClassified:'CLASSIFI\xc9',missionObjective:'Objectif de mission :',missionAgent:'AGENT-ACD557',missionSkip:'Passer',missionGo:'ACCEPTER LA MISSION',mission_obj:'Explorer et ma\xeetrisez Ham Sstv Gallery \u2014 analysez, exp\xe9rimentez et compl\xe9tez tous les d\xe9fis.',nightVisionTitle:'Mode Vision Nocturne',nightVisionOn:'VN ON',nightVisionOff:'VN OFF',nightVisionAuto:'VN Auto',
 ...
 howto_1:'تعرض الشاشة الرئيسية محاكاة Sstv Gallery. في الأعلى ترى التصور المباشر — الألوان والرسوم المتحركة تمثل بيانات حقيقية تتغير في الوقت الفعلي. أسفلها لوحة التحكم بها أزرار ومنزلقات تضبط كل معامل. Start by looking at how Select the operating band and tune to the target frequency.',
...
Object.assign(LANG.en, {"title": "SSTV Gallery", "subtitle": "🖼️ Receive SSTV images line by line", "sectionA": "Theory", "sectionB": "Controls", "sectionC": "Simulation", "mainSection": "SSTV Gallery", "mainDesc": "Receive SSTV images line by line", "start": "Start", "stop": "Stop", "simStarted": "Simulation started", "simStopped": "Simulation stopped", "theoryTitle": "SSTV Gallery Theory", "theoryDesc": "Receive SSTV images line by line"});
Object.assign(LANG.fr, {"title": "Galerie SSTV", "subtitle": "🖼️ Recevez images SSTV ligne par ligne", "sectionA": "Théorie", "sectionB": "Contrôles", "sectionC": "Simulation", "mainSection": "Galerie SSTV", "mainDesc": "Recevez images SSTV ligne par ligne", "start": "Démarrer", "stop": "Arrêter", "simStarted": "Simulation démarrée", "simStopped": "Simulation arrêtée", "theoryTitle": "Théorie Galerie SSTV", "theoryDesc": "Recevez images SSTV ligne par ligne"});
Object.assign(LANG.ar, {"title": "معرض SSTV", "subtitle": "🖼️ استقبل صور SSTV سطراً بسطر", "sectionA": "النظرية", "sectionB": "أدوات التحكم", "sectionC": "المحاكاة", "mainSection": "معرض SSTV", "mainDesc": "استقبل صور SSTV سطراً بسطر", "start": "بدء", "stop": "إيقاف", "simStarted": "بدأت المحاكاة", "simStopped": "توقفت المحاكاة", "theoryTitle": "نظرية معرض SSTV", "theoryDesc": "استقبل صور SSTV سطراً بسطر"});
// Re-apply current language to pick up new keys
setLanguage(currentLang);

/* ═══════ SSTV GALLERY SIM ═══════ */
let simRunning=false,simTimer=null,sstvLine=0;
const sstvC=$('sstvCanvas'),sstvCtx=sstvC?sstvC.getContext('2d'):null;
const SSTV_MODES=['Scottie S1','Scottie S2','Martin M1','Martin M2','Robot 36','PD 120'];
let currentSstvMode=0;

function drawSstvLine(){
 if(!sstvCtx||!simRunning)return;
 const W=sstvC.width;
 for(let x=0;x<W;x++){
 // Generate colorful pattern simulating an image
 const phase=sstvLine*0.05+x*0.02;
 const r=Math.floor(128+127*Math.sin(phase));
 const g=Math.floor(128+127*Math.sin(phase+2.1));
 const b=Math.floor(128+127*Math.sin(phase+4.2));
 sstvCtx.fillStyle=`rgb(${r},${g},${b})`;
 sstvCtx.fillRect(x,sstvLine,1,1);
 }
 sstvLine++;
 const sl=$('sstvLine');if(sl)sl.textContent='Line: '+sstvLine+'/'+sstvC.height;
 if(sstvLine>=sstvC.height){
 // Image complete - save to gallery
 const gallery=$('gallery');
 if(gallery){const img=document.createElement('canvas');img.width=80;img.height=64;
 const ctx=img.getContext('2d');ctx.drawImage(sstvC,0,0,80,64);
 img.style.cssText='border-radius:4px;border:1px solid var(--border)';
 gallery.appendChild(img);}
 log('SSTV image received ('+SSTV_MODES[currentSstvMode]+')','success');
 currentSstvMode=(currentSstvMode+1)%SSTV_MODES.length;
 const sm=$('sstvMode');if(sm)sm.textContent='Mode: '+SSTV_MODES[currentSstvMode];
 sstvLine=0;sstvCtx.fillStyle='#000';sstvCtx.fillRect(0,0,sstvC.width,sstvC.height);
 }
}
function startSim(){if(simRunning)return;simRunning=true;sstvLine=0;setStatus(true);
 if(sstvCtx){sstvCtx.fillStyle='#000';sstvCtx.fillRect(0,0,sstvC.width,sstvC.height);}
 log(LANG[currentLang].simStarted||'Started','success');
 simTimer=setInterval(drawSstvLine,50);}
