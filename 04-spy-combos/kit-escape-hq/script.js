/**
 * Workshop DIY — Escape HQ v1.0
 * Solve puzzles with ESP32 locks and micro:bit gadgets — spy escape room
...
const LANG = {
 ...
 missionTitle:'MISSION BRIEFING',missionClassified:'CLASSIFIED',missionObjective:'Your mission objective:',missionAgent:'AGENT-222FBD',missionSkip:'Skip',missionGo:'ACCEPT MISSION',mission_obj:'Explore and master Escape HQ \u2014 analyze, experiment, and complete all challenges.',nightVisionTitle:'Night Vision Mode',nightVisionOn:'NV ON',nightVisionOff:'NV OFF',nightVisionAuto:'Auto NV',
 ...
 howto_3:'Scroll down to the expandable sections. "How To Escape" shows detailed measurements and charts that update in real time. Click section headers to expand or collapse them. The data here helps you understand what the visualization is showing.',
 ...
 wiki_cipher_title: '🔤 Caesar Cipher', wiki_cipher:'A substitution cipher where each letter is shifted by a fixed amount. Encryption transforms readable data into scrambled ciphertext using a mathematical algorithm and a secret key. Without the correct key, the data appears random. Strong encryption uses keys so large that guessing them would take billions of years.',
 wiki_logic_title: '🧠 Logic Gates', wiki_logic:'AND, OR, NOT, XOR — fundamental building blocks of digital circuits. This concept is fundamental to understanding how the simulation works. Experiment with the controls to see it in action — changing related parameters will show you how different factors interact in the real system.',
 wiki_esp_title: '📡 ESP32 Locks', wiki_esp: 'In real escape rooms, ESP32s control electromagnetic locks triggered by puzzle solutions.',
 ...
 timeRemaining: 'Time Remaining', puzzle1Name: 'Cipher Lock', puzzle2Name: 'Pattern Grid',
 puzzle3Name: 'Sequence Key', puzzle4Name: 'Logic Gate',
 puzzleLocked: '🔒 Locked', puzzleSolved: '🔓 Solved',
 ...
 theory: 'Kit Escape Hq demonstrates key principles from RF intelligence. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. A radio channel is a defined frequency range allocated for communication. Adjacent channels can interfere, so proper channel planning is essential. Signals carry information through variations in amplitude, frequency, or phase. Noise and interference degrade signals during transmission. The simulation models these real-world mechanisms using mathematical equations running in your browser. Every control maps to a real parameter that engineers tune in professional settings. By experimenting here, you build the same intuition that professionals develop through years of hands-on experience.',
 ...
 missionTitle:'BRIEFING DE MISSION',missionClassified:'CLASSIFI\xc9',missionObjective:'Objectif de mission :',missionAgent:'AGENT-222FBD',missionSkip:'Passer',missionGo:'ACCEPTER LA MISSION',mission_obj:'Explorer et ma\xeetrisez Escape HQ \u2014 analysez, exp\xe9rimentez et compl\xe9tez tous les d\xe9fis.',nightVisionTitle:'Mode Vision Nocturne',nightVisionOn:'VN ON',nightVisionOff:'VN OFF',nightVisionAuto:'VN Auto',
 ...
 wiki_logic_title: '🧠 Portes logiques', wiki_logic: 'AND, OR, NOT, XOR — blocs fondamentaux des circuits numériques.',
 wiki_esp_title: '📡 Verrous ESP32', wiki_esp: 'Dans les vraies salles d\'évasion, les ESP32 contrôlent des verrous électromagnétiques.',
 ...
 timeRemaining: 'Temps restant', startEscape: 'Commencer l\'évasion', resetEscape: 'Réinitialiser',
 ...
 theory: 'Kit Escape Hq démontre les principes clés de renseignement RF. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. A radio channel is a defined frequency range allocated for communication. Adjacent channels can interfere, so proper channel planning is essential. Signals carry information through variations in amplitude, frequency, or phase. Noise and interference degrade signals during transmission. La simulation modélise ces mécanismes à l aide d équations mathématiques dans votre navigateur. Chaque contrôle correspond à un paramètre réel. En expérimentant ici, vous développez l intuition que les professionnels acquièrent après des années d expérience pratique.',
 ...
 wiki_logic_title: '🧠 البوابات المنطقية', wiki_logic: 'AND, OR, NOT, XOR — اللبنات الأساسية للدوائر الرقمية.',
 wiki_esp_title: '📡 أقفال ESP32', wiki_esp: 'في غرف الهروب الحقيقية، تتحكم ESP32 بأقفال كهرومغناطيسية.',
 wiki_privacy_title: '🔒 الخصوصية', wiki_privacy: 'محلي أولًا، خصوصية أولًا. كل البيانات تبقى في متصفحك.',
 working: 'جارٍ…',
 t_mosque: 'مسجد', t_zellige: 'زليج', t_andalus: 'أندلس', t_riad: 'رياض', t_medina: 'مدينة', t_space: 'فضاء', t_jungle: 'أدغال', t_robot: 'روبوت',
 ready: '🏰 مقر الهروب جاهز!',
 ...
 timeRemaining: 'الوقت المتبقي', startEscape: 'ابدأ الهروب', resetEscape: 'إعادة ضبط',
 ...
 theory: 'Kit Escape Hq يوضح المبادئ الأساسية في الاستخبارات اللاسلكية. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. A radio channel is a defined frequency range allocated for communication. Adjacent channels can interfere, so proper channel planning is essential. Signals carry information through variations in amplitude, frequency, or phase. Noise and interference degrade signals during transmission. تحاكي هذه المحاكاة الآليات الحقيقية باستخدام معادلات رياضية في متصفحك. كل عنصر تحكم يتوافق مع معامل حقيقي. بالتجربة هنا تبني نفس الحدس الذي يطوره المحترفون عبر سنوات من الخبرة العملية.',
...
/* ═══════ ESCAPE HQ SIMULATION ═══════ */

let escapeRunning = false;
let escapeTimerInterval = null;
let escapeTimeLeft = 600; // 10 min
let puzzlesSolved = [false, false, false, false];
let inventoryItems = [];
...
let cipherShift = 3;
let patternSolution = [];
let patternUserInput = [];
let sequenceAnswer = 0;
let logicAnswer = true;

function randomInt(min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; }

function updateEscapeTimer() {
 const display = $('escapeTimer');
 if (!display) return;
 const m = Math.floor(escapeTimeLeft / 60);
 const sec = escapeTimeLeft % 60;
 display.textContent = `${String(m).padStart(2, '0')}:${String(sec).padStart(2, '0')}`;
 if (escapeTimeLeft <= 60) display.classList.add('critical');
...
function drawRoomMap() {
 ...
 // Door (top center)
 const doorW = 40;
 ctx.fillStyle = puzzlesSolved.every(Boolean) ? '#22c55e' : '#ef4444';
 ctx.fillRect(w / 2 - doorW / 2, 10, doorW, 8);
 ctx.fillStyle = '#fff';
 ctx.font = '9px Courier New';
 ctx.textAlign = 'center';
 ctx.fillText(puzzlesSolved.every(Boolean) ? 'EXIT OPEN' : 'EXIT LOCKED', w / 2, 28);
 // Puzzle locations
 const puzzlePos = [
 { x: 60, y: 60, emoji: '🔤', name: 'Cipher' },
 { x: w - 60, y: 60, emoji: '🔲', name: 'Pattern' },
 { x: 60, y: h - 50, emoji: '🔢', name: 'Sequence' },
 { x: w - 60, y: h - 50, emoji: '🧠', name: 'Logic' },
 ];
 puzzlePos.forEach((p, i) => {
 ctx.fillStyle = puzzlesSolved[i] ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.15)';
 ctx.beginPath();
 ctx.arc(p.x, p.y, 22, 0, Math.PI * 2);
 ctx.fill();
 ctx.strokeStyle = puzzlesSolved[i] ? '#22c55e' : 'rgba(212,160,60,0.4)';
 ctx.lineWidth = 1;
 ctx.stroke();
 ctx.font = '16px serif';
 ctx.textAlign = 'center';
 ...
 ctx.fillStyle = puzzlesSolved[i] ? '#22c55e' : '#888';
...
function updateLocks() {
 ...
 const lock = $('lock' + (i + 1));
 if (lock) {
 lock.textContent = puzzlesSolved[i] ? '🔓' : '🔒';
 lock.className = 'lock-indicator ' + (puzzlesSolved[i] ? 'unlocked' : 'locked');
 const label = document.createElement('div');
 label.className = 'lock-label';
 label.textContent = 'Lock ' + (i + 1);
 lock.appendChild(label);
 }
 const status = $('puzzle' + (i + 1) + 'Status');
 if (status) {
 const s = LANG[currentLang];
 status.textContent = puzzlesSolved[i] ? (s.puzzleSolved || '🔓 Solved') : (s.puzzleLocked || '🔒 Locked');
 status.className = 'puzzle-status' + (puzzlesSolved[i] ? ' solved-text' : '');
 }
 const card = $('puzzle' + (i + 1) + 'Card');
 if (card) card.classList.toggle('solved', puzzlesSolved[i]);
 }
}

function addInventoryItem(emoji) {
 inventoryItems.push(emoji);
 for (let i = 0; i < 6; i++) {
 const slot = $('inv' + i);
 if (slot) {
 if (i < inventoryItems.length) {
 slot.textContent = inventoryItems[i];
...
function openPuzzle(puzzleId) {
 ...
 const idx = ['cipher', 'pattern', 'sequence', 'logic'].indexOf(puzzleId);
 if (idx === -1 || puzzlesSolved[idx]) return;
 activePuzzle = puzzleId;
 // Hide all workspaces
 ['cipherWorkspace', 'patternWorkspace', 'sequenceWorkspace', 'logicWorkspace'].forEach(id => {
 const ws = $(id);
 if (ws) ws.classList.remove('active');
 });
 // Highlight card
 for (let i = 1; i <= 4; i++) {
 const c = $('puzzle' + i + 'Card');
 if (c) c.classList.remove('active');
 }
 const card = $('puzzle' + (idx + 1) + 'Card');
 if (card) card.classList.add('active');
 // Show workspace
 const ws = $(puzzleId + 'Workspace');
 if (ws) ws.classList.add('active');
 // Generate puzzle
 if (puzzleId === 'cipher') generateCipher();
 else if (puzzleId === 'pattern') generatePattern();
 else if (puzzleId === 'sequence') generateSequence();
 else if (puzzleId === 'logic') generateLogic();
 log(`🔍 Opened puzzle: ${puzzleId}`, 'info');
}

function generateCipher() {
 const words = ['ESCAPE', 'UNLOCK', 'SECRET', 'CIPHER', 'DECODE', 'PUZZLE'];
 cipherAnswer = words[randomInt(0, words.length - 1)];
 cipherShift = randomInt(2, 7);
 const encrypted = cipherAnswer.split('').map(c => String.fromCharCode(((c.charCodeAt(0) - 65 + cipherShift) % 26) + 65)).join('');
 const grid = $('cipherGrid');
 if (grid) {
 grid.innerHTML = '';
 encrypted.split('').forEach(c => {
 const div = document.createElement('div');
 div.className = 'cipher-letter';
 div.textContent = c;
 grid.appendChild(div);
 });
 const hint = document.createElement('div');
 hint.style.cssText = 'font-size:0.7rem;color:var(--text-muted);margin-top:4px;';
 hint.textContent = `Shift: ${cipherShift} (shift each letter back by ${cipherShift})`;
...
function generatePattern() {
 ...
 cell.dataset.idx = i;
 cell.onclick = () => togglePatternCell(i);
 grid.appendChild(cell);
 }
 // Generate random pattern (4-6 lit cells)
 ...
 setTimeout(() => {
 for (let i = 0; i < 16; i++) {
 grid.children[i].classList.remove('lit');
 }
 log('🔲 Pattern hidden — recreate it from memory!', 'info');
 }, 2000);
 }, 500);
}

function togglePatternCell(idx) {
 const grid = $('patternGrid');
 if (!grid) return;
 const cell = grid.children[idx];
 cell.classList.toggle('lit');
 if (cell.classList.contains('lit')) {
 if (!patternUserInput.includes(idx)) patternUserInput.push(idx);
 } else {
 patternUserInput = patternUserInput.filter(i => i !== idx);
 }
}

function checkPattern() {
 const s = LANG[currentLang];
 const correct = patternSolution.length === patternUserInput.length &&
 patternSolution.every(i => patternUserInput.includes(i));
 if (correct) solvePuzzle(1);
 else { log(s.puzzleWrong, 'error'); playSound('error'); }
}
...
function generateSequence() {
 ...
 q.className = 'seq-num highlight';
 q.textContent = '?';
 display.appendChild(q);
}

function generateLogic() {
 const q = $('logicQuestion');
 if (!q) return;
 const questions = [
 { text: 'True AND True = ?', answer: true },
 { text: 'True AND False = ?', answer: false },
 { text: 'True OR False = ?', answer: true },
 { text: 'NOT True = ?', answer: false },
 { text: 'False OR False = ?', answer: false },
 { text: 'NOT False = ?', answer: true },
 { text: 'True XOR True = ?', answer: false },
 { text: 'True XOR False = ?', answer: true },
 ];
 const question = questions[randomInt(0, questions.length - 1)];
 q.textContent = question.text;
 logicAnswer = question.answer;
}

function solvePuzzle(idx) {
 const s = LANG[currentLang];
 if (puzzlesSolved[idx]) return;
 puzzlesSolved[idx] = true;
 updateLocks();
 drawRoomMap();
 const items = ['🔑', '📜', '🔦', '💎'];
 addInventoryItem(items[idx]);
 log(`${s.puzzleCorrect} Lock ${idx + 1} — ${s.lockUnlocked}`, 'success');
 playSound('success');
 // Hide workspace
 const names = ['cipher', 'pattern', 'sequence', 'logic'];
 const ws = $(names[idx] + 'Workspace');
 if (ws) ws.classList.remove('active');
 const card = $('puzzle' + (idx + 1) + 'Card');
 if (card) card.classList.remove('active');
 // Check if all solved
 if (puzzlesSolved.every(Boolean)) {
 clearInterval(escapeTimerInterval);
 escapeRunning = false;
 const elapsed = 600 - escapeTimeLeft;
 const m = Math.floor(elapsed / 60);
 const sec = elapsed % 60;
 log(`${s.escaped} Time: ${m}:${String(sec).padStart(2, '0')}`, 'success');
 setStatus(true);
 }
}
...
function startEscape() {
 escapeRunning = true;
 escapeTimeLeft = 600;
 puzzlesSolved = [false, false, false, false];
 inventoryItems = [];
 activePuzzle = null;
 updateLocks();
 updateEscapeTimer();
 ...
 for (let i = 0; i < 6; i++) {
 const slot = $('inv' + i);
 if (slot) { slot.textContent = '-'; slot.classList.add('empty'); }
 }
 setStatus(true);
 log('🏰 Escape started — 10:00 on the clock!', 'success');
...
function resetEscape() {
 ...
 drawRoomMap();
 ['cipherWorkspace', 'patternWorkspace', 'sequenceWorkspace', 'logicWorkspace'].forEach(id => {
 const ws = $(id); if (ws) ws.classList.remove('active');
 });
 for (let i = 0; i < 6; i++) {
 const slot = $('inv' + i);
 if (slot) { slot.textContent = '-'; slot.classList.add('empty'); }
 ...
 setStatus(false);
 log('🔄 Escape room reset', 'info');
}

function initEscapeApp() {
 // Puzzle card clicks
 for (let i = 1; i <= 4; i++) {
 const card = $('puzzle' + i + 'Card');
 if (card) card.onclick = () => openPuzzle(card.dataset.puzzle);
 }
 // Cipher submit
 const cipherSubmit = $('cipherSubmit');
 const cipherInput = $('cipherInput');
 if (cipherSubmit) cipherSubmit.onclick = () => {
 if (cipherInput && cipherInput.value.trim().toUpperCase() === cipherAnswer) solvePuzzle(0);
 else { log(LANG[currentLang].puzzleWrong, 'error'); playSound('error'); }
 if (cipherInput) cipherInput.value = '';
 };
 if (cipherInput) cipherInput.addEventListener('keydown', e => { if (e.key === 'Enter') cipherSubmit.click(); });
 // Pattern check
 const patternCheck = $('patternCheck');
 if (patternCheck) patternCheck.onclick = checkPattern;
 // Sequence submit
 const seqSubmit = $('sequenceSubmit');
 const seqInput = $('sequenceInput');
 if (seqSubmit) seqSubmit.onclick = () => {
 if (seqInput && parseInt(seqInput.value) === sequenceAnswer) solvePuzzle(2);
 else { log(LANG[currentLang].puzzleWrong, 'error'); playSound('error'); }
 if (seqInput) seqInput.value = '';
 };
 if (seqInput) seqInput.addEventListener('keydown', e => { if (e.key === 'Enter') seqSubmit.click(); });
 // Logic buttons
 const logicTrue = $('logicTrue'), logicFalse = $('logicFalse');
 if (logicTrue) logicTrue.onclick = () => { if (logicAnswer === true) solvePuzzle(3); else { log(LANG[currentLang].puzzleWrong, 'error'); playSound('error'); generateLogic(); } };
 if (logicFalse) logicFalse.onclick = () => { if (logicAnswer === false) solvePuzzle(3); else { log(LANG[currentLang].puzzleWrong, 'error'); playSound('error'); generateLogic(); } };
 // Start/Reset
 const startBtn = $('startEscapeBtn'), resetBtn = $('resetEscapeBtn');
 if (startBtn) startBtn.onclick = startEscape;
 if (resetBtn) resetBtn.onclick = resetEscape;
 // Speed challenges
 const speedBtn = $('speedEscapeBtn');
 if (speedBtn) speedBtn.onclick = () => {
 log('⚡ Speed Escape — solving all puzzles automatically...', 'tx');
 const result = $('speedEscapeResult');
 let idx = 0;
 const interval = setInterval(() => {
 puzzlesSolved[idx] = true;
 updateLocks();
 drawRoomMap();
 log(`✅ Puzzle ${idx + 1} auto-solved`, 'success');
 idx++;
 if (idx >= 4) {
 clearInterval(interval);
 const time = randomInt(90, 240);
 const m = Math.floor(time / 60);
 const sec = time % 60;
 if (result) result.innerHTML = `<div style="color:var(--accent);font-size:0.75rem;">🏆 Escaped in ${m}:${String(sec).padStart(2, '0')} — ${time < 180 ? 'AMAZING!' : 'Good job!'}</div>`;
 playSound('success');
 }
 }, 800);
 };
 const hardcoreBtn = $('hardcoreBtn');
 if (hardcoreBtn) hardcoreBtn.onclick = () => {
 const result = $('hardcoreResult');
 log('💀 Hardcore mode — extra difficulty...', 'tx');
 setTimeout(() => {
 if (result) result.innerHTML = '<div style="color:#ef4444;font-size:0.75rem;">💀 Hardcore: Cipher shift 13, 8x8 pattern grid, Fibonacci sequences, nested logic gates. Good luck!</div>';
 log('💀 Hardcore mode configured', 'info');
 }, 500);
 };
