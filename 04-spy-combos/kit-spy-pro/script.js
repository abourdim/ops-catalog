/**
 * Workshop DIY — Spy Kit Pro v1.0
 * micro:bit wearable + ESP32 field device + browser HQ — full spy loadout
...
const LANG = {
 ...
 howto_3:'Scroll down to the expandable sections. "Device Setup Guide" shows detailed measurements and charts that update in real time. Click section headers to expand or collapse them. The data here helps you understand what the visualization is showing.',
 ...
 wiki_ble_title: '📶 BLE', wiki_ble: 'Bluetooth Low Energy — short-range wireless used by the micro:bit wearable to transmit biometric data.',
 wiki_mqtt_title: '📡 MQTT', wiki_mqtt: 'Message Queuing Telemetry Transport — lightweight IoT protocol used by the ESP32 to relay data to HQ.',
 wiki_aes_title: '🔐 AES-256', wiki_aes: 'Advanced Encryption Standard with 256-bit keys — the gold standard for secure communications.',
 ...
 wearableStatus: 'Wearable', fieldStatus: 'Field Device', hqMap: 'HQ Map',
 deviceLabel: 'Device', heartRate: 'Heart Rate', steps: 'Steps', battery: 'Battery', statusLabel: 'Status',
 signal: 'Signal', temperature: 'Temp',
 missionSelect: 'Mission Selector', missionRecon: '🔍 Recon — Observe & Report',
 missionIntercept: '📡 Intercept — Capture Signal', missionExtract: '🚁 Extract — Retrieve Agent',
 startMission: 'Start Mission', connectDevices: 'Connect Devices',
 encryptedComms: 'Encrypted Comms', send: 'Send',
 agentTracker: 'Live Agent Tracker', addAgent: 'Add Agent', trackAll: 'Track All',
 challenge1Title: 'Challenge: Speed Recon', challenge1Desc: 'Complete a recon mission in under 60 seconds.',
 challenge2Title: 'Challenge: Decode Intercept', challenge2Desc: 'An encrypted message has been intercepted. Decode it.',
 ...
 theory: 'Kit Spy Pro demonstrates key principles from RF intelligence. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. Bluetooth Low Energy uses 40 channels in the 2.4 GHz ISM band. It supports advertising (broadcast) and connection (point-to-point) modes. A radio channel is a defined frequency range allocated for communication. Adjacent channels can interfere, so proper channel planning is essential. The simulation models these real-world mechanisms using mathematical equations running in your browser. Every control maps to a real parameter that engineers tune in professional settings. By experimenting here, you build the same intuition that professionals develop through years of hands-on experience.',
 ...
 wiki_mqtt_title: '📡 MQTT', wiki_mqtt: 'Protocole IoT léger utilisé par l\'ESP32 pour relayer les données.',
 wiki_aes_title: '🔐 AES-256', wiki_aes: 'Standard de chiffrement avancé avec clés 256 bits.',
 ...
 wearableStatus: 'Portable', fieldStatus: 'Appareil terrain', hqMap: 'Carte QG',
 deviceLabel: 'Appareil', heartRate: 'Rythme cardiaque', steps: 'Pas', battery: 'Batterie', statusLabel: 'Statut',
 signal: 'Signal', temperature: 'Temp',
 missionSelect: 'Sélecteur de mission', startMission: 'Démarrer la mission', connectDevices: 'Connecter les appareils',
 encryptedComms: 'Comms chiffrées', send: 'Envoyer',
 agentTracker: 'Suivi d\'agents en direct', addAgent: 'Ajouter un agent', trackAll: 'Suivre tous',
 ...
 theory: 'Kit Spy Pro démontre les principes clés de renseignement RF. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. Bluetooth Low Energy uses 40 channels in the 2.4 GHz ISM band. It supports advertising (broadcast) and connection (point-to-point) modes. A radio channel is a defined frequency range allocated for communication. Adjacent channels can interfere, so proper channel planning is essential. La simulation modélise ces mécanismes à l aide d équations mathématiques dans votre navigateur. Chaque contrôle correspond à un paramètre réel. En expérimentant ici, vous développez l intuition que les professionnels acquièrent après des années d expérience pratique.',
 ...
 wiki_mqtt_title: '📡 MQTT', wiki_mqtt: 'بروتوكول IoT خفيف يستخدمه ESP32 لنقل البيانات.',
 wiki_aes_title: '🔐 AES-256', wiki_aes: 'معيار التشفير المتقدم بمفاتيح 256 بت.',
 wiki_privacy_title: '🔒 الخصوصية', wiki_privacy: 'محلي أولًا، خصوصية أولًا. كل البيانات تبقى في متصفحك.',
 working: 'جارٍ…',
 t_mosque: 'مسجد', t_zellige: 'زليج', t_andalus: 'أندلس', t_riad: 'رياض', t_medina: 'مدينة', t_space: 'فضاء', t_jungle: 'أدغال', t_robot: 'روبوت',
 ready: '🎯 طقم التجسس برو جاهز!',
 ...
 wearableStatus: 'قابل للارتداء', fieldStatus: 'الجهاز الميداني', hqMap: 'خريطة المقر',
 deviceLabel: 'الجهاز', heartRate: 'نبض القلب', steps: 'الخطوات', battery: 'البطارية', statusLabel: 'الحالة',
 signal: 'الإشارة', temperature: 'الحرارة',
 missionSelect: 'اختيار المهمة', startMission: 'بدء المهمة', connectDevices: 'توصيل الأجهزة',
 encryptedComms: 'اتصالات مشفرة', send: 'إرسال',
 agentTracker: 'متتبع العملاء المباشر', addAgent: 'إضافة عميل', trackAll: 'تتبع الكل',
 ...
 theory: 'Kit Spy Pro يوضح المبادئ الأساسية في الاستخبارات اللاسلكية. Encryption transforms plaintext into ciphertext using a mathematical algorithm and a key. The strength depends on key length and algorithm choice. Bluetooth Low Energy uses 40 channels in the 2.4 GHz ISM band. It supports advertising (broadcast) and connection (point-to-point) modes. A radio channel is a defined frequency range allocated for communication. Adjacent channels can interfere, so proper channel planning is essential. تحاكي هذه المحاكاة الآليات الحقيقية باستخدام معادلات رياضية في متصفحك. كل عنصر تحكم يتوافق مع معامل حقيقي. بالتجربة هنا تبني نفس الحدس الذي يطوره المحترفون عبر سنوات من الخبرة العملية.',
...
function connectDevices() {
 const s = LANG[currentLang];
 log('🔗 Connecting wearable (BLE)...', 'tx');
 showToast(s.working, 0);

 setTimeout(() => {
 const ws = $('wearStatus');
 if (ws) ws.innerHTML = '<span class="status-indicator status-online"></span>Online';
 log('⌚ micro:bit v2 connected via BLE', 'success');

 setTimeout(() => {
 const fs = $('fieldDevStatus');
 if (fs) fs.innerHTML = '<span class="status-indicator status-online"></span>Online';
 log('📡 ESP32-S3 connected via MQTT', 'success');
 devicesConnected = true;
 setStatus(true);
 hideToast();
 log(s.devicesConnected, 'success');
 playSound('success');
 startTelemetry();
 }, 800);
 }, 600);
...
function startTelemetry() {
 ...
 const hr = $('wearHR'); if (hr) hr.textContent = randomInt(62, 95) + ' bpm';
 const st = $('wearSteps'); if (st) st.textContent = (parseInt(st.textContent) || 0) + randomInt(1, 8);
 const wb = $('wearBatt'); if (wb) wb.textContent = Math.max(10, 100 - Math.floor((Date.now() / 10000) % 90)) + '%';
 const sig = $('fieldSignal'); if (sig) sig.textContent = '-' + randomInt(30, 75) + ' dBm';
 const tmp = $('fieldTemp'); if (tmp) tmp.textContent = randomInt(18, 32) + '°C';
 const fb = $('fieldBatt'); if (fb) fb.textContent = Math.max(15, 100 - Math.floor((Date.now() / 12000) % 85)) + '%';
 }, 2000);
}

function drawHQMap(canvas) {
 if (!canvas) return;
 ...
 ctx.font = '10px Courier New'; ctx.fillText('HQ', canvas.width / 2 + 10, canvas.height / 2 + 4);
 // Agents
 agents.forEach((a, i) => {
 ...
 ctx.font = '8px Courier New'; ctx.fillText(a.name, a.x + 6, a.y + 3);
 });
}
...
function startMission() {
 ...
 const mission = sel ? sel.value : 'recon';
 const names = { recon: 'Recon', intercept: 'Intercept', extract: 'Extraction' };
 missionActive = true;
 log(`${s.missionStarted} ${names[mission]}`, 'success');

 let step = 0;
 const steps = {
 recon: ['📍 Deploying sensor Alpha...', '📍 Deploying sensor Bravo...', '📍 Deploying sensor Charlie...', '📊 Collecting signal data...', '📡 Transmitting report to HQ...'],
 intercept: ['📡 Scanning frequencies...', '🔍 Rogue signal detected at 2.437 GHz', '📐 Triangulating source...', '🎯 Source located: Grid E7', '🔓 Decoding transmission...'],
 extract: ['⌚ Agent wearable signal acquired', '📍 Tracking agent position...', '🗺️ Agent located: Sector 4', '🔐 Sending encrypted route...', '🚁 Extraction point secured'],
 };
 const missionSteps = steps[mission] || steps.recon;

 const interval = setInterval(() => {
 if (step >= missionSteps.length) {
 clearInterval(interval);
 missionActive = false;
 log(s.missionComplete, 'success');
 playSound('success');
 return;
 }
 log(missionSteps[step], step === missionSteps.length - 1 ? 'success' : 'rx');
 step++;
 }, 1200);
}
...
function sendCommsMessage() {
 ...
 const encrypted = fakeEncrypt(text);
 const hex = toHex(text).slice(0, 30) + '...';

 const msgDiv = document.createElement('div');
 msgDiv.className = 'comms-msg encrypted';
 msgDiv.textContent = `[TX] ${encrypted}`;
 box.appendChild(msgDiv);
 log(`${s.messageSent}: ${hex}`, 'tx');

 // Simulated response
 setTimeout(() => {
 const responses = ['Roger that.', 'Copy. Moving to position.', 'Acknowledged. Standing by.', 'Intel received. Over.', 'Affirmative. Proceeding.'];
 const resp = responses[randomInt(0, responses.length - 1)];
 const respDiv = document.createElement('div');
 respDiv.className = 'comms-msg decoded';
 respDiv.textContent = `[RX] ${resp}`;
 box.appendChild(respDiv);
 box.scrollTop = box.scrollHeight;
 log(`${s.messageReceived}: ${fakeEncrypt(resp)}`, 'rx');
 }, 1000 + randomInt(500, 2000));
...
function addAgent() {
 ...
 const canvas = $('trackerCanvas');
 if (!canvas) return;
 const idx = agents.length % names.length;
 agents.push({
 name: names[idx], color: colors[idx],
 x: randomInt(30, canvas.clientWidth - 30),
 y: randomInt(30, canvas.clientHeight - 30),
 });
 log(`➕ Agent ${names[idx]} added to tracker`, 'success');
}

function trackAllAgents() {
 if (agents.length === 0) { log('⚠️ No agents to track', 'error'); return; }
 agents.forEach(a => {
 log(`📍 ${a.name}: Grid ${String.fromCharCode(65 + randomInt(0, 7))}${randomInt(1, 9)} — Signal: -${randomInt(30, 70)} dBm`, 'rx');
 });
}

function runSpeedRecon() {
 const s = LANG[currentLang];
 log('⏱️ Speed Recon challenge started!', 'tx');
 const result = $('speedReconResult');
 let step = 0;
 const interval = setInterval(() => {
 step++;
 log(`📍 Sensor ${step} deployed — ${randomInt(1, 5)}s`, 'success');
 if (step >= 3) {
 clearInterval(interval);
 const time = randomInt(28, 55);
 if (result) result.innerHTML = `<div style="color:var(--accent);font-size:0.75rem;">🏆 Completed in ${time}s — ${time < 40 ? 'EXCELLENT!' : 'Good job!'}</div>`;
 log(`🏆 Speed Recon complete: ${time}s`, 'success');
 playSound('success');
 }
 }, randomInt(600, 1500));
}

function runDecodeIntercept() {
 const s = LANG[currentLang];
 const plaintext = ['MEET AT DAWN', 'DROP POINT ALPHA', 'ABORT MISSION', 'TARGET ACQUIRED'][randomInt(0, 3)];
 const encrypted = fakeEncrypt(plaintext);
 log(`🔐 Intercepted: ${encrypted}`, 'rx');
 const result = $('decodeResult');
 setTimeout(() => {
 if (result) result.innerHTML = `<div style="font-size:0.75rem;"><div style="color:var(--text-muted);">Encrypted: <span style="font-family:monospace;">${encrypted}</span></div><div style="color:#22c55e;margin-top:4px;">Decoded: <strong>${plaintext}</strong></div></div>`;
 log(`🔓 Decoded: ${plaintext}`, 'success');
 playSound('success');
 }, 1500);
}
...
function startMapAnimation() {
 const canvas = $('hqMapCanvas');
 const trackerCanvas = $('trackerCanvas');
 function animate() {
 drawHQMap(canvas);
 if (trackerCanvas) {
 trackerCanvas.width = trackerCanvas.clientWidth;
 trackerCanvas.height = trackerCanvas.clientHeight;
 drawHQMap(trackerCanvas);
 }
...
function initSpyKitPro() {
 const connectBtn = $('connectBtn');
 const startBtn = $('startMissionBtn');
 const sendBtn = $('commsSendBtn');
 const commsInput = $('commsInput');
 const addBtn = $('addAgentBtn');
 const trackBtn = $('trackAllBtn');
 const speedBtn = $('speedReconBtn');
 const decodeBtn = $('decodeBtn');

 if (connectBtn) connectBtn.onclick = connectDevices;
 if (startBtn) startBtn.onclick = startMission;
 if (sendBtn) sendBtn.onclick = sendCommsMessage;
 if (commsInput) commsInput.addEventListener('keydown', e => { if (e.key === 'Enter') sendCommsMessage(); });
 if (addBtn) addBtn.onclick = addAgent;
 if (trackBtn) trackBtn.onclick = trackAllAgents;
 if (speedBtn) speedBtn.onclick = runSpeedRecon;
 if (decodeBtn) decodeBtn.onclick = runDecodeIntercept;
