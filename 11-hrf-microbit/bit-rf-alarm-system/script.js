...
const LANG = {
 ...
 missionTitle:'MISSION BRIEFING',missionClassified:'CLASSIFIED',missionObjective:'Your mission objective:',missionAgent:'AGENT-2A785E',missionSkip:'Skip',missionGo:'ACCEPT MISSION',mission_obj:'Explore and master Rf Alarm System \u2014 analyze, experiment, and complete all challenges.',nightVisionTitle:'Night Vision Mode',nightVisionOn:'NV ON',nightVisionOff:'NV OFF',nightVisionAuto:'Auto NV',
 ...
 title: 'Rf Alarm System', subtitle: '🚨 Build · 📡 Jam · 🔐 Secure',
 disconnected: 'Disconnected', connected: 'Connected',
 mainSection: 'RF Alarm System — Jam & Spoof Lab', mainDesc: 'Build an alarm, then hack it to learn security',
 ...
 howto_1: 'The main display shows the Rf Alarm System simulation. At the top you see the live visualization — colors and animations represent real data changing in real time. Below it, the control panel has buttons and sliders that each adjust a specific parameter. Start by looking at how Configure the parameters for Rf Alarm System. Choose your in',
 ...
 howto_4: 'Switch to Secure mode and try the same attacks again — see how rolling codes defeat replay attacks.',
 wiki_jamming_title: '📡 RF Jamming', wiki_jamming: 'RF jamming floods a radio channel with noise, preventing legitimate devices from communicating. Attackers use it to block alarm disarm signals, keeping alarms from responding to commands.',
 wiki_replay_title: '🔄 Replay Attacks', wiki_replay: 'A replay attack records a valid wireless transmission and retransmits it later. If the system uses fixed codes, the replayed signal is accepted as genuine — the receiver cannot tell the difference.',
 wiki_rolling_title: '🔑 Rolling Codes', wiki_rolling: 'Rolling codes (also called hopping codes) use a synchronized counter between transmitter and receiver. Each code is valid only once, making captured codes useless for replay attacks.',
 wiki_secure_title: '🔒 Secure RF Protocols', wiki_secure: 'Modern secure RF protocols combine rolling codes with AES encryption, frequency hopping, and challenge-response authentication. This makes jamming, replay, and spoofing attacks extremely difficult.',
 working: 'Working…',
 t_mosque: 'Mosque', t_zellige: 'Zellige', t_andalus: 'Andalus',
 t_riad: 'Riad', t_medina: 'Medina',
 t_space: 'Space', t_jungle: 'Jungle', t_robot: 'Robot',
 ready: '🚨 RF Alarm Lab ready!',
 ...
 themeChanged: '🎨 Theme →',
 // Alarm-specific keys
 alarmArmed: 'ARMED', alarmDisarmed: 'DISARMED', alarmAlert: '!! ALERT !!',
 armBtn: 'Arm', disarmBtn: 'Disarm',
 jamBtn: 'Jam Signal', replayBtn: 'Record & Replay',
 rfChannelLabel: 'CH 7 — 433 MHz', signalLabel: 'Signal Quality',
 basicMode: 'Basic (no encryption)', secureMode: 'Secure (rolling codes)',
 howwork_1: 'The alarm system sends and receives commands over RF (radio frequency) on a fixed channel (433 MHz). Pressing "Arm" or "Disarm" on the remote transmits a code wirelessly.',
 howwork_2: 'In Basic mode, the system uses a fixed code ("1234") sent as plaintext. Any device on the same frequency can listen and read the code — no encryption, no protection.',
 howwork_3: 'A jammer floods the RF channel with noise, drowning out legitimate signals. While jamming is active, the alarm cannot receive arm/disarm commands — signal quality drops to near zero.',
 howwork_4: 'A replay attack records the disarm code when it is transmitted, then retransmits the captured frame later. In Basic mode this works perfectly — but Secure mode uses rolling codes that expire after one use.',
 lab_1: 'Make sure the alarm is in Basic mode (toggle off). Arm the alarm, then press "Jam Signal" and try to disarm — notice the command fails while jamming is active.',
 lab_2: 'Stop jamming, then press "Record & Replay". Disarm the alarm normally — the replay tool captures the disarm frame. Now arm the alarm again and press "Record & Replay" to retransmit the captured code — the alarm disarms!',
 lab_3: 'Switch to Secure mode (rolling codes). Arm the alarm, then try the replay attack again — it fails! The code has already been used and is no longer valid.',
 lab_4: 'Observe the rolling code display below the signal meter. Each arm/disarm command generates a unique code. Compare Basic mode (always "1234") with Secure mode (changes every time).',
 challenge_1_title: 'Challenge 1 — Disarm Without the Code',
 challenge_1_desc: 'The alarm is armed in Basic mode. Use only the attack tools (jam and replay) to disarm it without knowing the code. Hint: record first, then replay.',
 challenge_2_title: 'Challenge 2 — Defeat Rolling Codes',
 challenge_2_desc: 'Switch to Secure mode. Can you still disarm the alarm with a replay attack? Why does it fail? Discuss how rolling codes prevent replay attacks.',
 challenge_3_title: 'Challenge 3 — Design an Unbreakable Alarm',
 challenge_3_desc: 'Imagine you are building a real alarm system. What features would you add beyond rolling codes? Think about encryption, frequency hopping, two-factor authentication, and tamper detection.',
 // Simulation messages
 msgArming: '📡 TX → ARM command sent',
 msgArmed: '🔒 Alarm ARMED',
 msgDisarming: '📡 TX → DISARM command sent',
 msgDisarmed: '🔓 Alarm DISARMED',
 msgJamStart: '⚡ Jammer ON — flooding channel with noise',
 msgJamStop: '⚡ Jammer OFF — channel clear',
 msgJamBlock: '🚫 Command blocked — channel jammed!',
 msgRecording: '🔴 Recording — waiting for disarm signal...',
 msgRecorded: '📼 Disarm frame captured!',
 msgReplaying: '📡 TX → Replaying captured disarm frame...',
 msgReplayOk: '✅ Replay attack SUCCESS — alarm disarmed!',
 msgReplayFail: '❌ Replay attack FAILED — code expired (rolling codes)',
 msgNoCapture: '📼 No captured frame to replay. Record a disarm first.',
 msgKeypadOk: '🔑 Keypad code accepted',
 msgKeypadFail: '🔑 Keypad code rejected',
 msgSecureOn: '🔐 Secure mode ON — rolling codes enabled',
 msgSecureOff: '🔓 Secure mode OFF — basic fixed codes',
 msgAlertTriggered: '🚨 ALERT triggered — wrong code!',
 msgCodeUsed: 'Code: ',
 msgRollingCode: 'Rolling code: ',step1Title:'Set Up',step1Desc:'Configure the parameters for Rf Alarm System. Choose your input settings using the controls in the main card. Each control directly affects the simulation output.',step2Title:'Run',step2Desc:'Press "Start" to launch the simulation. Watch the main visualization update in real time as the system processes your inputs.',step3Title:'Observe',step3Desc:'Study the "How It Works" section below for detailed data. The numbers and graphs show exactly what is happening inside the simulation at each moment.',step4Title:'Experiment',step4Desc:'Change parameters one at a time and re-run. Compare results in "Lab". Try extreme values to discover the limits of the system.',sectionCode:'Device Code',faq_q1:'What is Rf Alarm System?',faq_a1:'Rf Alarm System is an interactive simulation that demonstrates satellite comms concepts. Build an alarm, then hack it to learn security. Everything runs in your browser — no hardware or installation needed. Adjust the controls, observe the results, and build real understanding through experimentation.',faq_q2:'How does the simulation work?',faq_a2:'The simulation models real RF exploration behavior in your browser. You control the inputs using sliders and buttons, and the visualization updates in real time to show you the results.',faq_q3:'What do the controls do?',faq_a3:'Press "Start" to begin. Each button and slider changes a specific parameter — the visualization responds immediately so you can see the effect. Check the How-To tab for a step-by-step guide.',faq_q4:'What is the science behind this?',faq_a4:'This app is based on real RF exploration principles used by professionals. The simulation applies the same math and physics — the difference is that here you can safely experiment without expensive equipment.',faq_q5:'What should I experiment with?',faq_a5:'Change one parameter at a time and observe the effect. Try extreme values to find the limits. Then combine changes to see how different factors interact. The challenges section gives you specific experiments to try.',faq_q6:'What hardware do I need for the real version?',faq_a6:'The simulation needs no hardware. To build the real project, you need HackRF + micro:bit. See the Device Code section for wiring diagrams and ready-to-use firmware.',faq_q7:'Is my data private?',faq_a7:'Yes. Everything runs locally in your browser using JavaScript. No data is sent to any server, no account is needed, and the app works completely offline. Your experiments stay on your device.',faq_q8:'What should I explore next?',faq_a8:'Try Bit Frequency Duel and Bit Micro Radar. Each app in this category teaches a different aspect of RF exploration. Try systematically: set all controls to their defaults, then change one variable at a time. Record what happens at minimum, midpoint, and maximum values. This methodical approach is exactly how researchers and engineers characterize real systems.',demo_s1:'Welcome to Rf Alarm System! Look at the main display — this is where the RF exploration simulation runs.',demo_s2:'Start by arming the alarm using the "Arm" remote button or enter code 1234 on the keypad and press OK. Watch how the visualization reacts.',demo_s3:'Try adjusting the controls. Each slider or button changes a specific parameter of the simulation.',demo_s4:'Scroll down to see "How It Works" for detailed data. The numbers and charts update as the simulation runs.',demo_s5:'Great job! Now try the challenges section to test your understanding of RF exploration.',sectionDemo:'Watch Demo',demoPlay:'Play',demoPause:'Pause',demoPrev:'Prev',demoNext:'Next',learn1Title:'RF Signals',learn1Desc:'How radio frequency energy carries information. Watch the simulation to see this happen in real time. The visualization makes the invisible visible.',learn1Tag:'RF',learn2Title:'Signal Analysis',learn2Desc:'How to identify and classify unknown signals. The controls let you experiment with different conditions. Each change reveals how this principle responds.',learn2Tag:'SIGINT',learn3Title:'Spectrum Monitoring',learn3Desc:'How to scan and map the radio spectrum. Try the challenges section to test your understanding. Real engineers use these same concepts daily.',learn3Tag:'Spectrum',learn4Title:'RF Security',learn4Desc:'How to detect and defend against RF threats. Compare results with different settings to build intuition. The data panels show precise measurements.',learn4Tag:'Security',sectionLearn:'What You Shall Learn',learnLevelVal:'Intermediate 🟡',learnLevel:'Level:',learnTimeVal:'20 min ⏱',learnTime:'Time:',learnAgeVal:'12+ 🧒',kidTitle:'For Young Explorers',kidIntro:'Welcome to Rf Alarm System! This is like a science experiment on your computer. You get to control a real satellite comms simulation — press buttons, move sliders, and watch what happens on screen. Try changing the controls and watch how Configure the parameters for Rf Alarm System. Choo Nothing can break — it is all just a simulation running safely in your browser!',kidSafe:'Completely safe! Nothing you do here can break anything. It all runs inside your browser like a game.',kidTry:'Press the big Start button and watch the screen change. Then try moving sliders to see what they do.',kidParent:'This app teaches RF exploration concepts through hands-on simulation. Suitable for STEM education in physics, electronics, and computer science.',ch1Title:'Signal Hunt',ch1Desc:'Tune across the frequency range and find the hidden signal. What frequency is it on? What modulation does it use?',ch2Title:'Noise Floor',ch2Desc:'Measure the noise floor at different frequencies. Where is it lowest? What natural and man-made sources contribute to noise?',ch3Title:'Bandwidth Test',ch3Desc:'Transmit data at different bandwidths. How does bandwidth affect data rate and signal quality? Find the optimal trade-off.',codeTitle:'Starter Code',codeLang:'MicroPython (micro:bit)',codeSnippet:'from microbit import *\\nimport radio\\n\\nradio.on()\\nradio.config(group=42, power=7)\\n\\nwhile True:\\n msg = radio.receive()\\n if msg:\\n display.scroll(msg)\\n if button_a.was_pressed():\\n radio.send("HELLO")\\n display.show(Image.YES)',codeExplain:'This code turns on the micro:bit radio on group 42 at full power. When button A is pressed, it broadcasts "HELLO". Any received message scrolls across the LED display. Group numbers act like channels — only micro:bits on the same group hear each other.',purpose:'Rf Alarm System: Build an alarm, then hack it to learn security. This simulation lets you experiment hands-on instead of just reading theory. Every parameter you change produces visible results, building real intuition for how the system behaves. The workflow goes from Configure RF through Capture Spectrum to Analyze Signal and Classify & Report.',guideTitle:'What Am I Looking At?',guideCanvas:'The main area shows a live visualization of the simulation. Colors and movement represent data changing in real time.',guideControls:'The buttons below the main display control the simulation. Start begins it, Stop pauses it, Reset clears everything.',guideSections:'Below the main card, "How It Works" and "Lab" show detailed data and analysis. Click the section headers to expand or collapse them.',guideStatus:'The colored dot in the top-right corner shows the connection status. Green means running, red means stopped.',learnAge:'Ages:',relatedTitle:'\ud83d\udd17 Related Apps',related1_name:'Spectrum Denial Lab',related1_desc:'RF area denial zones and frequency blocking simulation',related1_path:'../../54-rf-warfare/rfw-spectrum-denial-lab/index.html',related2_name:'Signal Classifier',related2_desc:'Identify modulation types with AI analysis',related2_path:'../../32-sdr-tools/sdr-signal-classifier/index.html',related3_name:'GPS Spoofing Simulator',related3_desc:'Simulate satellite spoofing and position manipulation',related3_path:'../../54-rf-warfare/rfw-gps-spoofing-sim/index.html',pathTitle:'\ud83d\udee4\ufe0f Learning Path',pathPrev_name:'Micro Radar — EM Map',pathPrev_path:'../../11-hrf-microbit/bit-micro-radar/index.html',pathNext_name:'RF Remote Control — Build Your Protocol',pathNext_path:'../../11-hrf-microbit/bit-rf-remote-control/index.html',
 shortcutsTitle:'⌨️ Keyboard Shortcuts',
 shortcutsInfo:'? = Help, Esc = Close, S = Start, R = Reset, 1-9 = Tabs',
 achieveTitle:'🏆 Achievements',
 achieveExplorer:'Explorer — visited 4+ help tabs',
 ...
 theory: 'Rf Alarm System demonstrates key principles from satellite comms. Communication satellites relay signals between ground stations. GEO satellites orbit at 35,786 km providing wide coverage; LEO satellites at 200-2000 km provide low latency. The simulation models these real-world mechanisms using mathematical equations running in your browser. Every control maps to a real parameter that engineers tune in professional settings. By experimenting here, you build the same intuition that professionals develop through years of hands-on experience.',
 faq_q9: 'What common mistakes should I avoid?',
 faq_a9: 'The most common mistake is changing multiple parameters at once, which makes it impossible to understand cause and effect. Always change one thing at a time. Another common error is ignoring the activity log — it records every event and helps you understand the sequence of operations. Finally, do not skip the challenge section: those questions test whether you truly understand the concepts or just memorized the button sequence.',
 faq_q10: 'How does this relate to real-world satellite communications?',
 faq_a10: 'This simulation models the same physics and mathematics used in professional satellite communications systems. The parameters you adjust correspond to real equipment settings. The visualizations show data patterns identical to what you would see on professional instruments like oscilloscopes, spectrum analyzers, or protocol decoders. The main difference is that this runs safely in your browser — real systems use HackRF SDR hardware and may have legal requirements for operation. Skills you develop here transfer directly to hands-on work.',
 ...
 missionTitle:'BRIEFING DE MISSION',missionClassified:'CLASSIFI\xc9',missionObjective:'Objectif de mission :',missionAgent:'AGENT-2A785E',missionSkip:'Passer',missionGo:'ACCEPTER LA MISSION',mission_obj:'Explorer et ma\xeetrisez Rf Alarm System \u2014 analysez, exp\xe9rimentez et compl\xe9tez tous les d\xe9fis.',nightVisionTitle:'Mode Vision Nocturne',nightVisionOn:'VN ON',nightVisionOff:'VN OFF',nightVisionAuto:'VN Auto',
 ...
 title: 'Rf Alarm System', subtitle: '🚨 Construire · 📡 Brouiller · 🔐 Securiser',
 disconnected: 'Deconnecte', connected: 'Connecte',
 mainSection: 'Alarme RF — Labo Brouillage & Usurpation', mainDesc: 'Construis une alarme, puis pirate-la pour apprendre la securite',
 sectionA: 'Comment ca marche', sectionB: 'Labo', sectionC: 'Defi',
 ...
 howto_1: 'L écran principal affiche la simulation Rf Alarm System. En haut, la visualisation en direct — les couleurs et animations représentent des données réelles changeant en temps réel. En dessous, le panneau de contrôle a des boutons et curseurs qui ajustent chaque paramètre. Start by looking at how Configure the parameters for Rf Alarm System. Choose your in',
 ...
 howto_4: 'Passe en mode Securise et reessaie les memes attaques — les codes tournants les bloquent !',
 wiki_jamming_title: '📡 Brouillage RF', wiki_jamming: 'Le brouillage RF inonde un canal radio de bruit, empechant les appareils legitimes de communiquer.',
 wiki_replay_title: '🔄 Attaques par rejeu', wiki_replay: 'Une attaque par rejeu enregistre une transmission valide et la retransmet plus tard. Avec des codes fixes, le signal rejoue est accepte.',
 wiki_rolling_title: '🔑 Codes tournants', wiki_rolling: 'Les codes tournants utilisent un compteur synchronise entre emetteur et recepteur. Chaque code n\'est valide qu\'une seule fois.',
 wiki_secure_title: '🔒 Protocoles RF securises', wiki_secure: 'Les protocoles RF securises modernes combinent codes tournants, chiffrement AES, saut de frequence et authentification defi-reponse.',
 working: 'En cours…',
 t_mosque: 'Mosquee', t_zellige: 'Zellige', t_andalus: 'Andalous',
 t_riad: 'Riad', t_medina: 'Medina',
 t_space: 'Espace', t_jungle: 'Jungle', t_robot: 'Robot',
 ready: '🚨 Labo alarme RF pret !',
 ...
 themeChanged: '🎨 Theme →',
 alarmArmed: 'ARMEE', alarmDisarmed: 'DESARMEE', alarmAlert: '!! ALERTE !!',
 armBtn: 'Armer', disarmBtn: 'Desarmer',
 jamBtn: 'Brouiller', replayBtn: 'Enregistrer & Rejouer',
 rfChannelLabel: 'CH 7 — 433 MHz', signalLabel: 'Qualite du signal',
 basicMode: 'Basique (sans chiffrement)', secureMode: 'Securise (codes tournants)',
 howwork_1: 'Le systeme d\'alarme envoie et recoit des commandes par RF (radiofrequence) sur un canal fixe (433 MHz). Appuyer sur "Armer" ou "Desarmer" transmet un code sans fil.',
 howwork_2: 'En mode Basique, le systeme utilise un code fixe ("1234") envoye en clair. Tout appareil sur la meme frequence peut ecouter et lire le code.',
 howwork_3: 'Un brouilleur inonde le canal RF de bruit, noyant les signaux legitimes. Tant que le brouillage est actif, l\'alarme ne peut pas recevoir de commandes.',
 howwork_4: 'Une attaque par rejeu enregistre le code de desarmement lors de sa transmission, puis le retransmet. En mode Basique ca marche — mais en mode Securise les codes tournants expirent apres usage.',
 lab_1: 'Assure-toi que l\'alarme est en mode Basique. Arme l\'alarme, puis appuie sur "Brouiller" et essaie de desarmer — la commande echoue pendant le brouillage.',
 lab_2: 'Arrete le brouillage, puis appuie sur "Enregistrer & Rejouer". Desarme normalement — l\'outil capture la trame. Rearme et rejoue le code capture — l\'alarme se desarme !',
 lab_3: 'Passe en mode Securise (codes tournants). Arme l\'alarme, puis reessaie l\'attaque par rejeu — elle echoue ! Le code a deja ete utilise.',
 lab_4: 'Observe l\'affichage des codes tournants. Chaque commande genere un code unique. Compare le mode Basique (toujours "1234") et le mode Securise (change a chaque fois).',
 challenge_1_title: 'Defi 1 — Desarmer sans le code',
 challenge_1_desc: 'L\'alarme est armee en mode Basique. Utilise uniquement les outils d\'attaque pour la desarmer sans connaitre le code.',
 challenge_2_title: 'Defi 2 — Vaincre les codes tournants',
 challenge_2_desc: 'Passe en mode Securise. Peux-tu encore desarmer avec une attaque par rejeu ? Pourquoi ca echoue ?',
 challenge_3_title: 'Defi 3 — Concevoir une alarme inviolable',
 challenge_3_desc: 'Imagine que tu construis un vrai systeme d\'alarme. Quelles fonctionnalites ajouterais-tu au-dela des codes tournants ?',
 msgArming: '📡 TX → Commande ARM envoyee',
 msgArmed: '🔒 Alarme ARMEE',
 msgDisarming: '📡 TX → Commande DISARM envoyee',
 msgDisarmed: '🔓 Alarme DESARMEE',
 msgJamStart: '⚡ Brouilleur ON — inondation du canal',
 msgJamStop: '⚡ Brouilleur OFF — canal libre',
 msgJamBlock: '🚫 Commande bloquee — canal brouille !',
 msgRecording: '🔴 Enregistrement — en attente du signal de desarmement...',
 msgRecorded: '📼 Trame de desarmement capturee !',
 msgReplaying: '📡 TX → Rejeu de la trame capturee...',
 msgReplayOk: '✅ Attaque par rejeu REUSSIE — alarme desarmee !',
 msgReplayFail: '❌ Attaque par rejeu ECHOUEE — code expire (codes tournants)',
 msgNoCapture: '📼 Pas de trame capturee. Enregistre un desarmement d\'abord.',
 msgKeypadOk: '🔑 Code clavier accepte',
 msgKeypadFail: '🔑 Code clavier rejete',
 msgSecureOn: '🔐 Mode securise ON — codes tournants actives',
 msgSecureOff: '🔓 Mode securise OFF — codes fixes basiques',
 msgAlertTriggered: '🚨 ALERTE declenchee — mauvais code !',
 msgCodeUsed: 'Code : ',
 msgRollingCode: 'Code tournant : ',step1Title:'Configurer',step1Desc:'Configure les paramètres de Rf Alarm System. Choisis tes réglages à l\'aide des contrôles de la carte principale. Chaque contrôle affecte directement le résultat.',step2Title:'Exécuter',step2Desc:'Appuie sur "Start" pour lancer la simulation. La visualisation se met à jour en temps réel.',step3Title:'Observer',step3Desc:'Étudie la section ci-dessous pour les données détaillées. Les chiffres et graphiques montrent ce qui se passe dans la simulation.',step4Title:'Expérimenter',step4Desc:'Change un paramètre à la fois et relance. Compare les résultats. Essaie des valeurs extrêmes pour découvrir les limites.',sectionCode:'Code Appareil',faq_q1:'Qu\'est-ce que Rf Alarm System ?',faq_a1:'Rf Alarm System est une simulation interactive qui démontre les concepts de communications satellite. Build an alarm, then hack it to learn security. Tout fonctionne dans votre navigateur — aucun matériel requis. Ajustez les contrôles, observez les résultats et construisez une vraie compréhension par l expérimentation.',faq_q2:'Comment fonctionne la simulation ?',faq_a2:'L\'application modélise un vrai comportement de exploration RF. Tu contrôles les entrées et tu observes les sorties changer en temps réel à l\'écran.',faq_q3:'Que font les contrôles ?',faq_a3:'Chaque bouton et curseur modifie un paramètre spécifique. Consulte l\'onglet Guide pour une procédure pas à pas.',faq_q4:'Quelle est la science derrière ?',faq_a4:'Cette application utilise de vrais principes de exploration RF. Les mêmes concepts sont utilisés par les professionnels.',faq_q5:'Que dois-je expérimenter ?',faq_a5:'Change un paramètre à la fois et observe l\'effet. Pousse les valeurs à l\'extrême pour voir les limites du système.',faq_q6:'Quel matériel pour la version réelle ?',faq_a6:'La simulation ne nécessite aucun matériel. Pour construire le vrai projet, il te faut HackRF + micro:bit. Voir la section Code Appareil.',faq_q7:'Mes données sont-elles privées ?',faq_a7:'Oui. Tout fonctionne localement dans ton navigateur. Aucune donnée n\'est envoyée, aucun compte nécessaire, et ça marche hors ligne.',faq_q8:'Que découvrir ensuite ?',faq_a8:'Essaie d\'autres applications de cette catégorie. Chacune enseigne un aspect différent de exploration RF. Essayez systématiquement : mettez tous les contrôles par défaut, puis changez une variable à la fois. Notez ce qui se passe aux valeurs minimale, médiane et maximale.',demo_s1:'Bienvenue dans Rf Alarm System ! Regarde l\'écran principal — c\'est ici que la simulation de exploration RF fonctionne.',demo_s2:'Clique sur Démarrer et regarde la visualisation réagir.',demo_s3:'Essaie d\'ajuster les contrôles. Chaque curseur ou bouton modifie un paramètre spécifique.',demo_s4:'Descends pour voir les données détaillées. Les chiffres et graphiques se mettent à jour en temps réel.',demo_s5:'Bravo ! Maintenant essaie les défis pour tester ta compréhension de exploration RF.',sectionDemo:'Voir la Démo',demoPlay:'Jouer',demoPause:'Pause',demoPrev:'Préc',demoNext:'Suiv',learn1Title:'RF Signals',learn1Desc:'How radio frequency energy carries information. Regarde la simulation pour voir ça en temps réel. La visualisation rend visible l\'invisible.',learn1Tag:'RF',learn2Title:'Signal Analysis',learn2Desc:'How to identify and classify unknown signals. Les contrôles te permettent d\'expérimenter. Chaque changement révèle comment ce principe réagit.',learn2Tag:'SIGINT',learn3Title:'Spectrum Monitoring',learn3Desc:'How to scan and map the radio spectrum. Essaie les défis pour tester ta compréhension. Les vrais ingénieurs utilisent ces mêmes concepts.',learn3Tag:'Spectrum',learn4Title:'RF Security',learn4Desc:'How to detect and defend against RF threats. Compare les résultats avec différents réglages. Les panneaux de données montrent des mesures précises.',learn4Tag:'Security',sectionLearn:'Ce que tu vas apprendre',learnLevelVal:'Intermédiaire 🟡',learnLevel:'Niveau :',learnTimeVal:'20 min ⏱',learnTime:'Durée :',learnAgeVal:'12+ 🧒',kidTitle:'Pour les Jeunes Explorateurs',kidIntro:'Bienvenue dans Rf Alarm System ! C est comme une expérience scientifique sur ton ordinateur. Tu contrôles une vraie simulation de communications satellite — appuie sur les boutons, bouge les curseurs et regarde ce qui se passe. Try changing the controls and watch how Configure the parameters for Rf Alarm System. Choo Rien ne peut casser — tout est une simulation qui tourne en sécurité dans ton navigateur !',kidSafe:'Complètement sûr ! Rien de ce que tu fais ici ne peut casser quoi que ce soit. Tout fonctionne dans ton navigateur comme un jeu.',kidTry:'Appuie sur le gros bouton Démarrer et regarde l\'écran changer. Puis essaie de bouger les curseurs.',kidParent:'Cette appli enseigne des concepts de exploration RF par simulation interactive. Adaptée à l\'enseignement STEM en physique, électronique et informatique.',ch1Title:'Chasse au Signal',ch1Desc:'Balaye les fréquences et trouve le signal caché. Sur quelle fréquence est-il ? Quelle modulation utilise-t-il ?',ch2Title:'Plancher de Bruit',ch2Desc:'Mesure le plancher de bruit à différentes fréquences. Où est-il le plus bas ? Quelles sources contribuent au bruit ?',ch3Title:'Test de Bande Passante',ch3Desc:'Transmets des données à différentes bandes passantes. Comment cela affecte-t-il le débit et la qualité ?',codeTitle:'Code de Démarrage',codeLang:'MicroPython (micro:bit)',codeSnippet:'from microbit import *\\nimport radio\\n\\nradio.on()\\nradio.config(group=42, power=7)\\n\\nwhile True:\\n msg = radio.receive()\\n if msg:\\n display.scroll(msg)\\n if button_a.was_pressed():\\n radio.send("HELLO")\\n display.show(Image.YES)',codeExplain:'Ce code active la radio du micro:bit sur le groupe 42 à pleine puissance. Quand le bouton A est pressé, il diffuse "HELLO". Tout message reçu défile sur l\'écran LED. Les numéros de groupe fonctionnent comme des canaux.',purpose:'Rf Alarm System : Build an alarm, then hack it to learn security. Cette simulation te permet d\'expérimenter au lieu de simplement lire la théorie. Chaque paramètre que tu changes produit des résultats visibles, construisant une vraie intuition du comportement du système.',guideTitle:'Que vois-je à l\'écran ?',guideCanvas:'La zone principale affiche une visualisation en direct de la simulation. Les couleurs et mouvements représentent les données en temps réel.',guideControls:'Les boutons sous l\'écran principal contrôlent la simulation. Démarrer la lance, Arrêter la met en pause, Réinitialiser efface tout.',guideSections:'Sous la carte principale, les sections montrent les données détaillées et l\'analyse. Clique sur les en-têtes pour les déplier.',guideStatus:'Le point coloré en haut à droite montre l\'état. Vert signifie en marche, rouge signifie arrêté.',learnAge:'Âge :',relatedTitle:'\ud83d\udd17 Apps Similaires',related1_name:'Labo Deni Spectral',related1_desc:'Zones de deni RF et blocage de frequences',related1_path:'../../54-rf-warfare/rfw-spectrum-denial-lab/index.html',related2_name:'Classifieur Signal',related2_desc:'Identifier les modulations par analyse IA',related2_path:'../../32-sdr-tools/sdr-signal-classifier/index.html',related3_name:'Simulateur Spoofing GPS',related3_desc:'Simuler le spoofing satellite et la manipulation de position',related3_path:'../../54-rf-warfare/rfw-gps-spoofing-sim/index.html',pathTitle:'\ud83d\udee4\ufe0f Parcours',pathPrev_name:'Micro Radar — Carte EM',pathPrev_path:'../../11-hrf-microbit/bit-micro-radar/index.html',pathNext_name:'Telecommande RF — Construis ton Protocole',pathNext_path:'../../11-hrf-microbit/bit-rf-remote-control/index.html',
 printBtn: '🖨️ Imprimer',quizTab:'Quiz',quizTitle:'Testez vos connaissances',quizRetry:'Rejouer',quizCorrect:'Correct !',quizWrong:'Faux !',quizScore:'Score',quiz_q1:'Qu\'est-ce qu\'un réseau de neurones ?',quiz_q1a:'Fils physiques',quiz_q1b:'Système informatique inspiré des neurones',quiz_q1c:'Réseau social',quiz_q1d:'Réseau radio',quiz_q1_answer:'1',quiz_q2:'Quel processeur utilise le micro:bit V2 ?',quiz_q2a:'ATmega328',quiz_q2b:'nRF52833',quiz_q2c:'ESP32',quiz_q2d:'STM32',quiz_q2_answer:'1',quiz_q3:'Que signifie IA ?',quiz_q3a:'Entrée automatisée',quiz_q3b:'Intelligence Artificielle',quiz_q3c:'Interface analogique',quiz_q3d:'Intégration active',quiz_q3_answer:'1',quiz_q4:'Qui a inventé le code Morse ?',quiz_q4a:'Tesla',quiz_q4b:'Samuel Morse',quiz_q4c:'Edison',quiz_q4d:'Bell',quiz_q4_answer:'1',quiz_q5:'Combien de LEDs a l\'écran du micro:bit ?',quiz_q5a:'16',quiz_q5b:'25',quiz_q5c:'36',quiz_q5d:'64',quiz_q5_answer:'1',realworldTitle:'🌍 Histoires réelles',realworld1:'L\'opération Ivy Bells (1970-80) était une mission conjointe NSA/Marine pour intercepter les cbles de communication sous-marins soviétiques en mer d\'Okhotsk.',realworld2:'Les stations de nombres diffusent des messages chiffrés par ondes courtes aux agents de terrain depuis la Guerre froide. UVB-76 émet un bourdonnement monotone depuis 1982.',realworld3:'L\'affaire Aldrich Ames (1994) a révélé comment une taupe de la CIA utilisait des boîtes aux lettres mortes pour transmettre des renseignements classifiés à l\'Union soviétique pendant près d\'une décennie.',experimentTitle:'🔬 Expériences',experiment_1_title:'Mesure de référence',experiment_1:'Réglez tous les contrôles sur les valeurs par défaut et notez les lectures initiales. Ce sont vos mesures de référence. Un bon scientifique établit toujours une référence avant de modifier des variables — cela donne un point de comparaison pour mesurer tous les changements futurs.',experiment_2_title:'Analyse de sensibilité',experiment_2:'Changez un paramètre à sa valeur minimale, notez le résultat, puis réglez-le au maximum. La différence révèle la sensibilité du système à cette variable. En surveillance, savoir quels paramètres comptent le plus vous aide à concentrer vos efforts.',experiment_3_title:'Effets d\'interaction',experiment_3:'Après avoir testé les paramètres individuellement, changez-en deux simultanément. L\'effet combiné est-il égal à la somme des effets individuels? Les interactions non linéaires sont courantes en surveillance et révèlent la complexité cachée sous des systèmes simples en apparence.',wiki_concept_title:'💡 Concept fondamental',wiki_concept:'Rf Alarm System illustre un concept fondamental en surveillance. Cette simulation modélise comment les systèmes réels traitent les signaux, les données ou les phénomènes physiques. L\'idée clé est que des comportements complexes émergent de règles simples appliquées de manière répétée.',wiki_realworld_title:'🌐 Applications réelles',wiki_realworld:'Les principes démontrés dans Rf Alarm System ont des applications directes dans le monde réel. Les professionnels de surveillance utilisent ces mêmes concepts quotidiennement. Dans l\'industrie, ESP32 et du matériel similaire implémentent ces algorithmes dans des systèmes embarqués.',wiki_safety_title:'⚠️ Sécurité et responsabilité',wiki_safety:'Travailler en surveillance implique des responsabilités importantes. Opérez toujours dans les limites légales. Cette simulation est conçue pour un usage éducatif sûr — elle ne transmet pas de vrais signaux et n\'accède pas à de vrais réseaux.',proTipTitle:'💡 Conseils de pro',proTip1:'Utilisez le saut de canal (canaux 1, 6, 11) pour le scan WiFi — ce sont les seuls canaux 2,4 GHz non chevauchants et ils captent 90% du trafic.',proTip2:'Ajoutez un condensateur de 10μF aux bornes d\x27alimentation de l\x27ESP32. La transmission WiFi cause des pics de courant qui peuvent planter la carte sans découplage.',funFactTitle:'🎯 Le saviez-vous ?',funFact:'Pendant la Guerre froide, la CIA a construit une libellule robot dans les années 1970 pour transporter un microphone miniature. Le projet Insectothopter a été abandonné car il ne pouvait pas voler dans le vent.',mistakeTitle:'⚠️ Erreurs courantes',mistake1:'Changer plusieurs paramètres à la fois rend impossible l\x27isolation de la cause et de l\x27effet. Changez toujours UNE seule variable à la fois.',mistake2:'Sauter la mesure de référence. Sans connaître le comportement par défaut, vous ne pouvez pas mesurer l\x27impact de vos changements.',mistake3:'Ignorer le journal d\x27activité. Il enregistre chaque événement avec des horodatages — essentiel pour comprendre les séquences.'},
 wiki_history_title: '📜 Histoire de communications satellite',
 wiki_history: 'Sputnik (1957) was the first satellite. Telstar (1962) relayed the first TV signals. Modern constellations like Starlink use thousands of LEO satellites. Rf Alarm System s appuie sur ces fondations pour vous permettre d explorer ces concepts historiques par simulation interactive.',
 wiki_math_title: '📐 Mathématiques de Rf Alarm System',
 wiki_math: 'Les mathématiques derrière Rf Alarm System : Free-space path loss FSPL = (4πd/λ)². For GEO at 36,000 km on C-band (4 GHz), FSPL ≈ 196 dB. Link budgets must account for this enormous loss.',
 wiki_advanced_title: '🔬 Techniques avancées',
 wiki_advanced: 'Au-delà des bases de cette simulation, les praticiens avancés de communications satellite utilisent des techniques sophistiquées. Les algorithmes adaptatifs ajustent automatiquement les paramètres. L apprentissage automatique classifie les signaux avec précision. La corrélation multi-canaux détecte des motifs invisibles à l analyse mono-canal. Les réseaux de capteurs distribués combinent les données de plusieurs emplacements.',
 wiki_compare_title: '⚖️ Comparaison des approches',
 wiki_compare: 'Il existe plusieurs approches pour communications satellite. Les solutions matérielles avec HackRF SDR offrent des performances en temps réel. Les simulations logicielles (comme cette app) permettent une expérimentation sûre sans coût de matériel. Les plateformes cloud offrent une évolutivité mais introduisent latence et préoccupations de confidentialité. Cette simulation vous donne les bases pour utiliser efficacement toute approche.',
 wiki_debug_title: '🔧 Guide de dépannage',
 wiki_debug: 'Problèmes courants en communications satellite : (1) Les résultats inattendus proviennent souvent de paramètres incorrects — réinitialisez et modifiez une variable à la fois. (2) Si la visualisation semble gelée, vérifiez que la simulation tourne. (3) Les lectures erratiques indiquent des interférences. (4) Vérifiez vos unités (Hz vs kHz vs MHz). Le débogage systématique est une compétence essentielle.',
 wiki_ethics_title: '⚖️ Éthique et aspects légaux',
 wiki_ethics: 'Communications satellite implique des responsabilités éthiques et légales importantes. De nombreux pays réglementent l utilisation de HackRF SDR. Obtenez toujours une autorisation avant de tester des systèmes que vous ne possédez pas. Respectez les lois sur la vie privée et la protection des données. Cette simulation est conçue à des fins éducatives — elle démontre des principes sans transmettre de signaux réels ni accéder à de vrais réseaux.',
 gloss1_term: 'Link Budget',
 gloss1_def: 'An accounting of all gains and losses in a communication link: transmit power + antenna gains - path loss - atmospheric loss = received signal level.',
 gloss2_term: 'Latency',
 gloss2_def: 'Time delay between sending and receiving data. Lower latency means faster response. Measured in milliseconds (ms).',
 ...
 theory: 'Rf Alarm System démontre les principes clés de communications satellite. Communication satellites relay signals between ground stations. GEO satellites orbit at 35,786 km providing wide coverage; LEO satellites at 200-2000 km provide low latency. La simulation modélise ces mécanismes à l aide d équations mathématiques dans votre navigateur. Chaque contrôle correspond à un paramètre réel. En expérimentant ici, vous développez l intuition que les professionnels acquièrent après des années d expérience pratique.',
 faq_q9: 'Quelles erreurs courantes dois-je éviter ?',
 faq_a9: 'L erreur la plus courante est de modifier plusieurs paramètres simultanément, ce qui rend impossible la compréhension de cause à effet. Modifiez toujours un seul élément à la fois. Une autre erreur est d ignorer le journal d activité — il enregistre chaque événement. Enfin, ne sautez pas la section défis : ces questions testent votre compréhension réelle des concepts.',
 faq_q10: 'Quel est le lien avec satellite communications dans le monde réel ?',
 ...
 mainSection: 'نظام إنذار RF — مختبر التشويش والانتحال', mainDesc: 'ابنِ إنذارًا، ثم اخترقه لتتعلم الأمان',
 ...
 howto_1: 'تعرض الشاشة الرئيسية محاكاة Rf Alarm System. في الأعلى ترى التصور المباشر — الألوان والرسوم المتحركة تمثل بيانات حقيقية تتغير في الوقت الفعلي. أسفلها لوحة التحكم بها أزرار ومنزلقات تضبط كل معامل. Start by looking at how Configure the parameters for Rf Alarm System. Choose your in',
 howto_2: 'جرب زر "تشويش" وراقب جودة الإشارة تنخفض وتفشل الأوامر.',
 howto_3: 'استخدم "تسجيل وإعادة" لالتقاط رمز التعطيل، ثم أعد بثه.',
 howto_4: 'انتقل للوضع الآمن وجرب نفس الهجمات — الرموز المتدحرجة تمنعها!',
 wiki_jamming_title: '📡 تشويش RF', wiki_jamming: 'تشويش RF يغمر قناة الراديو بالضوضاء، مما يمنع الأجهزة الشرعية من التواصل.',
 wiki_replay_title: '🔄 هجمات الإعادة', wiki_replay: 'هجمة الإعادة تسجل إرسالًا صالحًا وتعيد بثه لاحقًا. مع الرموز الثابتة، يُقبل الإشارة المعادة.',
 wiki_rolling_title: '🔑 الرموز المتدحرجة', wiki_rolling: 'الرموز المتدحرجة تستخدم عدادًا متزامنًا بين المرسل والمستقبل. كل رمز صالح مرة واحدة فقط.',
 wiki_secure_title: '🔒 بروتوكولات RF آمنة', wiki_secure: 'البروتوكولات الآمنة الحديثة تجمع بين الرموز المتدحرجة وتشفير AES والقفز الترددي ومصادقة التحدي-الاستجابة.',
 working: 'جارٍ…',
 t_mosque: 'مسجد', t_zellige: 'زليج', t_andalus: 'أندلس',
 t_riad: 'رياض', t_medina: 'مدينة',
 t_space: 'فضاء', t_jungle: 'أدغال', t_robot: 'روبوت',
 ready: '🚨 مختبر إنذار RF جاهز!',
 ...
 themeChanged: '🎨 المظهر ←',
 alarmArmed: 'مُفَعَّل', alarmDisarmed: 'مُعَطَّل', alarmAlert: '!! إنذار !!',
 armBtn: 'تفعيل', disarmBtn: 'تعطيل',
 jamBtn: 'تشويش', replayBtn: 'تسجيل وإعادة',
 rfChannelLabel: 'CH 7 — 433 MHz', signalLabel: 'جودة الإشارة',
 basicMode: 'أساسي (بدون تشفير)', secureMode: 'آمن (رموز متدحرجة)',
 howwork_1: 'يرسل نظام الإنذار ويستقبل الأوامر عبر RF (تردد لاسلكي) على قناة ثابتة (433 MHz). الضغط على "تفعيل" أو "تعطيل" يبث رمزًا لاسلكيًا.',
 howwork_2: 'في الوضع الأساسي، يستخدم النظام رمزًا ثابتًا ("1234") يُرسل كنص عادي. أي جهاز على نفس التردد يمكنه الاستماع وقراءة الرمز.',
 howwork_3: 'المشوش يغمر قناة RF بالضوضاء، مما يغرق الإشارات الشرعية. أثناء التشويش، لا يستطيع الإنذار استقبال أوامر.',
 howwork_4: 'هجمة الإعادة تسجل رمز التعطيل عند إرساله، ثم تعيد بث الإطار الملتقط. في الوضع الأساسي ينجح — لكن الوضع الآمن يستخدم رموزًا تنتهي بعد استخدام واحد.',
 lab_1: 'تأكد أن الإنذار في الوضع الأساسي. فعّل الإنذار، ثم اضغط "تشويش" وحاول التعطيل — ستفشل الأوامر أثناء التشويش.',
 lab_2: 'أوقف التشويش، ثم اضغط "تسجيل وإعادة". عطّل الإنذار عاديًا — تلتقط الأداة الإطار. أعد التفعيل ثم أعد بث الرمز الملتقط!',
 lab_3: 'انتقل للوضع الآمن (رموز متدحرجة). فعّل الإنذار، ثم جرب هجمة الإعادة — ستفشل! الرمز استُخدم بالفعل.',
 lab_4: 'راقب عرض الرموز المتدحرجة. كل أمر يولد رمزًا فريدًا. قارن الوضع الأساسي (دائمًا "1234") والوضع الآمن (يتغير كل مرة).',
 challenge_1_title: 'التحدي 1 — تعطيل بدون الرمز',
 challenge_1_desc: 'الإنذار مفعّل في الوضع الأساسي. استخدم أدوات الهجوم فقط لتعطيله بدون معرفة الرمز.',
 challenge_2_title: 'التحدي 2 — هزيمة الرموز المتدحرجة',
 challenge_2_desc: 'انتقل للوضع الآمن. هل يمكنك التعطيل بهجمة إعادة؟ لماذا تفشل؟',
 challenge_3_title: 'التحدي 3 — تصميم إنذار غير قابل للاختراق',
 challenge_3_desc: 'تخيل أنك تبني نظام إنذار حقيقي. ما الميزات التي ستضيفها بعد الرموز المتدحرجة؟',
 msgArming: '📡 إرسال → أمر التفعيل',
 msgArmed: '🔒 الإنذار مُفَعَّل',
 msgDisarming: '📡 إرسال → أمر التعطيل',
 msgDisarmed: '🔓 الإنذار مُعَطَّل',
 msgJamStart: '⚡ المشوش مُفعّل — إغراق القناة بالضوضاء',
 msgJamStop: '⚡ المشوش مُوقف — القناة صافية',
 msgJamBlock: '🚫 الأمر محجوب — القناة مشوشة!',
 msgRecording: '🔴 تسجيل — في انتظار إشارة التعطيل...',
 msgRecorded: '📼 إطار التعطيل ملتقط!',
 msgReplaying: '📡 إرسال → إعادة بث الإطار الملتقط...',
 msgReplayOk: '✅ هجمة الإعادة نجحت — الإنذار معطّل!',
 msgReplayFail: '❌ هجمة الإعادة فشلت — الرمز منتهي (رموز متدحرجة)',
 msgNoCapture: '📼 لا يوجد إطار ملتقط. سجل تعطيلًا أولًا.',
 msgKeypadOk: '🔑 رمز لوحة المفاتيح مقبول',
 msgKeypadFail: '🔑 رمز لوحة المفاتيح مرفوض',
 msgSecureOn: '🔐 الوضع الآمن مُفعّل — الرموز المتدحرجة نشطة',
 msgSecureOff: '🔓 الوضع الآمن مُوقف — رموز ثابتة أساسية',
 msgAlertTriggered: '🚨 إنذار — رمز خاطئ!',
 msgCodeUsed: 'الرمز: ',
 msgRollingCode: 'رمز متدحرج: ',step1Title:'إعداد',step1Desc:'اضبط معاملات Rf Alarm System. اختر إعداداتك باستخدام أدوات التحكم في البطاقة الرئيسية. كل أداة تؤثر مباشرة على نتيجة المحاكاة.',step2Title:'تشغيل',step2Desc:'اضغط "Start" لبدء المحاكاة. شاهد التصور المرئي يتحدث في الوقت الفعلي.',step3Title:'مراقبة',step3Desc:'ادرس القسم أدناه للبيانات التفصيلية. الأرقام والرسوم البيانية تُظهر ما يحدث داخل المحاكاة.',step4Title:'تجريب',step4Desc:'غيّر معاملاً واحداً في كل مرة وأعد التشغيل. قارن النتائج. جرّب قيماً متطرفة لاكتشاف حدود النظام.',sectionCode:'كود الجهاز',faq_q1:'ما هو Rf Alarm System؟',faq_a1:'Rf Alarm System هي محاكاة تفاعلية توضح مفاهيم اتصالات الأقمار. Build an alarm, then hack it to learn security. كل شيء يعمل في متصفحك — لا حاجة لأجهزة أو تثبيت. اضبط عناصر التحكم، راقب النتائج، وابنِ فهماً حقيقياً من خلال التجربة.',faq_q2:'كيف تعمل المحاكاة؟',faq_a2:'التطبيق يحاكي سلوكاً حقيقياً في استكشاف الترددات. أنت تتحكم في المدخلات وتشاهد المخرجات تتغير في الوقت الفعلي.',faq_q3:'ماذا تفعل أدوات التحكم؟',faq_a3:'كل زر ومنزلق يغيّر معاملاً محدداً. راجع تبويب "كيف تستخدم" للحصول على دليل خطوة بخطوة.',faq_q4:'ما العلم وراء هذا؟',faq_a4:'هذا التطبيق يستخدم مبادئ حقيقية من استكشاف الترددات. نفس المفاهيم يستخدمها المحترفون.',faq_q5:'بماذا أجرّب؟',faq_a5:'غيّر معاملاً واحداً في كل مرة وراقب التأثير. ادفع القيم للحدود القصوى لترى حدود النظام.',faq_q6:'ما العتاد المطلوب للنسخة الحقيقية؟',faq_a6:'المحاكاة لا تحتاج عتاداً. لبناء المشروع الحقيقي تحتاج HackRF + micro:bit. راجع قسم كود الجهاز.',faq_q7:'هل بياناتي خاصة؟',faq_a7:'نعم. كل شيء يعمل محلياً في متصفحك. لا تُرسل أي بيانات، لا حاجة لحساب، ويعمل بدون إنترنت.',faq_q8:'ماذا أستكشف بعد ذلك؟',faq_a8:'جرّب تطبيقات أخرى في هذه الفئة. كل تطبيق يعلّم جانباً مختلفاً من استكشاف الترددات. جرب بشكل منهجي: اضبط جميع عناصر التحكم على الافتراضي، ثم غير متغيراً واحداً في كل مرة. سجل ما يحدث عند القيم الدنيا والمتوسطة والقصوى.',demo_s1:'مرحباً في Rf Alarm System! انظر إلى الشاشة الرئيسية — هنا تعمل محاكاة استكشاف الترددات.',demo_s2:'اضغط بدء وشاهد كيف يتفاعل التصوير المرئي.',demo_s3:'جرّب تعديل أدوات التحكم. كل منزلق أو زر يغيّر معاملاً محدداً.',demo_s4:'انزل للأسفل لرؤية البيانات التفصيلية. الأرقام والرسوم البيانية تتحدث في الوقت الفعلي.',demo_s5:'أحسنت! الآن جرّب قسم التحديات لاختبار فهمك لـاستكشاف الترددات.',sectionDemo:'شاهد العرض',demoPlay:'تشغيل',demoPause:'إيقاف',demoPrev:'السابق',demoNext:'التالي',learn1Title:'RF Signals',learn1Desc:'How radio frequency energy carries information. شاهد المحاكاة لرؤية هذا في الوقت الفعلي. التصور المرئي يجعل غير المرئي مرئياً.',learn1Tag:'RF',learn2Title:'Signal Analysis',learn2Desc:'How to identify and classify unknown signals. أدوات التحكم تتيح لك التجريب. كل تغيير يكشف كيف يستجيب هذا المبدأ.',learn2Tag:'SIGINT',learn3Title:'Spectrum Monitoring',learn3Desc:'How to scan and map the radio spectrum. جرّب التحديات لاختبار فهمك. المهندسون الحقيقيون يستخدمون نفس هذه المفاهيم.',learn3Tag:'Spectrum',learn4Title:'RF Security',learn4Desc:'How to detect and defend against RF threats. قارن النتائج بإعدادات مختلفة لبناء الفهم. لوحات البيانات تعرض قياسات دقيقة.',learn4Tag:'Security',sectionLearn:'ماذا ستتعلم',learnLevelVal:'متوسط 🟡',learnLevel:'المستوى:',learnTimeVal:'20 min ⏱',learnTime:'المدة:',learnAgeVal:'12+ 🧒',kidTitle:'للمستكشفين الصغار',kidIntro:'مرحباً في Rf Alarm System! هذا مثل تجربة علمية على حاسوبك. تتحكم في محاكاة حقيقية لـاتصالات الأقمار — اضغط الأزرار، حرك المنزلقات وشاهد ما يحدث على الشاشة. Try changing the controls and watch how Configure the parameters for Rf Alarm System. Choo لا شيء يمكن أن ينكسر — كل شيء محاكاة آمنة في متصفحك!',kidSafe:'آمن تماماً! لا شيء تفعله هنا يمكن أن يكسر أي شيء. كل شيء يعمل داخل متصفحك مثل لعبة.',kidTry:'اضغط على زر البدء الكبير وشاهد الشاشة تتغير. ثم جرّب تحريك المنزلقات.',kidParent:'يعلّم هذا التطبيق مفاهيم استكشاف الترددات من خلال المحاكاة التفاعلية. مناسب لتعليم STEM في الفيزياء والإلكترونيات وعلوم الحاسوب.',ch1Title:'البحث عن الإشارة',ch1Desc:'امسح نطاق الترددات واعثر على الإشارة المخفية. على أي تردد هي؟ أي تعديل تستخدم؟',ch2Title:'أرضية الضوضاء',ch2Desc:'قِس أرضية الضوضاء على ترددات مختلفة. أين هي الأدنى؟ ما المصادر التي تساهم في الضوضاء؟',ch3Title:'اختبار عرض النطاق',ch3Desc:'أرسل بيانات بعرض نطاق مختلف. كيف يؤثر ذلك على معدل البيانات وجودة الإشارة؟',codeTitle:'كود البداية',codeLang:'MicroPython (micro:bit)',codeSnippet:'from microbit import *\\nimport radio\\n\\nradio.on()\\nradio.config(group=42, power=7)\\n\\nwhile True:\\n msg = radio.receive()\\n if msg:\\n display.scroll(msg)\\n if button_a.was_pressed():\\n radio.send("HELLO")\\n display.show(Image.YES)',codeExplain:'يشغّل هذا الكود راديو micro:bit على المجموعة 42 بأقصى طاقة. عند الضغط على الزر A يبث "HELLO". أي رسالة مستلمة تمرر على شاشة LED.',purpose:'Rf Alarm System: Build an alarm, then hack it to learn security. هذه المحاكاة تتيح لك التجريب العملي بدلاً من قراءة النظرية فقط. كل معامل تغيّره ينتج نتائج مرئية، مما يبني فهماً حقيقياً لسلوك النظام.',guideTitle:'ماذا أرى على الشاشة؟',guideCanvas:'المنطقة الرئيسية تعرض تصويراً مباشراً للمحاكاة. الألوان والحركة تمثل البيانات المتغيرة في الوقت الفعلي.',guideControls:'الأزرار أسفل الشاشة الرئيسية تتحكم في المحاكاة. بدء يشغلها، إيقاف يوقفها مؤقتاً، إعادة تعيين تمسح كل شيء.',guideSections:'أسفل البطاقة الرئيسية، الأقسام تعرض البيانات التفصيلية والتحليل. انقر على العناوين لطيها أو فتحها.',guideStatus:'النقطة الملونة في أعلى اليمين تُظهر الحالة. أخضر يعني يعمل، أحمر يعني متوقف.',
 wiki_history_title: '📜 تاريخ اتصالات الأقمار',
 wiki_history: 'Sputnik (1957) was the first satellite. Telstar (1962) relayed the first TV signals. Modern constellations like Starlink use thousands of LEO satellites. يبني Rf Alarm System على هذه الأسس ليتيح لك استكشاف هذه المفاهيم التاريخية من خلال المحاكاة التفاعلية.',
 wiki_math_title: '📐 الرياضيات وراء Rf Alarm System',
 wiki_math: 'الرياضيات وراء Rf Alarm System: Free-space path loss FSPL = (4πd/λ)². For GEO at 36,000 km on C-band (4 GHz), FSPL ≈ 196 dB. Link budgets must account for this enormous loss.',
 wiki_advanced_title: '🔬 تقنيات متقدمة',
 wiki_advanced: 'بما يتجاوز الأساسيات في هذه المحاكاة، يستخدم ممارسو اتصالات الأقمار المتقدمون تقنيات متطورة. تضبط الخوارزميات التكيفية المعاملات تلقائياً. يصنف التعلم الآلي الإشارات بدقة عالية. يكتشف الارتباط متعدد القنوات أنماطاً غير مرئية للتحليل أحادي القناة. تجمع شبكات الاستشعار الموزعة البيانات من مواقع متعددة.',
 wiki_compare_title: '⚖️ مقارنة الأساليب',
 wiki_compare: 'هناك عدة أساليب في اتصالات الأقمار. توفر الحلول المادية باستخدام HackRF SDR أداءً في الوقت الفعلي. توفر المحاكاة البرمجية (مثل هذا التطبيق) تجربة آمنة بدون تكلفة المعدات. توفر المنصات السحابية قابلية التوسع لكنها تقدم تأخيراً ومخاوف تتعلق بالخصوصية. تمنحك هذه المحاكاة الأساس لاستخدام أي نهج بفعالية.',
 wiki_debug_title: '🔧 دليل استكشاف الأخطاء',
 wiki_debug: 'مشاكل شائعة في اتصالات الأقمار: (1) النتائج غير المتوقعة غالباً ما تأتي من إعدادات معاملات غير صحيحة — أعد التعيين وغير متغيراً واحداً في كل مرة. (2) إذا بدت الرسوم المتحركة متجمدة، تأكد أن المحاكاة تعمل. (3) القراءات المتقطعة تشير عادة إلى التداخل. (4) تحقق من وحداتك (هرتز مقابل كيلوهرتز مقابل ميغاهرتز).',
 wiki_ethics_title: '⚖️ الأخلاقيات والاعتبارات القانونية',
 wiki_ethics: 'اتصالات الأقمار يحمل مسؤوليات أخلاقية وقانونية مهمة. تنظم العديد من الدول استخدام HackRF SDR والمعدات المماثلة. احصل دائماً على إذن مناسب قبل اختبار أنظمة لا تملكها. احترم قوانين الخصوصية وحماية البيانات. هذه المحاكاة مصممة لأغراض تعليمية — تعرض المبادئ دون إرسال إشارات حقيقية أو الوصول لشبكات فعلية.',
 gloss1_term: 'Link Budget',
 gloss1_def: 'An accounting of all gains and losses in a communication link: transmit power + antenna gains - path loss - atmospheric loss = received signal level.',
 gloss2_term: 'Latency',
 gloss2_def: 'Time delay between sending and receiving data. Lower latency means faster response. Measured in milliseconds (ms).',
 ...
 theory: 'Rf Alarm System يوضح المبادئ الأساسية في اتصالات الأقمار. Communication satellites relay signals between ground stations. GEO satellites orbit at 35,786 km providing wide coverage; LEO satellites at 200-2000 km provide low latency. تحاكي هذه المحاكاة الآليات الحقيقية باستخدام معادلات رياضية في متصفحك. كل عنصر تحكم يتوافق مع معامل حقيقي. بالتجربة هنا تبني نفس الحدس الذي يطوره المحترفون عبر سنوات من الخبرة العملية.',
 faq_q9: 'ما الأخطاء الشائعة التي يجب تجنبها؟',
 faq_a9: 'الخطأ الأكثر شيوعاً هو تغيير معاملات متعددة في وقت واحد مما يجعل فهم السبب والنتيجة مستحيلاً. غير دائماً شيئاً واحداً في كل مرة. خطأ شائع آخر هو تجاهل سجل النشاط — فهو يسجل كل حدث ويساعدك على فهم تسلسل العمليات. أخيراً لا تتخط قسم التحديات: تلك الأسئلة تختبر فهمك الحقيقي للمفاهيم.',
 faq_q10: 'كيف يرتبط هذا بـsatellite communications في العالم الحقيقي؟',
 faq_a10: 'تحاكي هذه المحاكاة نفس الفيزياء والرياضيات المستخدمة في الأنظمة المهنية. تتوافق المعاملات التي تضبطها مع إعدادات المعدات الحقيقية. تعرض الرسوم البيانية أنماط بيانات مطابقة لما تراه على الأجهزة المهنية. الفرق الرئيسي هو أن هذا يعمل بأمان في متصفحك — تستخدم الأنظمة الحقيقية أجهزة HackRF SDR وقد تتطلب تراخيص قانونية للتشغيل.',
 glossTitle: '📚 مصطلحات أساسية',learnAge:'العمر:',relatedTitle:'\ud83d\udd17 تطبيقات ذات صلة',related1_name:'\\u0645\\u062e\\u062a\\u0628\\u0631 \\u062d\\u0631\\u0645\\u0627\\u0646 \\u0627\\u0644\\u0637\\u064a\\u0641',related1_desc:'\\u0645\\u062d\\u0627\\u0643\\u0627\\u0629 \\u0645\\u0646\\u0627\\u0637\\u0642 \\u062d\\u0631\\u0645\\u0627\\u0646 RF',related1_path:'../../54-rf-warfare/rfw-spectrum-denial-lab/index.html',related2_name:'مصنف الإشارات',related2_desc:'تعرف على أنواع التعديل بتحليل ذكي',related2_path:'../../32-sdr-tools/sdr-signal-classifier/index.html',related3_name:'\\u0645\\u062d\\u0627\\u0643\\u064a \\u062a\\u0632\\u064a\\u064a\\u0641 GPS',related3_desc:'\\u0645\\u062d\\u0627\\u0643\\u0627\\u0629 \\u062a\\u0632\\u064a\\u064a\\u0641 \\u0627\\u0644\\u0623\\u0642\\u0645\\u0627\\u0631 \\u0627\\u0644\\u0635\\u0646\\u0627\\u0639\\u064a\\u0629',related3_path:'../../54-rf-warfare/rfw-gps-spoofing-sim/index.html',pathTitle:'\ud83d\udee4\ufe0f مسار التعلم',pathPrev_name:'رادار مصغّر — خريطة EM',pathPrev_path:'../../11-hrf-microbit/bit-micro-radar/index.html',pathNext_name:'جهاز تحكم لاسلكي — ابنِ بروتوكولك',pathNext_path:'../../11-hrf-microbit/bit-rf-remote-control/index.html',
 printBtn: '🖨️ طباعة',quizTab:'اختبار',quizTitle:'اختبر معلوماتك',quizRetry:'إعادة',quizCorrect:'صحيح!',quizWrong:'خطأ!',quizScore:'النتيجة',quiz_q1:'ما هي الشبكة العصبية؟',quiz_q1a:'أسلاك مادية',quiz_q1b:'نظام حوسبة مستوحى من الخلايا العصبية',quiz_q1c:'شبكة اجتماعية',quiz_q1d:'شبكة راديو',quiz_q1_answer:'1',quiz_q2:'ما المعالج الذي يستخدمه micro:bit V2؟',quiz_q2a:'ATmega328',quiz_q2b:'nRF52833',quiz_q2c:'ESP32',quiz_q2d:'STM32',quiz_q2_answer:'1',quiz_q3:'ماذا تعني AI؟',quiz_q3a:'إدخال آلي',quiz_q3b:'الذكاء الاصطناعي',quiz_q3c:'واجهة تناظرية',quiz_q3d:'تكامل نشط',quiz_q3_answer:'1',quiz_q4:'من اخترع شيفرة مورس؟',quiz_q4a:'تسلا',quiz_q4b:'صامويل مورس',quiz_q4c:'إديسون',quiz_q4d:'بيل',quiz_q4_answer:'1',quiz_q5:'كم عدد مصابيح LED في شاشة micro:bit؟',quiz_q5a:'16',quiz_q5b:'25',quiz_q5c:'36',quiz_q5d:'64',quiz_q5_answer:'1',realworldTitle:'🌍 قصص واقعية',realworld1:'كانت عملية آيفي بيلز في السبعينيات والثمانينيات مهمة مشتركة بين وكالة الأمن القومي والبحرية للتنصت على كابلات الاتصالات السوفيتية تحت البحر.',realworld2:'تبث محطات الأرقام رسائل مشفرة عبر الموجات القصيرة للعملاء الميدانيين منذ الحرب الباردة.',realworld3:'كشفت قضية ألدريتش أيمز (1994) كيف استخدم عميل مزدوج نقاط التسليم السرية لنقل معلومات استخبارية مصنفة للاتحاد السوفيتي لمدة عقد تقريبًا.',experimentTitle:'🔬 تجارب',experiment_1_title:'القياس المرجعي',experiment_1:'اضبط جميع عناصر التحكم على القيم الافتراضية وسجّل القراءات الأولية. هذه هي قياساتك المرجعية. يقوم العالم الجيد دائمًا بتحديد خط الأساس قبل تغيير المتغيرات — فهو يمنحك نقطة مرجعية لقياس جميع التغييرات المستقبلية.',experiment_2_title:'تحليل الحساسية',experiment_2:'غيّر معلمة واحدة إلى قيمتها الدنيا وسجّل النتيجة ثم اضبطها على الحد الأقصى. يكشف الفرق عن حساسية النظام لهذا المتغير. في المراقبة معرفة المعلمات الأكثر أهمية تساعدك على تركيز جهودك بكفاءة.',experiment_3_title:'تأثيرات التفاعل',experiment_3:'بعد اختبار المعلمات بشكل فردي غيّر اثنتين في وقت واحد. هل التأثير المشترك يساوي مجموع التأثيرات الفردية؟ التفاعلات غير الخطية شائعة في المراقبة وتكشف التعقيد الخفي تحت أنظمة تبدو بسيطة.',wiki_concept_title:'💡 المفهوم الأساسي',wiki_concept:'Rf Alarm System يوضح مفهومًا أساسيًا في المراقبة. تحاكي هذه المحاكاة كيفية معالجة الأنظمة الحقيقية للإشارات والبيانات أو الظواهر الفيزيائية. الفكرة الرئيسية هي أن السلوكيات المعقدة تنشأ من قواعد بسيطة تُطبق بشكل متكرر.',wiki_realworld_title:'🌐 التطبيقات الواقعية',wiki_realworld:'المبادئ المعروضة في Rf Alarm System لها تطبيقات مباشرة في العالم الحقيقي. يستخدم المحترفون في المراقبة هذه المفاهيم نفسها يوميًا. في الصناعة يُنفذ ESP32 وأجهزة مماثلة هذه الخوارزميات في أنظمة مدمجة.',wiki_safety_title:'⚠️ السلامة والمسؤولية',wiki_safety:'العمل في مجال المراقبة يحمل مسؤوليات مهمة. تعمل دائمًا ضمن الحدود القانونية. هذه المحاكاة مصممة للاستخدام التعليمي الآمن — لا ترسل إشارات حقيقية ولا تصل إلى شبكات حقيقية.',proTipTitle:'💡 نصائح احترافية',proTip1:'استخدم التنقل بين القنوات (1، 6، 11) لمسح الواي فاي — هذه هي القنوات غير المتداخلة الوحيدة في نطاق 2.4 جيجاهرتز وتلتقط 90% من حركة المرور.',proTip2:'أضف مكثفًا بسعة 10 ميكروفاراد عبر دبابيس الطاقة في ESP32. يسبب إرسال الواي فاي ذروات تيار يمكن أن تتسبب في تعطل اللوحة.',funFactTitle:'🎯 هل تعلم؟',funFact:'خلال الحرب الباردة، بنت وكالة المخابرات المركزية يعسوبًا آليًا في السبعينيات لحمل ميكروفون مصغر. تم التخلي عن المشروع لأنه لم يستطع الطيران في الرياح.',mistakeTitle:'⚠️ أخطاء شائعة',mistake1:'تغيير عدة معلمات في وقت واحد يجعل من المستحيل عزل السبب والنتيجة. غيّر دائمًا متغيرًا واحدًا فقط في كل مرة.',mistake2:'تخطي القياس المرجعي. بدون معرفة السلوك الافتراضي لا يمكنك قياس تأثير تغييراتك على النظام.',peerTitle:'👥 \u0648\u0636\u0639 \u0627\u0644\u0646\u0638\u064a\u0631',peerConnect:'\u0627\u062a\u0635\u0627\u0644',peerDisconnect:'\u0642\u0637\u0639',peerStatus:'\u062d\u0627\u0644\u0629 \u0627\u0644\u0646\u0638\u064a\u0631',peerSend:'\u0623\u0631\u0633\u0644',peerReceive:'\u0627\u0633\u062a\u0644\u0645',peerInfo:'\u0627\u0641\u062a\u062d \u0647\u0630\u0627 \u0627\u0644\u062a\u0637\u0628\u064a\u0642 \u0641\u064a \u062a\u0628\u0648\u064a\u0628\u064a\u0646 \u0644\u0644\u0645\u0632\u0627\u0645\u0646\u0629',heatmapTitle:'📅 \u062e\u0631\u064a\u0637\u0629 \u0627\u0644\u0646\u0634\u0627\u0637',heatmapToday:'\u0627\u0644\u064a\u0648\u0645',heatmapStreak:'\u0633\u0644\u0633\u0644\u0629',heatmapTotal:'\u0627\u0644\u0645\u062c\u0645\u0648\u0639',heatmapLegend:'\u0623\u0642\u0644 \u2192 \u0623\u0643\u062b\u0631',mistake3:'تجاهل سجل النشاط. يسجل كل حدث مع طوابع زمنية — ضروري لفهم التسلسلات وتصحيح النتائج غير المتوقعة.'}
...
// Alarm state: 'disarmed' | 'armed' | 'triggered'
let alarmState = 'disarmed';
let secureMode = false;
let jamActive = false;
let jamInterval = null;
let replayMode = 'idle'; // 'idle' | 'recording' | 'hasCapture'
let capturedFrame = null;
let rollingCounter = 1000;
let lastUsedCode = null;
let keypadBuffer = '';

function generateRollingCode() {
 rollingCounter++;
 return 'RC-' + rollingCounter.toString(16).toUpperCase().padStart(4, '0');
}

function getCurrentCode() {
 if (secureMode) {
 return generateRollingCode();
 }
 return '1234';
}
...
function updateAlarmDisplay() {
 ...
 const panelEl = $('alarmPanel');
 if (!statusEl || !panelEl) return;

 statusEl.classList.remove('armed', 'disarmed', 'alert');

 if (alarmState === 'armed') {
 statusEl.textContent = s.alarmArmed;
 statusEl.classList.add('armed');
 } else if (alarmState === 'triggered') {
 statusEl.textContent = s.alarmAlert;
 statusEl.classList.add('alert');
 } else {
 statusEl.textContent = s.alarmDisarmed;
 statusEl.classList.add('disarmed');
 }

 panelEl.classList.toggle('jamming', jamActive);
}
...
function updateSignalMeter() {
 ...
 if (!fill) return;
 if (jamActive) {
 fill.style.width = (5 + Math.random() * 15) + '%';
 fill.classList.add('poor');
 } else {
 fill.style.width = '100%';
 fill.classList.remove('poor');
 }
}

function showRollingCode(code) {
 const el = $('rollingCodeDisplay');
 if (!el) return;
 const s = LANG[currentLang];
 if (secureMode) {
 el.textContent = s.msgRollingCode + code;
 } else {
 el.textContent = s.msgCodeUsed + code;
 }
}
...
function armRemote() {
 ...
 showRollingCode(code);
 alarmState = 'armed';
 updateAlarmDisplay();
 log(s.msgArmed, 'success');
 playSound('success');
 setStatus(true);
 }
}
...
function disarmRemote() {
 ...
 if (replayMode === 'recording') {
 capturedFrame = { code: code, secureMode: secureMode, counter: rollingCounter };
 replayMode = 'hasCapture';
 log(s.msgRecorded, 'success');
 const btn = $('replayBtn');
 if (btn) btn.querySelector('span').textContent = '▶ ' + s.replayBtn;
 }

 if (alarmState === 'armed' || alarmState === 'triggered') {
 alarmState = 'disarmed';
 updateAlarmDisplay();
 log(s.msgDisarmed, 'success');
 playSound('success');
 setStatus(false);
 }
}
...
function toggleJam() {
 ...
 if (jamActive) {
 log(s.msgJamStart, 'error');
 playSound('error');
 // Animate signal meter during jamming
 jamInterval = setInterval(updateSignalMeter, 200);
 updateSignalMeter();
 showToast(s.msgJamStart, 2000);
 } else {
 log(s.msgJamStop, 'info');
 playSound('click');
 if (jamInterval) { clearInterval(jamInterval); jamInterval = null; }
 updateSignalMeter();
 }
 ...
 const btn = $('jamBtn');
 if (btn) {
 btn.classList.toggle('active', jamActive);
 }
}
...
function toggleReplay() {
 ...
 capturedFrame = null;
 log(s.msgRecording, 'info');
 showToast(s.msgRecording, 2000);
 const btn = $('replayBtn');
 if (btn) btn.querySelector('span').textContent = '🔴 ' + s.replayBtn;

 } else if (replayMode === 'recording') {
 // Cancel recording
 replayMode = 'idle';
 const btn = $('replayBtn');
 if (btn) btn.querySelector('span').textContent = s.replayBtn;

 } else if (replayMode === 'hasCapture') {
 // Replay the captured frame
 log(s.msgReplaying, 'tx');
 ...
 if (!capturedFrame) {
 log(s.msgNoCapture, 'error');
 playSound('error');
 return;
 }

 showRollingCode(capturedFrame.code);

 // In basic mode, replay always works
 if (!secureMode) {
 if (alarmState === 'armed' || alarmState === 'triggered') {
 alarmState = 'disarmed';
 updateAlarmDisplay();
 log(s.msgReplayOk, 'success');
 playSound('success');
 setStatus(false);
 }
 } else {
 // In secure mode, check if code was already used (it was — so fail)
 log(s.msgReplayFail, 'error');
 playSound('error');
 // Trigger alert on failed attempt
 if (alarmState === 'armed') {
 alarmState = 'triggered';
 updateAlarmDisplay();
 log(s.msgAlertTriggered, 'error');
 }
 }
...
function keypadPress(key) {
 ...
 keypadBuffer += key;
 if (keypadBuffer.length > 6) keypadBuffer = keypadBuffer.slice(-6);
 }
 const display = $('keypadDisplay');
 if (display) display.textContent = '*'.repeat(keypadBuffer.length);
}

function keypadSubmit() {
 const s = LANG[currentLang];
 const code = keypadBuffer;
 keypadBuffer = '';
 const display = $('keypadDisplay');
...
function toggleSecurityMode() {
 ...
 const sw = $('securitySwitch');
 if (sw) sw.classList.toggle('active', secureMode);

 if (secureMode) {
 log(s.msgSecureOn, 'success');
 playSound('success');
 } else {
 log(s.msgSecureOff, 'info');
 playSound('click');
 }

 // Reset replay state when switching modes
 replayMode = 'idle';
 capturedFrame = null;
 const btn = $('replayBtn');
 if (btn) btn.querySelector('span').textContent = s.replayBtn;
...
function initAlarmSimulation() {
 updateAlarmDisplay();
 updateSignalMeter();
 log('🚨 RF Alarm Simulation loaded', 'info');
 log('📡 Channel: 433 MHz | Mode: Basic (no encryption)', 'info');
}
